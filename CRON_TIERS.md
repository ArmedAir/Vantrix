...
- For `billing-recovery` / `character-refund-recovery` / `message-recovery`
  (money- and message-delivery–sensitive, every 5 min), GitHub Actions'
  schedule timing is best-effort and can lag under load. If exact timing
  matters before you're ready for Pro, point a free precise pinger (e.g.
  cron-job.org) at those routes instead of relying on the generated
  workflow for just those — see "Precise-pinger stopgap" below for the
  exact setup.
- `character-refund-recovery` is the repair-worker pattern applied to a
  gap this repo's own token-economy audit flagged: character creation can
  fail *after* the token charge lands, the route attempts a compensating
  refund, and if that refund call itself fails, the user was left
  charged with no character and no automated way back. Same DLQ-and-retry
  shape as `billing-recovery` (own queue, not shared — see
  character-refund-dlq.ts's header for why), same 5-min cadence, same
  heartbeat coverage.
...
## Precise-pinger stopgap for `billing-recovery` / `character-refund-recovery` / `message-recovery`

SCALE-FIX (2026-09-04): this was previously just a suggestion ("point a
free precise pinger at those two routes") with no concrete steps. Turning
it into a runbook, since it's cheaper to do this now than to research it
during an actual billing/message-delivery incident. (`character-refund-recovery`
joined the two originals later, same reasoning, same cadence — add it to
the same pinger setup rather than skipping it.)

1. Sign up for a free account at [cron-job.org](https://cron-job.org) (or
   healthchecks.io / BetterStack — anything that supports a custom header
   and a 1–5 minute interval).
2. Create three cron jobs, all hitting your real deployment on a 5-minute
   interval (cron-job.org's free tier supports down to 1 minute — 5 is
   what these routes actually expect, matching their declared `schedule`
   in `config/cron-jobs.mjs`, so there's no benefit to going tighter):
   - `GET https://<your-domain>/api/cron/billing-recovery`
   - `GET https://<your-domain>/api/cron/character-refund-recovery`
   - `GET https://<your-domain>/api/cron/message-recovery`
3. On all three jobs, add a custom request header:
   `Authorization: Bearer <your CRON_SECRET value>` — the same secret
   already set as the `CRON_SECRET` env var on Vercel and as the
   `CRON_SECRET` GitHub repo secret. `requireCronAuth()` accepts this
   header from any caller, not just Vercel/GitHub — that's what makes
   this work without touching any application code.
4. Nothing needs to change in `vantrix-free-tier-crons.yml` or
   `config/cron-jobs.mjs` — the GitHub Actions version of these jobs
   can keep running as a redundant second trigger. `src/lib/cron/lock.ts`'s
   distributed lock (see Notes above) already makes a double-fire within
   the same window a harmless no-op, so overlap between the pinger and
   the GitHub Actions workflow is safe, not something to avoid.
5. If a `HEARTBEAT_BILLING_RECOVERY` / `HEARTBEAT_MESSAGE_RECOVERY` URL is
   configured (see `src/lib/cron/heartbeat.ts`), the dead-man's-switch
   monitor will now report accurately for these two jobs regardless of
   which trigger fired them.

This closes the gap without requiring a Vercel Pro upgrade — worth doing
even after moving to Pro, since it's a second independent trigger path
rather than a dependency on any one scheduler.
