...
const nextConfig = {
  ...
  experimental: {
    ...
    // constrain on a genuinely memory-limited build host (opt in with
    // CONSTRAINED_BUILD_HOST=true — see the `*:sandbox` npm scripts),
    // where single-threaded compilation is the tradeoff that keeps `next
    // build` from OOMing. Do NOT hardcode cpus:1 here again — that was
    // previously forcing single-threaded compilation on every machine
    // (including full dev workstations with plenty of cores/RAM to spare),
    // not just the constrained host it was meant for.
    ...(process.env.CONSTRAINED_BUILD_HOST === "true"
      ? { cpus: 1, workerThreads: false }
      : {
          // BUILD-OPT (2026-09-06): the webpack() function below (async_hooks
          // fallback + warning suppression) makes this a "custom webpack
          // configuration" in Next's own terms, which silently disables its
          // webpack build-worker parallelization by default — see
          // https://nextjs.org/docs/messages/parallel-build-without-worker.
          // Forcing it back on is Next's own documented fix for exactly this
          // situation. Only applied on the full-power path, not the
          // constrained-host one above: a build worker is a second process,
          // trading memory for speed — the opposite of what
          // CONSTRAINED_BUILD_HOST exists to do.
          webpackBuildWorker: true,
        }),
    // BUILD-OPT (2026-09-06): stable since Next 15.0.0, documented as
    // low-risk. Trims webpack's peak memory at a slight compile-time cost —
    // exactly the trade this app already needs headroom for (see the 8GB
    // NODE_OPTIONS heap bump on build/lint/typecheck in package.json).
    // Left on unconditionally — helpful on both the constrained and
    // full-power paths, unlike webpackBuildWorker above.
  ...
  serverExternalPackages: [
    ...
    // paying to bundle it.
    "posthog-node",
    // VERCEL-BRAIN: @xenova/transformers ships prebuilt native ONNX
    // Runtime binaries (onnxruntime-node) it loads via a runtime
    // require(), not a static import — webpack can't trace through that
    // and either fails the build trying to bundle a .node binary or
    // silently produces a broken bundle that throws at cold start. Same
    // class of problem as sharp above, same fix: leave both as real
    // Node `require()`s at runtime instead of attempting to bundle them.
    // Only src/lib/ai/local-brain.ts (used exclusively by the
    // src/app/api/brain/* routes, see that module's header) imports
    // this, so external-izing it doesn't affect any other route's
    // bundle size.
    "@xenova/transformers",
    "onnxruntime-node",
  ],
