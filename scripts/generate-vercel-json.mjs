...
const OTHER_FUNCTIONS = {
  ...
  // VERCEL-BRAIN: local-brain.ts's transformers.js model can take a few
  // seconds to load on a cold container (~35MB quantized ONNX weights) on
  // top of the actual batch-encode work — 30s comfortably covers a cold
  // start plus inference on any tier, well under Hobby's 60s ceiling so
  // capDuration() never needs to clamp these.
  'src/app/api/brain/health/route.ts': 30,
  'src/app/api/brain/embed/route.ts':  30,
...
// ── functions{} ──────────────────────────────────────────────────────────
// FUNCTIONS-CAP FIX: vercel.json's `functions` map is capped at 50
// properties by Vercel's schema validation — one entry per CRON_JOBS route
// (41) plus OTHER_FUNCTIONS (25) is 66, which fails deployment outright
// ("functions should NOT have more than 50 properties"). Almost every cron
// job lives at src/app/api/cron/<name>/route.ts, so those are collapsed
// into a single glob entry capped at the same ceiling every individual job
// was already being capped to (capDuration never lets any of them exceed
// FREE_MAX_DURATION on `free`, so granting the glob's routes that same
// ceiling changes nothing about actual allowed runtime). The one cron job
// that doesn't live under api/cron/ (queue/worker) keeps its own entry.
const functions = {};
const cronGlobJobs = CRON_JOBS.filter((j) => j.path.startsWith('/api/cron/'));
const nonGlobCronJobs = CRON_JOBS.filter((j) => !j.path.startsWith('/api/cron/'));
if (cronGlobJobs.length > 0) {
  const globMaxDuration = Math.max(...cronGlobJobs.map((j) => capDuration(j.maxDuration)));
  functions['src/app/api/cron/*/route.ts'] = { maxDuration: globMaxDuration };
}
for (const job of nonGlobCronJobs) {
