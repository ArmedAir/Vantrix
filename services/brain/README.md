...
**Deploying on Vercel instead of this container:** this service can't run
as-is on Vercel (no persistent process, torch is too heavy for a serverless
bundle) — but you don't need it there. `src/lib/ai/local-brain.ts` is a
transformers.js (ONNX, no Python/torch) port of the exact same model,
exposed as native Next.js routes at `src/app/api/brain/{health,embed,rerank}`.
`env.ts` auto-points `BRAIN_SERVICE_URL` at those routes on any Vercel
deployment when the var isn't explicitly set — just set
`BRAIN_SERVICE_API_KEY` too (see `brain-service-auth.ts`) so the self-call
authenticates. Set `BRAIN_SERVICE_URL` explicitly (e.g. to this container's
Fly.io/Railway/VPS address) to keep using the Python sidecar from a
Vercel-hosted frontend instead — an explicit value always wins over the
auto-default.
