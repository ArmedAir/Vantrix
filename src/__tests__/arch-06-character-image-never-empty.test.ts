...
import {
  resolveImageSrc,
  CHARACTER_IMAGE_FALLBACK,
  WORLD_IMAGE_FALLBACK,
  SCENARIO_IMAGE_FALLBACK,
} from '../lib/utils';
...
describe('ARCH-06 — CONSOLIDATED FALLBACK: world/scenario placeholders removed (2026-09-12)', () => {
  it('WORLD_IMAGE_FALLBACK and SCENARIO_IMAGE_FALLBACK are aliases of CHARACTER_IMAGE_FALLBACK, not separate assets', () => {
    expect(WORLD_IMAGE_FALLBACK).toBe(CHARACTER_IMAGE_FALLBACK);
    expect(SCENARIO_IMAGE_FALLBACK).toBe(CHARACTER_IMAGE_FALLBACK);
  });

  it('the old dedicated placeholder images no longer exist in public/', () => {
    const publicDir = join(__dirname, '..', '..', 'public');
    expect(existsSync(join(publicDir, 'images', 'world-placeholder.png'))).toBe(false);
    expect(existsSync(join(publicDir, 'images', 'scenario-placeholder.png'))).toBe(false);
  });
});
