...
describe('ARCH-10 — GET /api/universe/daily-choice returns userVote as a bare option, never wrapped', () => {
  it("getUserVote()'s return type is the option itself, not an object", () => {
    const file = src('lib', 'universe', 'daily-choice.ts');
    expect(file).toMatch(/export async function getUserVote\([^)]*\): Promise<"a" \| "b" \| null>/);
  });

  it("getUserVote() never silently swallows a query error (regression: .maybeSingle() errors on 2+ rows and only `data` was destructured, making a real error look like \"no vote\")", () => {
    const file = src('lib', 'universe', 'daily-choice.ts');
    const fnStart = file.indexOf('export async function getUserVote');
    const fnBody = file.slice(fnStart, file.indexOf('\n}', fnStart));
    expect(fnBody).toMatch(/const\s*\{\s*data,\s*error\s*\}/);
    expect(fnBody).toMatch(/if\s*\(error\)\s*\{/);
    expect(fnBody).not.toMatch(/\.maybeSingle\(\)/);
  });

  it('the route never wraps userVote in an object before returning it', () => {
    const file = src('app', 'api', 'universe', 'daily-choice', 'route.ts');
    expect(file).not.toMatch(/userVote:\s*\{\s*option/);
  });

  it(
    'ROOT-CAUSE REGRESSION ("voting keeps repeating after already voted"): the frontend data hook ' +
      'reads body.userVote directly, never as body.userVote.option or body.userVote?.option — the ' +
      'route has always returned userVote as the bare "a"|"b"|null option, so `.option` access always ' +
      'evaluated to undefined and the client could never learn from a fresh GET that it had already voted',
    () => {
      const file = src('hooks', 'use-daily-choice.ts');
      expect(file).toMatch(/body\.userVote\s*\?\?/);
      expect(file).not.toMatch(/body\.userVote\??\.option/);
    },
  );

  it('the card component no longer does its own fetch/parsing — it consumes the typed hook', () => {
    const file = src('components', 'world', 'daily-choice-card.tsx');
    expect(file).toMatch(/useDailyChoice\(\)/);
    expect(file).not.toMatch(/fetch\(["']\/api\/universe\/daily-choice["']\)/);
  });

  it('the GET/POST response shapes are imported from the shared, precisely-typed API contract on both ends', () => {
    const routeFile = src('app', 'api', 'universe', 'daily-choice', 'route.ts');
    const hookFile = src('hooks', 'use-daily-choice.ts');
    expect(routeFile).toMatch(/from ["']@\/types\/daily-choice-api["']/);
    expect(hookFile).toMatch(/from ["']@\/types\/daily-choice-api["']/);
  });
});
