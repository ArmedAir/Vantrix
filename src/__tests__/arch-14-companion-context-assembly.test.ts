...
vi.mock('@/lib/ai/attachment-engine',     () => ({
  getPsychology: vi.fn().mockResolvedValue({ psych: true }),
  // RESILIENCE-1: companion-context.ts now imports this as the fail-open
  // fallback for getPsychology — needs a value here purely so the mocked
  // module has the export, not exercised by the happy-path assertions below.
  DEFAULT_PSYCHOLOGY: { mockDefaultPsychology: true },
}));
vi.mock('@/lib/ai/relationship-engine',   () => ({ ensureRelationship: vi.fn().mockResolvedValue({ rel: true }) }));
vi.mock('@/lib/ai/memory-graph',          () => ({
  // RaaS memory-tier gating (companion-context.ts's gatedMemoryGraph,
  // via lib/commerce/raas.ts) filters this by event_time and slices to
  // a cap before it reaches ctx.memory.graph — getRelationshipTier isn't
  // mocked in this file, so it fails open to 'spark' (see raas.ts), whose
  // policy is a 7-day window / 5-node cap. A plain `{ graph: true }`
  // object here made assembleCompanionContext's `.filter()` throw before
  // ever reaching the assertions below; an array with a `event_time`
  // inside that window is what the real getMemoryGraph() (Promise<MemoryNode[]>)
  // actually returns, and survives the gate unfiltered since one recent
  // node is well under both the window and the cap.
  getMemoryGraph: vi.fn().mockResolvedValue([{ graph: true, event_time: new Date().toISOString() }]),
...
vi.mock('@/lib/ai/character-revolution',  () => ({
  getRevolutionProfile: vi.fn().mockResolvedValue({ rev: true }),
  // RESILIENCE-1: companion-context.ts's fail-open fallback for
  // getRevolutionProfile builds a stub profile from these two exports —
  // not exercised by the happy-path assertions below, just needs to exist.
  buildAttachmentProfile: vi.fn().mockReturnValue({ mockAttachment: true }),
  RELATIONSHIP_GOALS: [],
}));
...
// MEMORY-TIERS: new independent subsystem wired into the same Promise.all
// as the five above — same hang-not-reject risk if left unmocked (see the
// COMPANION-STATE CONSOLIDATION note above this block).
vi.mock('@/lib/memory-tiers/memory-tier-engine', () => ({ getTieredMemoryContext: vi.fn().mockResolvedValue({ tieredMemory: true }) }));
vi.mock('@/lib/logger', () => ({ logger: { debug: vi.fn(), info: vi.fn(), warn: vi.fn(), error: vi.fn() } }));
// SANCTUARY-MODE: isSanctuaryMode() (companion-context.ts) queries
// conversations.sanctuary_mode directly via supabaseAdmin whenever
// conversationId is non-null — same hang-not-reject risk as the
// COMPANION-STATE CONSOLIDATION block above if left unmocked here.
vi.mock('@/lib/supabase/admin', () => ({
  supabaseAdmin: {
    from: () => ({
      select: () => ({
        eq: () => ({
          maybeSingle: () => Promise.resolve({ data: { sanctuary_mode: false }, error: null }),
        }),
      }),
    }),
  },
...
describe('ARCH-14 — CompanionContext assembly', () => {
  it('places every engine output under its declared precedence group', async () => {
    ...
    // Precedence 4 — memory
    // Array-of-one, not the raw mock object directly — RaaS gating (spark
    // tier, see the getMemoryGraph mock's own comment above) is real
    // pass-through logic being exercised here, not incidental; matching
    // its actual output shape is the point, not a workaround for it.
    expect(ctx.memory.graph).toEqual([expect.objectContaining({ graph: true })]);
    ...
    // Also RaaS-gated at spark tier (includePriorityMemories: false, see
    // raas.ts) — gated to [] regardless of what getPriorityMemories
    // itself resolves, same reasoning as ctx.memory.graph above.
    expect(ctx.memory.priority).toEqual([]);
    ...
    expect(ctx.cognition.tieredMemory).toEqual({ tieredMemory: true });
