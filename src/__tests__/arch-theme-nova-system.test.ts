...
describe('ARCH-THEME-NOVA-SYSTEM — toggle plumbing', () => {
  ...
  it('the toggle is mounted in the authenticated rail and the signed-out header, not the mobile top bar', () => {
    // THEME-TOGGLE REMOVED (see top-bar.tsx's own doc comment): TopBar
    // deliberately dropped ThemeToggle on every breakpoint; Sidebar's
    // footer (desktop, `variant="sidebar"`) and PublicHeader (signed-out)
    // are the two places a visitor can actually reach it now — signed-in
    // mobile falls back to Profile  Settings' full ThemePicker instead.
