 ...
 *
 * PATCH /api/profile/edit accepts `avatar_url` (previously writable
 * nowhere despite /api/upload existing and GET already reading it back
 * — see that route's own AVATAR-FIX comment, now on profile/edit/route.ts
 * after the PROFILE / SETTINGS SPLIT moved identity fields off of
 * profile/settings/route.ts). The one thing that write path can't skip
 * is host validation: without it, a user could set their avatar to any
 * arbitrary URL rather than something that actually went through
 * /api/upload's magic-byte/size/type checks.
 *
 * Mirrors the guard in profile/edit/route.ts (same shape as sec-01's
 * sanitizeRedirect tests: pure logic, no Next.js request/route plumbing
 * needed) — reuses the real, already-exported isAllowedImageHost from
 * lib/utils.ts rather than re-implementing it, since that's the actual
...
/** Exact validation shape used in profile/edit/route.ts's PATCH handler. */
