...
describe('tierToPriority', () => {
  // TWO-TIER MODEL: 'premium' is the only paid tier (tiers/config.ts) and
  // gets the top 'high' lane outright, so its jobs always dequeue ahead of
  // free-tier jobs. Legacy tier ids (elite/enterprise/basic/spark) were
  // fully retired by the Aug 2026 single-plan refactor and now fall
  // through to the same 'low' bucket as an unrecognized string.
  it('maps premium to high', async () => {
    const { tierToPriority } = await import('../lib/queue/index');
    expect(tierToPriority('premium')).toBe('high');
  });

  it('maps free and any unrecognized/legacy tier id to low', async () => {
    const { tierToPriority } = await import('../lib/queue/index');
    expect(tierToPriority('free')).toBe('low');
    expect(tierToPriority('elite')).toBe('low');
    expect(tierToPriority('enterprise')).toBe('low');
    expect(tierToPriority('basic')).toBe('low');
