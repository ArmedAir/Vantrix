...
afterEach(() => {
  vi.unstubAllGlobals();
  vi.restoreAllMocks();
  // MODERATION-FALLBACK FIX: aiModerationCheck() now retries once on the
  // same 'ai:openrouter' breaker (pinned model, then the NANO-tier fallback
  // model — both go through the 'openrouter' provider) before failing
  // closed. getCircuitBreaker() is a module-level singleton registry that
  // persists across tests in this file, so the deliberately-failing tests
  // above now trip its failureThreshold twice as fast as before (two calls
  // per test instead of one) and can leave it OPEN for later tests, making
  // an intentionally-healthy mock look like a failure for reasons unrelated
  // to what that test is checking. Reset it after every test so each one
  // starts from a clean CLOSED state, same as it implicitly relied on
  // before this file's call count changed.
