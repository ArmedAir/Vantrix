  ...
  return (
    ...
    <div className="mx-auto max-w-3xl px-4 md:px-8 py-6">
      ...
      */}
      ...
      {/* RaaS surfacing: paid relationship tiers (bond/soulbound) already
          exist server-side (lib/commerce/raas.ts, purchase_relationship_tier
          RPC) but had no UI anywhere — see the component's own comment.
          Same signed-out/owner no-op posture as the widget above it. */}
      <CharacterRelationshipTier characterId={character.id} />
      ...
      <div className="mt-8 flex flex-col items-center gap-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <RemixButton characterId={character.id} />
        </div>
        {(remixSourceName || character.remix_count > 0) && (
          <div className="flex items-center gap-2 text-[11px] text-text-tertiary">
            {remixSourceName && <span>Remixed from {remixSourceName}</span>}
            {remixSourceName && character.remix_count > 0 && <span>·</span>}
            {character.remix_count > 0 && (
              <span>{character.remix_count} remix{character.remix_count === 1 ? "" : "es"}</span>
            )}
          </div>
        )}
            ...
            {user ? (
              ...
              <div className="flex flex-col items-center gap-3 py-12 text-center">
                ...
                <Button asChild size="sm">
                  <Link href={`/login?redirect=/characters/${character.id}`}>Sign In</Link>
                </Button>
              </div>
            )}
          </TabsContent>
