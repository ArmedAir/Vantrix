        ...
        <div className="min-w-0 flex-1">
          ...
          <ShareCardButton
            ...
            request={{
              ...
              // SHARE-CARD-PLACEHOLDER FIX: resolveImageSrc() exists to keep
              // next/image from hard-erroring on an un-allowlisted remote
              // host (see its own comment in lib/utils.ts) — it silently
              // substitutes CHARACTER_IMAGE_FALLBACK for any host not on
              // that list, even though the URL itself may be perfectly
              // fetchable. That's the right call for *rendering this page*,
              // but wrong for the share-card payload: this value gets
              // persisted into share_cards.data.characterImage and is what
              // /api/share/[id]/og fetches with plain `fetch()`, which has
              // no next/image host restriction at all. Using
              // resolveImageSrc() here meant any character photo hosted
              // off the allowlist got permanently baked in as the generic
              // silhouette placeholder for every future view of the card.
              // The raw URL is used when present; resolveImageSrc() is only
              // the fallback for a genuinely missing image, matching the
              // OG route's own "no photo -> emoji layout" behavior.
              characterImage: character.image_url || resolveImageSrc(character.image_url),
              ...
              >
                ...
                <ShareCardButton
                  ...
                  request={{
                    ...
                    characterId: character.id,
                    characterName: character.name,
                    // See SHARE-CARD-PLACEHOLDER FIX comment above (same
                    // reasoning applies to milestone cards).
                    characterImage: character.image_url || resolveImageSrc(character.image_url),
