 ...
 * For You/Following filtering, per-post like + inline comments.
 *
 * FOR-YOU/FOLLOWING SIMPLIFICATION (2026-09-07): the New and All tabs are
 * gone (see types/feed.ts's note — both ran the identical query), so the
 * default server-rendered page now loads the "trending" (labeled "For
 * You" client-side) filter instead of "new".
 *
 ...
 *
 * SPONSORED-OFF (product decision): sponsored/inline ads are currently
 * disabled in the feed — FeedGrid ignores its `ads` prop for now (see that
 * file), so getInlineAds() is no longer called here. getInlineAds() itself,
 * FeedInlineAd, and the admin ad form's 'inline' position are untouched —
 * re-enabling later is just wiring this fetch back in and passing the
 * result to FeedGrid.
 */
export default async function FeedPage() {
  ...
  try {
    ...
    if (user) {
      page = await getFeedPostsPage(user.id, { filter: "trending" });
    }
  } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-[520px] py-4">
      ...
      <FeedGrid initialPosts={page.posts} initialNextCursor={page.nextCursor} />
    </div>
  );
}
