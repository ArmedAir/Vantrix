...
export default async function AppLayout({
  ...
  if (!session) {
    ...
    // PRICING-PUBLIC FIX: /premium is now a public pricing page — same
    // acquisition-funnel reasoning as "/" in the 0.3.1 FIX above (a
    // visitor should be able to see what Premium costs before being
    // forced through /login), so it's exempted from the redirect the
    // same way. It renders with currentTier="free" below exactly like
    // "/" already does; premium/page.tsx itself already treats "no
    // session" the same as "free" (see its own isFreeUser comment) and
    // the checkout/trial CTAs route through /login when actually clicked,
    // so nothing paid is reachable without auth — only the pricing
    // information itself is now public.
    // RAAS-PUBLIC FIX: /relationships is a public explainer page for the
    // Spark/Bond/Soulbound relationship tiers (see (app)/relationships/
    // page.tsx) — same acquisition-funnel reasoning as /premium just
    // above it, exempted from the redirect the same way. It renders with
    // no session fine (pure marketing copy, no account data), and its
    // own CTAs route through /login/​/discover like /premium's do.
    if (pathname !== "/" && pathname !== "/premium" && pathname !== "/relationships") {
  ...
  //
  // DISCORD-RESTORE (reference-screenshot pass): the prior DISCORD-FOLD
  // note above no longer applies — Sidebar's footer grid has a standalone
  // Discord tile again (see sidebar.tsx's own comment), so getDiscordUrl()
  // is back here too, fetched alongside contactEmail the same way
  // home/footer.tsx already does.
  ...
  // SIDEBAR-ADS-WIRING: "No ads" is a promised premium perk (tiers/
  // config.ts's features list) — only fetched for free-tier sessions so
  // a premium user's render never even queries the `ads` table, rather
  // than fetching unconditionally and gating in the component.
  const sidebarAds = session.profile.tier === "free" ? await getSidebarAds(4) : [];
