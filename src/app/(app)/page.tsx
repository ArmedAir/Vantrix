...
import { getActiveDailyChoice } from "@/lib/universe/daily-choice";
...
export default async function HomePage() {
  ...
  return (
    <div className="relative pb-2 md:pb-8">
      {/* COLOR-LEAK FIX: this used to be a fixed, low-opacity radial wash
          drawn from --gold-500 (the live theme accent) sitting behind the
          whole page — a real color tint on top of the "black" background,
          just faint. Backgrounds must render flat black with zero accent
          bleed regardless of theme, so this now resolves to --color-base
          (neutral) instead of the accent step. Left as a no-op div rather
          than deleted so a future neutral vignette can reuse the same
          fixed/pointer-events-none scaffolding. */}
      <div
        className="fixed inset-0 pointer-events-none opacity-[0.04]"
        style={{
          background:
            "radial-gradient(ellipse 80% 50% at 50% -10%, rgb(var(--color-base)), transparent 70%)",
        }}
        aria-hidden
      />
      <Greeting
