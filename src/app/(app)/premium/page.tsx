...
/**
 * PRICING-PUBLIC FIX: now reachable signed-out (see (app)/layout.tsx's
 * own PRICING-PUBLIC FIX comment), so this needs the same crawlable
...
export default async function PremiumPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 md:px-8 py-10">
      {/* Signed-in visitors get nav from AppChrome already; a signed-out
          visitor reaching this page directly (see layout's PRICING-PUBLIC
          FIX) has no chrome at all otherwise — same PublicHeader every
          other public page under app/ uses. */}
      {!session && <PublicHeader />}
      <PaywallViewed surface="premium_page" currentTier={currentTier ?? "free"} />
      {/* MOTION (2026-09-07): the whole page sits above the fold and is
          server-rendered — a scroll-triggered MotionWrapper wouldn't
          reveal anything (nothing to scroll to), so this gets a one-time
          page-load entrance instead. This is the highest-stakes page in
          the app (the only checkout surface) and previously had zero
          motion at all — every other major page had at least a scroll
          reveal. */}
      <CinematicEntrance>
      {isFreeUser && (
        <div className="text-center max-w-lg mx-auto">
          <div className="relative mx-auto h-16 w-16">
            ...
            <div className="relative h-16 w-16 rounded-full border border-gold-500/50 flex items-center justify-center">
              <Crown className="h-6 w-6 text-gold-500" strokeWidth={1.75} />
            </div>
          </div>
          <div className="mt-5 inline-flex items-center gap-1.5 rounded-full border border-gold-500/40 bg-gold-500/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-gold-400">
            Membership
          </div>
          <h1 className="font-display text-4xl leading-tight text-text-primary mt-4 text-balance">
            The full Vantrix experience.
          </h1>
          <p className="text-text-secondary mt-3 text-balance">
            {/* COPY-FIX (2026-09-05): see premium-banner.tsx — every character
                is reachable by every tier now, so "exclusive companions" was
                a false claim. */}
            Unlimited conversations, LoRA character training, and the full
            depth of Vantrix&rsquo;s living world — as a Premium member.
          </p>
          ...
          {trialEligible && (
            <div className="mt-6 max-w-xs mx-auto">
              ...
              <p className="text-text-tertiary text-xs mt-2">
                Card required to begin your trial &middot; cancel anytime
                before day 7 to avoid the $9.99/mo charge.
              </p>
            </div>
          )}
        </div>
