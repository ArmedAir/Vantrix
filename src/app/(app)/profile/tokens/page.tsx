...
import { getTokenLedger } from "@/lib/economy/token-ledger";
import { TokenPackCard } from "@/components/premium/token-pack-card";
import { TokenLedgerList } from "@/components/premium/token-ledger-list";
...
export default async function TokensPage() {
  const [session, ledgerEntries] = await Promise.all([
    getShellSession(),
    getTokenLedger(25),
  ]);
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 md:px-8 py-10">
      ...
      <div className="mt-12">
        <h2 className="font-display text-lg text-text-primary mb-3">History</h2>
        <TokenLedgerList entries={ledgerEntries} />
      </div>
    </div>
  );
}
