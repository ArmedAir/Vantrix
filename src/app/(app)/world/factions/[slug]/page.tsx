...
export default async function FactionDetailPage({
  ...
  // ALREADY-DONE FIX: see the location page's identical comment —
  // getAuthedUser() is React-cache()'d, no extra real auth round trip.
  const { user } = await getAuthedUser();
  const [faction, scenarios] = await Promise.all([
    getWorldFaction(slug),
    listScenariosForFaction(slug, user?.id ?? null),
  ]);
  if (!faction) notFound();
