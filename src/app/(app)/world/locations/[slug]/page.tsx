...
export default async function LocationDetailPage({
  ...
  // ALREADY-DONE FIX: getAuthedUser() is React-cache()'d (see get-authed-user.ts)
  // so this doesn't add a second real auth round trip — (app)/layout.tsx
  // already resolved the session for this request.
  const { user } = await getAuthedUser();
  const [location, scenarios] = await Promise.all([
    getWorldLocation(slug),
    listScenariosForLocation(slug, user?.id ?? null),
  ]);
