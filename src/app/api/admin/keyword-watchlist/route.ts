...
const postSchema = z.object({
  ...
  // 'hold_for_review' only takes effect on content-CREATION surfaces
  // (moderateCharacter() call sites) — see keyword_watchlist.action's
  // migration comment. Never applied to the live chat reply path.
  action:  z.enum(['log', 'hold_for_review']).optional().default('log'),
});
...
const patchSchema = z.object({
  ...
  notes:   z.string().max(2000).optional(),
  action:  z.enum(['log', 'hold_for_review']).optional(),
});
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (rest.action  !== undefined) update.action   = rest.action;
