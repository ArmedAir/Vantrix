 ...
 *
 * RLS FIX (2026-09-12): referral_partners' only policies are "read own"
 * (auth.uid() = user_id) and an insert-own-free-tier-only policy — there
 * is no policy letting anyone read or update a row that isn't their own,
 * admins included. That meant this route's own SELECT below (fetching the
 * *applicant's* row, virtually never the admin's own) returned no rows for
 * every real approval attempt, and even past that, the UPDATE would have
 * hit the same missing-policy wall applications themselves were hitting.
 * Admin authorization is already fully enforced above via requireAdmin()
 * and requirePermission() before either query runs, so both now go
 * through supabaseAdmin (service role) rather than the session-bound
 * client — same reasoning as the identical fix in
...
export async function POST(req: NextRequest) {
  ...
  const { data: partner } = await supabaseAdmin.from('referral_partners').select('id,status').eq('id', partnerId).single();
  ...
  const { error } = await supabaseAdmin.from('referral_partners').update(update).eq('id', partnerId);
