...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (bulk.success) {
      const { ids, status } = bulk.data;
      const result = await bulkUpdateReviewStatus('user_reports', ids, {
        status,
        reviewed_by: user.id,
      ...
      await recordAdminAction({
        ...
        targetId:   'user_reports',
        targetLabel: `Bulk ${status} — ${result.updated} user_reports`,
