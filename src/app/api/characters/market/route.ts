 ...
 * visitor to the Market browsing view re-ran getMarketValueLeaderboard()
 * (a real DB read) even though the underlying values only change once
 * per market_value_tick run (a scheduled job, not per-request — see this
 * route's own docstring), and the response has no auth/per-user
 * variance to protect. Safe as a `public` shared cache for the same
 * reason config/login-portraits is: nothing here is gated by who's
 * asking. 60s balances freshness against load; the tick job doesn't run
 * anywhere near that often, so staleness here is a non-issue in
 * practice.
 */
