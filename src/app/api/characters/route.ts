...
import { enqueueCharacterRefundRetry } from '@/lib/ai/character-refund-dlq';
...
const characterCreateSchema = z.object({
  ...
  // REMIX: set by the Creation Studio when a draft originated from
  // buildRemixSeed() (see @/lib/characters/remix) — persisted purely for
  // lineage/attribution display, never trusted for anything access-
  // related (canRemix() is re-checked server-side at remix-seed time,
  // not here).
  remixed_from_character_id: z.string().uuid().optional(),
});
...
export async function GET(req: NextRequest) {
  try {
    ...
    // VIBE-FILTER-FIX: backs the Characters Browse "Vibe" filter (see
    // characters-browse.tsx) — selecting a mood (playful/romantic/...)
    // sends its MOOD_TAGS list here so the query can match characters
    // carrying any of those tags. Comma-separated, same validation
    ...
    let query = supabase
      ...
      .eq('is_public', true)
      // LIST-DETAIL-PARITY FIX: this was the only character-listing query in
      // the codebase missing this filter (every other one — getCharacterDetail,
      // discover/featured, trending, character-recommender, the SEO surfaces,
      // etc. — all gate on is_live too). Card grids built from this endpoint
      // could show an active+public-but-not-yet-live character (is_live is
      // even selected above, to drive the "LIVE" badge on the card), and
      // clicking through 404'd on /characters/[id], since getCharacterDetail()
      // there does require is_live=true. Filtering here matches the detail
      // page's gate so nothing rendered in a list is a dead link.
      .eq('is_live', true);
    ...
    if (tagsParam) {
      const tagsFilter = tagsParam.split(',').map(t => t.trim().slice(0, 40)).filter(Boolean).slice(0, 10);
      // .overlaps == "any character tag matches any requested tag" — a
      // vibe maps to several tags (see MOOD_TAGS) and a character only
      // needs to carry one of them to count as a match, not all.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // holdContext wires in the admin hold-for-review queue (see
    // src/lib/moderation/keyword-watch.ts's checkKeywordsForHold and
    // moderation_holds' migration comment) — purely additive on top of the
    // existing blocklist + AI checks, which run first and are unaffected.
    const modResult = await moderateCharacter({
      ...
      scenario:    d.scenario,
    }, { surface: 'character', userId: user.id });
    ...
    const sanitized = {
      ...
      like_count:  0,
      total_swipes: 0,
      remixed_from_character_id: d.remixed_from_character_id ?? null,
    };
    ...
    if (error) {
      ...
      await supabaseAdmin.rpc('refund_tokens', {
        ...
        if (refundErr) {
          ...
          // REPAIR-WORKER FIX: the refund itself just failed — without
          // this, that's the end of the line, the user is out
          // CHARACTER_CREATION_COST tokens with no character and no
          // automated way back. Queue it for the recovery cron instead
          // of letting a logged error be the only trace. See
          // character-refund-dlq.ts's own header for the full reasoning.
          void enqueueCharacterRefundRetry(user.id, CHARACTER_CREATION_COST, randomUUID(), 'insert_failed');
        }
      });
      throw error;
    }
    ...
    if (!brainResult.success) {
      ...
      await supabaseAdmin.rpc('refund_tokens', {
        ...
        if (refundErr) {
          ...
          // REPAIR-WORKER FIX: same reasoning as the insert-failure site
          // above — queue the failed refund for the recovery cron rather
          // than letting the logged error be the end of it.
          void enqueueCharacterRefundRetry(user.id, CHARACTER_CREATION_COST, randomUUID(), 'brain_init_failed');
        }
      });
