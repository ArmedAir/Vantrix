...
import { clampAngleToStage }           from '@/lib/ai/media-milestone-engine';
import {
  getOrCreateSeed,
  buildImagePrompt,
...
export async function POST(req: NextRequest) {
  try {
    ...
    // COST-GUARD: dailyImageCap above only counts requests, not dollars —
    // see media-budget.ts's header comment. Same pattern peak-budget.ts
    // already uses for PEAK text generation, applied here for image spend.
    const mediaBudget = await checkMediaBudget('image', user.id, tier);
    if (!mediaBudget.allowed) {
      return NextResponse.json({
        error: 'Monthly image generation budget reached for your plan — try again next month or contact support',
        code: 'MEDIA_BUDGET_EXCEEDED',
        spendUsd: mediaBudget.spendUsd,
        ceilingUsd: mediaBudget.ceilingUsd,
      }, { status: 429 });
    }
    ...
    if (!isAdminProfile(profile) && profile.tokens < TOKEN_COST) {
      await releaseMediaReservation('image', user.id);
      return NextResponse.json({
    ...
    // MEDIA-MILESTONE: only the closer/more personal framings (close_up,
    // over_shoulder) are gated by relationship depth — see
    // media-milestone-engine.ts. Relationship is only fetched when an
    // angle was actually requested, so the common case (no explicit
    // angle) pays no extra round trip. A locked angle silently downgrades
    // to 'portrait' rather than failing the whole photo request — the
    // user still gets a photo, just not framed the way that hasn't been
    // earned yet.
    let angleLocked = false;
    if (angle) {
      const relationship = await ensureRelationship(user.id, characterId);
      const clamped = clampAngleToStage(relationship.stage, angle);
      angleLocked = clamped !== angle;
      scene.angle = clamped;
    }
    ...
    try {
      ...
      // COST-GUARD: generation never succeeded, so nothing was actually
      // billed by Fal.ai for this attempt — give the reservation back
      // rather than counting it as spend. Contrast with the video status
      // route, where a 'failed' job still settles as spend because video
      // providers commonly bill for attempted compute even on failure.
      await releaseMediaReservation('image', user.id);
    ...
    // COST-GUARD: generation succeeded — settle the reservation as real
    // spend now, rather than leaving it sitting in "reserved" for the rest
    // of the month (see media-budget.ts's recordMediaUsage contract).
    await recordMediaUsage('image', user.id);
    ...
    return NextResponse.json({
      ...
      angleLocked, // true = requested angle wasn't earned yet, downgraded to 'portrait'
      messageId,
