...
/**
 * ROLE-SAFETY: `messages` rows carry a couple of UI-only role values
 * beyond the provider-accepted 'user'|'assistant'|'system' trio —
 * 'gift' (a compact receipt line, see gift-engine.ts) and now
 * 'secret_moment' (a generated poem/letter, see secret-moments.ts). No
 * provider accepts either as a role. Both history-mapping call sites
 * below previously did a raw `as 'user' | 'assistant'` TYPE cast on
 * `m.role` — which changes nothing at runtime — so a conversation with a
 * gift or secret-moment row anywhere in its trimmed history window would
 * send a literal `role: "gift"` (or now "secret_moment") straight to the
 * provider and fail outright with an invalid-role error on every future
 * turn, forever, for that conversation. This collapses anything that
 * isn't literally 'user' to 'assistant' — correct at face value for
 * secret_moment (the content genuinely is something the character wrote)
 * and a safe conservative default for gift (imperfect context, but never
 * mistakable for user input, and never a hard failure).
 */
...
import { createAchievementPost,
         emojiForStage,
         emojiForExtraMilestone,
         emojiForStreak }          from '@/lib/feed/achievement-posts';
import { checkStreak,
         progressQuest,
...
import { extractAndStoreEdges }     from '@/lib/ai/knowledge-graph';
...
import {
  ...
  // COGNITION-WIRE (session 3): habit-engine.ts / routine-engine.ts /
  // automatic-behavior.ts's storage gap was already closed; the missing
  // piece was cue detection (didn't exist anywhere) and the call-site
  // wiring itself. deriveHabitCue() classifies this turn from signals
  // already computed below (message, emotion, openThreads); the resulting
  // AutomaticDecision is folded into the prompt as S22 and its outcome
  // recorded once the turn resolves — see each call site's comment.
  deriveHabitCue, formatAutomaticDecisionForPrompt, recordAutomaticOutcome,
...
export async function POST(req: NextRequest) {
  ...
  const [{ data: profile }, platformUsageRaw, isDuplicateEarly] = await Promise.all([
    supabase.from('profiles').select('tier,role,is_admin,gender,preferred_language,is_disabled').eq('id', userId).single(),
  ...
  // profiles.is_disabled / disabled_at have existed since the initial
  // schema and the DB-side can_send_message() RPC has always had an
  // 'account_disabled' branch for this — but nothing ever called that RPC
  // (see /admin/users, the first admin surface to actually SET
  // is_disabled), and nothing ever read the column back here either. A
  // disabled account could keep chatting normally. Checked off the
  // profile row already fetched above — zero extra round trips — same
  // placement pattern as the anomaly-suspension gate this mirrors.
  ...
  // ── Rate/cap/slot gates — run sequentially ──────────────────────────────
  // REVERTED (2026-09-05): checkChatLimit, checkDailyMessageCap, and
  // checkPerCharacterMessageCap each atomically increment their own Redis
  // counter as part of "checking". The previous revision ran all three in
  // Promise.all with acquireStreamSlot as an explicit, accepted speed
  // tradeoff — but "accepted" undersold the actual cost: a free-tier user
  // sitting at their daily-cap boundary (30/day) or per-character boundary
  // (5/character) had the *other* counter ticked on every single rejected
  // request, not occasionally — the increments aren't conditional on the
  // sibling check passing, so a user who spam-taps after hitting one cap
  // burns down their other cap for replies that never send. That's the
  // exact quota-vs-rejection race this revision closes: gates now run
  // sequentially, in the same precedence order as before (rateLimit >
  // dailyCap > perCharCap > stream slot), so a request that fails an
  // earlier gate never touches a later counter at all. acquireStreamSlot
  // still only runs once we know the request will actually be allowed
  // through the counters, so it's no longer racing them either — it's
  // acquired last, immediately before the checks that consume it below.
  const rateLimit = await checkChatLimit(userId, tier);
  const dailyCap  = rateLimit.allowed ? await checkDailyMessageCap(userId, tier) : { allowed: false, used: 0, limit: 0, reset: Date.now() };
  const perCharCap = rateLimit.allowed && dailyCap.allowed
    ? await checkPerCharacterMessageCap(userId, characterId, tier)
    : { allowed: false, used: 0, limit: 0, reset: Date.now() };
  const slotAcquired = rateLimit.allowed && dailyCap.allowed && perCharCap.allowed
    ? await acquireStreamSlot(userId, streamScopeId)
    : false;
  ...
  const planPromise = historyPromise.then((h) => {
    ...
    return planResponse({
      ...
      recentMessages:   historyForPlan.map(m => ({ role: toModelRole(m.role), content: m.content })),
  ...
  // COGNITION-WIRE (session 3): the cue automatic-behavior.ts's gate
  // needs, classified from state already computed above for other
  // purposes — no new fetches. See deriveHabitCue()'s own doc comment for
  // why a heuristic classifier is an acceptable first pass here.
  const habitCue = deriveHabitCue({
    message,
    isOpeningMessage: sessionCount === 0,
    emotion: emotionTransitioned,
    openThreads,
  });
  ...
  const automaticDecision = cognitionCycle.automatic;
  ...
  // SELF-AWARENESS FIX (2026-09-14): companionContext.commerce.relationshipTier
  // was already computed above (assembleCompanionContext -> getRelationshipTier)
  // and already used to gate memory depth (see companion-context.ts's
  // retentionPolicy filtering) but was never actually surfaced to the model
  // as text — see formatRelationshipTierForPrompt's own doc comment for why
  // this is a distinct, always-present axis from the narrative `relationship`
  // stage passed below.
  const relationshipTierPrompt = formatRelationshipTierForPrompt(companionContext.commerce.relationshipTier);
  ...
  // S22: Automatic behavior (habit/routine fast-path) — see
  // automatic-behavior.ts's header for the System-1/System-2 framing.
  // `fire: false` (no strong-enough habit/routine matched this cue, or a
  // watch_flag suppressed the gate) is the common case and
  // formatAutomaticDecisionForPrompt() returns '' for it, so
  // addRelationshipCandidate's `if (!content) return` guard drops it —
  // no candidate added, no budget spent. When it does fire, this is
  // meant as a *consistency* cue (a real reason this needs to compete for
  // the same budget as everything else, not bypass it) rather than a
  // verbatim line to repeat, so importance sits close to voice-fingerprint
  // rather than something safety-exempt like S11/rupture.
  addRelationshipCandidate(
    'automatic-behavior',
    formatAutomaticDecisionForPrompt(automaticDecision),
    58, 45,
  );
  ...
  const readable = new ReadableStream({
    async start(controller) {
      ...
      // COGNITION-WIRE (session 3): closes the loop for habit-engine.ts /
      // routine-engine.ts the same way the block above closes it for
      // metacognition.ts — without this, reinforcement never happens from
      // real usage and every habit/routine just sits at whatever strength
      // it was seeded or last cron-decayed to. Same honest caveat as
      // above: `!intentRepeated` is a proxy for "landed," not a ground-
      // truth read of whether the user actually liked the automatic
      // response — there's no deeper signal available at this call site
      // without a real reaction classifier.
      if (automaticDecision.fire) {
        after(() => recordAutomaticOutcome(
          userId, characterId, recentIntents.length, habitCue, automaticDecision, !intentRepeated,
        ).catch(bg('recordAutomaticOutcome')));
      }
      ...
      after(async () => {
        ...
        // own row (secret_moments table) and, best-effort, as a
        // role='secret_moment' message inserted just before the normal
        // reply, so it reads as a discovery rather than prompt flavor
        // folded into the next ordinary turn. role='secret_moment' (not
        // 'assistant') so message-bubble.tsx can give it the envelope/card
        // treatment the doc calls for instead of an ordinary bubble — see
        // 20270101_messages_secret_moment_role.sql and toModelRole() above
        // for why this is safe to feed back into the model as history.
        if (relationship) {
              ...
              try {
                ...
                if (conversationId) {
                  await supabase.from('messages').insert({
                    conversation_id: conversationId, role: 'secret_moment',
        ...
        // MEMORY-TIERS: independent short-term buffer (Redis, 2-6h sliding
        // depending on tier) — fire-and-forget, same posture as
        // updateMemory() above. Promotion to medium/long-term happens later
        // via the consolidation cron, not inline here — see
        // memory-tiers/README.md. `tier` (resolved above via
        // resolveEffectiveTier()) is passed straight through so premium
        // gets the bigger window / salience-protected retention documented
        // in short-term-memory.ts, at no extra cost on this hot path — no
        // new lookup, just forwarding what's already in scope.
        recordTurn(userId, characterId, 'user', message, tier).catch(bg('recordTurn.user'));
        recordTurn(userId, characterId, 'character', fullReply, tier).catch(bg('recordTurn.character'));
        ...
        // Knowledge-graph extraction (deterministic triplets) — run
        // independently of the fact-graph chain above rather than folded
        // into it: it writes its own table (knowledge_graph_edges) and
        // busts its own Redis cache, so there's no shared-state ordering
        // requirement forcing it to wait on extractAndStoreFacts().
        extractAndStoreEdges(userId, characterId, message, fullReply, newCount)
          .catch(bg('extractAndStoreEdges'));
        ...
        // ACHIEVEMENT FEED (2026-09-05): a streak milestone was previously
        // only a toast (recordSurprise, below) and an on-demand external
        // share card (viral-share.ts) — never a post inside Vantrix's own
        // feed. This is the third, native surface: a private post only this
        // user sees, in the character's own voice. Fire-and-forget, same as
        // every other bg() call on this turn — a feed-post failure must
        // never affect the chat reply or the streak bookkeeping itself.
        if (streakResult?.milestone) {
          createAchievementPost(userId, characterId, {
            key:        `streak_${streakResult.milestone.days}`,
            label:      `${streakResult.milestone.days}-Day Streak`,
            emoji:      emojiForStreak(),
            kind:       'streak',
            detail:     streakResult.milestone.message,
            streakDays: streakResult.milestone.days,
          }).catch(bg('createAchievementPost.streak'));
        }
        ...
        if (relationship) {
          ...
          for (const unlock of extraUnlocks) {
            ...
            // Same achievement-feed treatment as the streak case above —
            // see createAchievementPost's header comment for why this
            // exists alongside (not instead of) the toast.
            createAchievementPost(userId, characterId, {
              key:   unlock.key,
              label: unlock.label,
              emoji: emojiForExtraMilestone(unlock.key),
              kind:  unlock.key.includes('streak') ? 'streak' : 'anniversary',
            }).catch(bg('createAchievementPost.unlock'));
          }
          ...
          if (progression?.leveledUp) {
            ...
            const stageLabel = progression.newStage.replace(/_/g, ' ')
              .replace(/\b\w/g, (c) => c.toUpperCase());
            createAchievementPost(userId, characterId, {
              key:   `stage_${progression.newStage}`,
              label: stageLabel,
              emoji: emojiForStage(progression.newStage),
              kind:  'stage',
              bondScore: relationship.bond_score,
            }).catch(bg('createAchievementPost.levelUp'));
          }
        }
