...
import { checkMediaBudget, releaseMediaReservation } from '@/lib/media-budget';
...
export async function POST(req: NextRequest) {
  try {
    ...
    // COST-GUARD: dailyVideoCap above only counts requests, not dollars —
    // see media-budget.ts's header comment for why video specifically
    // needed the same dollar-ceiling treatment peak-budget.ts already
    // applies to PEAK text generation. Admits/reserves atomically; must be
    // paired with releaseMediaReservation() on every early-return path
    // below that doesn't reach submitVideo(), or recordMediaUsage() from
    // the status route once the job actually completes.
    ...
    if (!isAdminProfile(profile) && profile.tokens < TOKEN_COST) {
      await releaseMediaReservation('video', user.id);
      return NextResponse.json({
    ...
    // Reservation is intentionally NOT released or settled here — the job
    // has only been submitted, not completed. It stays reserved until the
    // status route (GET /api/chat/video/status) sees the job finish, then
    // calls recordMediaUsage() to settle it against real spend. If the job
    // never resolves (client stops polling, server restart, etc.) the
    // reservation still expires naturally via TTL_SECONDS in media-budget.ts.
