...
export async function GET(req: NextRequest) {
  try {
    ...
    if (status.status === 'failed') {
      ...
      // COST-GUARD: settle (not release) the reservation made in the submit
      // route — a failed job at Kling's level typically still consumed
      // provider compute, so this errs toward counting it as real spend
      // rather than silently giving budget back for an attempt that cost
      // money. See media-budget.ts.
      await recordMediaUsage('video', user.id);
    ...
    if (!uploaded.success || !uploaded.r2Url) {
      await redis.del(jobKey);
      // Generation itself succeeded and was billed by the provider even
      // though our own R2 upload failed afterward — still settle as spend.
      await recordMediaUsage('video', user.id);
