 ...
 *
 * Also returns the caller's current daily swipe usage (peekSwipeLimit — a
 * read-only status check against the same vantrix:swipe:* counter POST
 * /api/dating/swipe's checkSwipeLimit() actually enforces/increments) so
 * the deck UI can show "X swipes left today" and disable swiping once the
 * cap is hit, instead of letting the user swipe blind into a 429.
 *
 * SWIPE-DISPLAY-FIX: this used to call checkSwipeLimit() directly, which
 * atomically increments the counter on every call. That's correct for the
 * one real swipe action in POST /api/dating/swipe, but wrong here — it
 * meant merely loading or refreshing the deck page burned a swipe with no
 * swipe having happened, silently exhausting the daily cap through normal
 * browsing alone. See peekSwipeLimit()'s own comment (lib/rate-limit) for
 * the full story.
 */
import { NextRequest, NextResponse } from 'next/server';
import { getAuthedUser } from '@/lib/auth/get-authed-user';
import { getRecommendations } from '@/lib/recommendations/engine';
import { normalizeTier, peekSwipeLimit, resolveEffectiveTier } from '@/lib/rate-limit';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [candidates, swipeStatus] = await Promise.all([
      ...
      peekSwipeLimit(user.id, effectiveTier),
    ]);
