...
export async function POST(req: NextRequest) {
  ...
  // SWIPE-EXHAUSTION-FIX: getSwipedIds() (recommendations/engine.ts) now
  // excludes a swipe from the deck only for a rolling 24h window (not
  // forever — see that function's comment for why), keyed off this row's
  // created_at. Postgres/Supabase upsert only sets the columns present in
  // the payload, so without an explicit created_at here, a re-swipe of a
  // recycled-back character would silently keep its original (now-stale)
  // timestamp — the character would then immediately look "old" again and
  // re-qualify for recycling on the very next deck load instead of the
  // fresh 24h window a brand-new swipe should start. Setting it explicitly
  // on every upsert keeps the recycle timer accurate for repeat swipes.
  const { error: swipeErr } = await supabaseAdmin.from('dating_swipes').upsert(
    { user_id: userId, character_id: characterId, direction, created_at: new Date().toISOString() },
