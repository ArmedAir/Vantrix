 ...
 * DELETE /api/digital-twin — scoped wipe: deletes the trained persona and
 *                            all generated reply history, without touching
 *                            the rest of the account. See deleteTwinData()'s
 *                            doc comment in lib/digital-twin/engine.ts.
 *
 * GET/PATCH are Elite-tier gated (see canUseDigitalTwin() in
 * lib/tiers/config.ts). DELETE is deliberately NOT plan-gated — a user
 * whose plan lapsed should still be able to remove their own trained data,
 * same reasoning as DELETE /api/user/delete not requiring an active plan.
 */
...
import { getDigitalTwinProfile, updateManualProfile, deleteTwinData } from '@/lib/digital-twin/engine';
...
export async function DELETE() {
  try {
    ...
    await deleteTwinData(user.id);
    return NextResponse.json({ ok: true });
  } catch (err) {
    logger.error('digital-twin DELETE error', errorLogFields(err));
    const status = err instanceof Error && 'statusCode' in err ? (err as { statusCode: number }).statusCode : 500;
    return NextResponse.json(toErrorBody(err), { status });
  }
}
