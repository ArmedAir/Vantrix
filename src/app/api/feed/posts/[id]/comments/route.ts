...
import { moderateCommentSync, runAsyncCommentReview } from '@/lib/moderation';
...
export async function GET(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    let query = supabaseAdmin
      ...
      .eq('post_id', postId)
      // Comments publish immediately and are reviewed asynchronously (see
      // moderateCommentSync/runAsyncCommentReview in @/lib/moderation) —
      // 'pending' and 'approved' both stay visible; only a confirmed
      // 'rejected' verdict pulls a comment from the public thread.
...
export async function POST(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // ASYNC-MODERATION FIX (2026-09-09): this used to run the full
    // moderateCharacter() gate — including the blocking AI call — before
    // insert. When that AI call failed on every attempt (provider outage,
    // bad key, etc.) it fails closed, and since this same check gated
    // every comment platform-wide, one AI-layer hiccup looked like
    // commenting itself was broken for every user. See
    // 20260909_async_comment_moderation.sql for the full rationale.
    //
    // Only the instant, dependency-free hard blocklist (minors, sexual
    // violence, hate, real-world-harm instructions, exploitation) still
    // runs here and still hard-rejects before insert — that part is
    // unchanged. The AI nuance layer moves below, after insert, via
    // runAsyncCommentReview() in an after() background task: it can
    // adjust moderation_status once it resolves, but it can never block
    // or fail-closed the publish itself.
    const syncResult = moderateCommentSync(content);
    if (!syncResult.allowed) {
      return NextResponse.json({
        error: syncResult.reason ?? 'Comment rejected by content policy',
    ...
    // Comment is already live at this point (moderation_status defaults to
    // 'pending' at the DB level). The AI nuance review runs after the
    // response has been sent and can only ever adjust that status on this
    // row later — see runAsyncCommentReview()'s doc comment.
    after(() => {
      runAsyncCommentReview({
        commentId: inserted.id,
        postId,
        userId:    user.id,
        content,
      }).catch(bg('runAsyncCommentReview'));
    });
