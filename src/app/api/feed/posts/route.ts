 ...
 *   filter    — "trending" | "following" | "new" (default "trending"; "new"
 *               is chronological/ungated — only meaningful paired with
 *               `character`, see get-posts.ts's "new" branch comment)
 *   character — filter to single character ID
 *
 * FOR-YOU/FOLLOWING SIMPLIFICATION (2026-09-07): "new" and "all" are gone
 * — both ran the identical created_at-DESC query with no distinct
 * behavior (see lib/feed/get-posts.ts), so the feed now offers just the
 * two tabs that actually differ. "trending" is labeled "For You" in the
 * UI and is the default.
 *
 * "following" scopes the feed to characters in the caller's own
 * character_follows rows (see lib/feed/get-posts.ts) — personalized, so
 * unlike "trending" it's never served from the shared cache.
 *
 ...
 *
 * Trending ("For You") sort: posts with > 10 likes, sorted by
 * likes_count DESC.
 *
...
export async function GET(req: NextRequest) {
  try {
    ...
    const filter     = (url.searchParams.get('filter') ?? 'trending') as 'trending' | 'following' | 'new';
