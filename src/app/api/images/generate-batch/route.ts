  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        // BILLING-FIX (P0, external audit 2026-09-06): the refund block used
        // to live in the same `try` as the generation loop, after it. If
        // anything in the loop threw (not just an individual image failing —
        // those are already caught per-image via allSettled — but e.g. a
        // controller.enqueue error, or an unexpected throw in
        // buildBatchPrompt), execution jumped straight to `catch`, which
        // only logs — the refund block was never reached. Every already-
        // deducted-but-undelivered spec in that scenario stayed permanently
        // charged with nothing to show for it: provider cost (for whichever
        // images DID finish generating) + user charge + no refund. The loop
        // and the refund are now two separate try/catch blocks so the
        // refund unconditionally runs regardless of how the loop exited,
        // using specsProcessed to correctly count anything never reached
        // (mid-batch throw, not just the existing soft-deadline path) as
        // failed-and-refundable, same as the soft-deadline branch already did.
        ...
        try {
          ...
          for (let i = 0; i < specs.length; i += PARALLELISM) {
            ...
            if (Date.now() - startedAt > SOFT_DEADLINE_MS) {
              ...
              specsProcessed = specs.length; // all accounted for (either done or marked failed above)
              break;
            }
          ...
          logger.error('studio:stream-error', { error: String(err), specsProcessed, total: specs.length });
          // BILLING-FIX: anything the loop above never got to (didn't reach
          // an enqueue for) must still be counted as failed so it's
          // refunded below — previously a mid-loop throw silently dropped
          // these from the count entirely, undercounting the refund.
          const unaccounted = specs.length - specsProcessed;
          if (unaccounted > 0) {
            failedCount += unaccounted;
            for (let k = specsProcessed; k < specs.length; k++) {
              try {
                controller.enqueue(encoder.encode(
                  JSON.stringify({ specId: specs[k].id, url: '', failed: true }) + '\n',
                ));
              } catch {
                // controller may already be unusable if that's what threw
                // above — refund still proceeds below regardless.
              }
            }
          }
        }
        ...
        // never spent). This now runs unconditionally after the loop
        // above, whether it completed cleanly, hit the soft deadline, or
        // threw partway through — see the BILLING-FIX comment at the top
        // of this function.
        try {
          ...
          logger.error('studio:refund-block-error', { error: String(err), userId: user.id, failedCount });
        } finally {
          controller.close();
        }
      },
