...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  // NEW-SIGNUPS GATE: this route is the one place a brand-new auth.users
  // row first gets a profiles row (see the file header) — no separate
  // signup API route exists. Only block here when there's genuinely no
  // profiles row yet, so the admin "new signups off" toggle can never
  // lock an existing user out of their own account on a routine sign-in
  // (which also calls this route, idempotently, every time).
  ...
  if (!existingProfile) {
    ...
    if (!platformSettings.newSignupsEnabled) {
      return NextResponse.json(
        { error: "New signups are temporarily disabled.", code: "SIGNUPS_DISABLED" },
        { status: 403 },
      );
    }
  }
