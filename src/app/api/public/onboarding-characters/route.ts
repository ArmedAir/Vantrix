 ...
 *
 * ONBOARDING-GENDER-LOCK: the flow's copy is written exclusively for a
 * female companion (see public-character.ts's getOnboardingCharacterPool
 * doc comment), so this shortlist is filtered to gender = 'female'
 * server-side rather than left to the client-side tag scorer.
 */
