 ...
 * influencer applications exist for.
 *
 ...
 * (re)apply from.
 *
 * RLS FIX (2026-09-12, part 2): fixing the 409 above only swapped which
 * write hit the same underlying wall. referral_partners' live RLS has an
 * INSERT policy that only permits `class = 'user' AND status = 'active'`
 * — the free-tier self-provision shape — and NO update policy at all (the
 * migration files in this repo don't even show the insert policy that
 * exists in production, meaning even the schema-as-migrations trail has
 * drifted from reality here). A query against the live database confirms
 * every referral_partners row that has ever existed, before this fix, was
 * class='user'/status='active' — meaning literally zero dev/influencer
 * applications had ever successfully been created OR upgraded-to, via
 * either the old INSERT-only code path or the UPDATE this route now
 * attempts. Both are rejected by RLS before ever reaching this route's own
 * business logic.
 *
 * This route already performs full authorization in application code
 * (session check above, class/status eligibility above) before touching
 * referral_partners, so — matching the pattern already used elsewhere in
 * this codebase for the same reason (see ensureProfile, /r/[code]'s
 * recordClick) — the actual read/write below goes through supabaseAdmin
 * (service role, bypasses RLS) rather than the session-bound client. The
 * alternative (adding a new UPDATE/INSERT RLS policy to reproduce this
 * route's eligibility rules in SQL) would duplicate that logic in two
 * places and risk drifting out of sync with it again.
 */
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  if (vanitySlug) {
    let slugQuery = supabaseAdmin.from('referral_partners').select('id').eq('vanity_slug', vanitySlug);
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const { data: partner, error } = await supabaseAdmin.from('referral_partners').insert({
      user_id: user.id,
