 ...
 *
 * RELIABILITY FIX (2026-09-14): same gap and same fix as the badge route
 * — embedded as an <img> on partner sites, had no try/catch and wasn't
 * on the shared withErrorHandling boundary, so an unhandled throw broke
 * the image silently on every site it's installed on with zero logging.
 */
export const GET = withErrorHandling(async (req: NextRequest, { params }: { params: Promise<{ size: string }> }) => {
