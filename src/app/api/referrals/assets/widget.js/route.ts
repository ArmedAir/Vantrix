 ...
 *
 * RELIABILITY FIX (2026-09-14): same gap as the badge/banner routes —
 * no try/catch, not on the shared withErrorHandling boundary. This one
 * matters less (pure string templating, nothing that should throw) but
 * a `<script src>` tag that got an unhandled HTML error page back would
 * throw a hard syntax error in the partner's page console; wrapping it
 * keeps every embed route consistent and logged the same way.
 */
...
(function () {
 ...
 'padding:12px 16px;background:#3A3A40;' +
