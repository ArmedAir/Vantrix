...
import { deleteAllForUser }          from '@/lib/memory-tiers/long-term-memory';
import { removeUserDirtyPairs }      from '@/lib/memory-tiers/short-term-memory';
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const deleteOps = await Promise.allSettled([
      ...
      // MEMORY-TIERS: permanent tier lives in Postgres (Redis tiers are
      // swept via keyPatterns below). deleteAllForUser() is this module's
      // own purpose-built erasure hook (see long-term-memory.ts's "Erasure
      // (GDPR / user-facing reset)" section) — call it directly rather
      // than duplicating its delete() call inline here.
    ...
    const keyPatterns = [
      ...
      // MEMORY-TIERS: short/medium-term buffers (lib/memory-tiers/) live in
      // Redis under their own prefixes, separate from the legacy
      // `vantrix:memory:*` key above — not covered by it. Dirty-set tokens
      // for this user are swept explicitly below since they're not a
      // per-user-suffixed key pattern scanAndDelete can match.
      `vantrix:tier:stm:${userId}:*`,
      `vantrix:tier:mtm:${userId}:*`,
    ];
    ...
    // MEMORY-TIERS: dirty-pair tokens live in one global set, not a
    // per-user key — not reachable by the pattern sweep above.
    try {
      redisKeysDeleted += await removeUserDirtyPairs(userId);
    } catch (redisErr) {
      logger.warn('gdpr:redis-cleanup-partial', { pattern: 'vantrix:tier:dirty-pairs', error: String(redisErr) });
    }
