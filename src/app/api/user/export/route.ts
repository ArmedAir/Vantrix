...
export async function POST(_req: NextRequest) {
  try {
    ...
    const [
      ...
      // MEMORY-TIERS: the only permanent-storage tier (memory_tier_long_term)
      // lives outside memory_graph/user_facts above — separate table, see
      // long-term-memory.ts's header comment. Short/medium-term are Redis-
      // backed, TTL'd working data (2h / 21d sliding), not the kind of
      // durable record data-portability exports are meant to cover.
      supabaseAdmin.from('memory_tier_long_term').select('*').eq('user_id', userId).limit(500),
    ]);
    ...
    const exportData = {
      ...
      longTermMemories: longTermMemoryResult.status === 'fulfilled' ? (longTermMemoryResult.value.data ?? []) : [],
    };
