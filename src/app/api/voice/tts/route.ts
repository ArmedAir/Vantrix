...
import { canGenerateVoiceNote } from '@/lib/ai/media-milestone-engine';
...
import {
  ...
  synthesizeWithVoiceStudio,
  VoiceStudioError,
} from '@/lib/voice/voicestudio-tts';
import { resolveVoiceStudioVoiceId } from '@/lib/voice/voicestudio-voice-library';
import { inferRoleFromVoiceProfile } from '@/lib/ai/character-role';
...
// VoiceStudio (self-hosted, free) — second-tier fallback, tried only when
// ElevenLabs is unconfigured/circuit-open/failed. Self-hosted hardware can
// legitimately be slower than a cloud provider under load, so this breaker
// is a bit more forgiving on both counts than the ElevenLabs one above —
// it shouldn't trip on the first sign of a warm-up delay, but a genuinely
// down/misconfigured instance still needs to fail fast into Web Speech
// rather than hang every request behind it.
const VOICESTUDIO_BREAKER_CONFIG = { failureThreshold: 5, timeout: 20_000 } as const;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: voiceRow } = await supabaseAdmin
      .from('characters').select('voice_profile,elevenlabs_voice_id,voicestudio_voice_id,gender,is_nsfw').eq('id', characterId).maybeSingle();
    ...
    // MEDIA-MILESTONE: whole-feature relationship-depth gate (see
    // media-milestone-engine.ts) — same "unlock as you level up" mechanic
    // secret-tier-engine.ts already applies to narrative secrets, now
    // applied to voice messages. Admins bypass this the same way they
    // bypass the token wallet below — staff shouldn't need to grind a
    // relationship to test/use voice.
    if (!isAdminProfile(profile)) {
      const relationship = await ensureRelationship(user.id, characterId);
      if (!canGenerateVoiceNote(relationship.stage)) {
        return NextResponse.json({
          error: "Voice messages unlock once you two know each other a little better.",
          code: 'MEDIA_MILESTONE_LOCKED',
        }, { status: 403 });
      }
    }
    ...
    if (elevenLabsKey && elevenLabsKey !== 'placeholder-elevenlabs-key' && voiceId) {
      ...
      try {
        ...
        // Index the URL in Redis for future cache hits. A write failure here
        // just means future requests re-generate — the audio itself is
        // already safely in R2 and already returned below. Wrapped in try/
        // catch (not just `.catch()`) for the same reason as
        // CircuitBreaker.reset() (circuit-breaker.ts): `redis` throws
        // SYNCHRONOUSLY on access when Redis env vars aren't set, so
        // `redis.set(...)` itself could throw before `.catch()` ever
        // attaches — which, uncaught here, would fall through to the outer
        // catch below and discard this already-successful, already-paid-for
        // generation in favor of the free Web Speech fallback.
        try {
        ...
        // Fall through to VoiceStudio, then Web Speech params
      }
    }

    // ── Fallback path: self-hosted VoiceStudio (OpenAI-compatible) ──────────
    // Reached when ElevenLabs is unconfigured, its circuit is open, or the
    // live call above failed. Entirely optional — skipped outright if the
    // operator hasn't set VOICESTUDIO_BASE_URL, in which case behavior is
    // unchanged from before this tier existed (straight to Web Speech).
    // Same cache-first / circuit-breaker / R2-upload shape as the ElevenLabs
    // path above, so a VoiceStudio-served line is just as cacheable and just
    // as URL-backed for the client as an ElevenLabs one.
    if (isVoiceStudioConfigured()) {
      // UNIQUE-VOICE: prefer the character's own assigned voice
      // (voicestudio_voice_id, set at creation by digital-person-
      // bootstrap.ts — see voicestudio-voice-library.ts). Characters
      // created before this column existed won't have one yet; resolve
      // and persist it now (lazy backfill) so every subsequent request
      // for this same character reuses the same assigned voice instead
      // of re-rolling the least-used pick on every single message.
      let voicestudioVoice = voiceRow?.voicestudio_voice_id ?? null;
      if (!voicestudioVoice) {
        // ROLE-CONSCIOUS VOICE FIX (2026-09-14): this route only ever
        // selects the character's already-resolved voice_profile (see
        // `characterVoice` above), not the raw personality/backstory text
        // selectPreset() originally matched to produce it — re-fetching
        // that text just for this lazy-backfill path would be a wasted
        // round trip on every pre-existing character's first voice
        // message. inferRoleFromVoiceProfile() reconstructs the same
        // role signal from the numeric profile already in hand instead;
        // see character-role.ts's header for why that's a
        // reconstruction, not an independent second guess.
        const inferredRole = inferRoleFromVoiceProfile(characterVoice);
        voicestudioVoice = await resolveVoiceStudioVoiceId(supabaseAdmin, characterId, voiceRow?.gender, inferredRole);
        try {
          await supabaseAdmin
            .from('characters')
            .update({ voicestudio_voice_id: voicestudioVoice })
            .eq('id', characterId);
        } catch (persistErr) {
          // Non-fatal — worst case this character re-resolves (and may
          // land on a different least-used pick) on its next message.
          logger.warn('voice:voicestudio-voice-id-persist-failed', {
            characterId, error: String(persistErr),
          });
        }
      }
      const cacheKey = cacheKeyFor(cleanedText, `voicestudio:${voicestudioVoice}`, stability, style);

      try {
        const cachedUrl = await redis.get<string>(cacheKey);
        if (cachedUrl) {
          logger.info('voice:voicestudio-cache-hit', { userId: user.id, characterId, tone: tone.emotion });
          return NextResponse.json({
            mode:      'audio',
            audioUrl:  cachedUrl,
            mimeType:  'audio/mpeg',
        ...
        logger.warn('voice:voicestudio-cache-read-failed', { error: String(cacheErr) });
      }

      try {
        const breaker = getCircuitBreaker(`voice:voicestudio:${voicestudioVoice}`, VOICESTUDIO_BREAKER_CONFIG);

        const { buffer: audioBuffer, mimeType } = await breaker.execute(() =>
          synthesizeWithVoiceStudio({
            text: cleanedText,
            voice: voicestudioVoice,
            responseFormat: 'mp3',
            timeoutMs: VOICESTUDIO_BREAKER_CONFIG.timeout,
          })
        );

        const r2Key = `voice-cache/${cacheKey.replace('voice:cache:', '')}.mp3`;
        const upload = await uploadBufferToR2(audioBuffer, r2Key, mimeType);
        ...
        try {
          await redis.set(cacheKey, upload.r2Url, { ex: CACHE_TTL_SECONDS }).catch(err =>
            logger.warn('voice:voicestudio-cache-write-failed', { error: String(err) })
          );
        } catch (err) {
          logger.warn('voice:voicestudio-cache-write-failed', { error: String(err) });
        }

        logger.info('voice:voicestudio-generated', {
          userId: user.id, characterId, tone: tone.emotion,
        });
      ...
      } catch (voicestudioErr) {
        if (voicestudioErr instanceof CircuitOpenError) {
          logger.warn('voice:voicestudio-circuit-open-fallback', { details: voicestudioErr.details });
        } else if (voicestudioErr instanceof VoiceStudioError) {
          logger.warn('voice:voicestudio-failed-fallback', { error: voicestudioErr.message, status: voicestudioErr.status });
        } else {
          logger.warn('voice:voicestudio-failed-fallback', { error: String(voicestudioErr) });
        }
