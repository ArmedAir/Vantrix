 ...
 *
 * GENDER-WIRE-FIX (audit follow-up): signup now also collects gender,
 * carried as `pending_gender` in user_metadata exactly like `pending_dob`
 * (see the signUp() call below). lib/profile/ensure-profile.ts's
 * pendingGender() already read this key and always resolved to null —
 * this closes that gap. profiles.gender then flows into every character's
 * system prompt as `userGender` (see chat/stream/route.ts and
 * lib/ai/prompt.ts's describeUserGender()), which was already fully wired
 * and just had no real data reaching it from signup.
 */
/**
...
export function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data, error: authError } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // GENDER-WIRE-FIX: carried the same way pending_dob already is —
        // no profiles row exists yet at signup time, so this has nowhere
        // else to live until ensureProfile() (lib/profile/ensure-profile.ts)
        // creates that row and reads pending_gender back out of this exact
        // metadata field. Unlike DOB, gender needs no separate authed PATCH
        // step (no age-verification-style check applies to it), so setting
        // it here is sufficient for both the immediate-session and
        // confirm-email-required signup paths.
        data: { pending_dob: dateOfBirth, pending_gender: gender },
    ...
    if (!dobRes.ok) {
      ...
      if (body.code === "PENDING_MANUAL_REVIEW") {
        setPendingAgeReview(true);
        return;
      }
    ...
    <div className="w-full max-w-[400px]">
        ...
        {pendingAgeReview ? (
          <div className="text-center">
            <h1 className="font-display text-2xl mb-2 text-balance">Almost there</h1>
            <p className="text-text-secondary text-sm mb-8">
              Your account needs a quick manual review before it can be verified. This usually
              doesn&apos;t take long — we&apos;ll let you know as soon as it&apos;s done.
            </p>
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {mode === "sign-up" && (
                <div>
                  <label htmlFor="signup-gender" className="block text-xs uppercase tracking-wide text-text-secondary mb-1.5">
                    Gender
                  </label>
                  <select
                    id="signup-gender"
                  ...
                  <p className="text-xs text-text-tertiary mt-1.5">
                    Helps your companions understand and address you naturally.
                  </p>
                </div>
              )}
              ...
              <Button
                ...
                variant="primary"
                size="lg"
                disabled={loading || (mode === "sign-up" && !gender)}
                className="w-full"
              >
                {loading
                  ? "Please wait…"
