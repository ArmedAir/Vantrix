...
import { CharacterChatPreview } from "@/components/admin/character-chat-preview";
    ...
    <RevealItem>
      <Card interactive={false} className="p-4 flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* PERSONA-VISIBILITY FIX: backstory/goal/traits were fetched
              nowhere before this pass (see admin-characters.ts) — this is
              the actual content that drives chat behavior, not just
              display copy, so it belongs in the review, not hidden behind
              a separate detail page that doesn't exist. */}
          {(character.backstory || character.archetype || character.current_goal) && (
            <div className="text-xs text-text-tertiary space-y-1 mb-3 border-l-2 border-border-hairline pl-2.5">
              {character.archetype && <p><span className="text-text-secondary">Archetype:</span> {character.archetype}</p>}
              {character.backstory && <p className="line-clamp-3"><span className="text-text-secondary">Backstory:</span> {character.backstory}</p>}
              {character.current_goal && <p><span className="text-text-secondary">Goal:</span> {character.current_goal}</p>}
            </div>
          )}

          <div className="mb-3">
            <CharacterChatPreview characterId={character.id} characterName={character.name} />
          </div>
