...
export function KeywordWatchlistManager() {
  ...
  // 'hold_for_review' only affects content-CREATION surfaces (character
  // bios, posts, comments) — it never touches the live chat reply path.
  // See keyword_watchlist.action's migration comment.
  async function cycleAction(entry: KeywordWatchlistEntry) {
    const next = entry.action === "hold_for_review" ? "log" : "hold_for_review";
    setEntries((prev) =>
      prev.map((e) => (e.id === entry.id ? { ...e, action: next } : e))
    );
    await setKeywordAction(entry.id, next).catch(load);
  }
          ...
          {entries.map((entry) => (
            <RevealItem key={entry.id}>
              <Card interactive={false} className="p-3 flex items-center gap-3">
                ...
                <button
                  onClick={() => cycleAction(entry)}
                  title="Log-only just records a hit. Hold for review blocks auto-publish on content-creation surfaces (bios, posts, comments) and routes it to the Holds queue."
                  className={cn(
                    "text-[10px] uppercase font-bold shrink-0 px-2 py-1 rounded-xs border transition-colors",
                    entry.action === "hold_for_review"
                      ? "text-danger border-danger/40 bg-danger/10"
                      : "text-text-tertiary border-border-hairline hover:text-text-secondary"
                  )}
                >
                  {entry.action === "hold_for_review" ? "Hold for review" : "Log only"}
                </button>
                <button
