...
import { fadeInUp } from "@/components/immersive/motion";
...
import { GridRemixButton } from "@/components/characters/grid-remix-button";
import {
  useCharacterSearch,
  type GenderFilter,
...
import { USER_MOODS, MOOD_LABELS, MOOD_TAGS, type UserMood } from "@/lib/recommendations/moods";
...
// VIBE-FILTER-FIX: characters carry real personality tags (see
// characters.tags) and this app already has a curated tag taxonomy for
// exactly this — MOOD_TAGS in lib/recommendations/moods.ts, previously
// only reachable through the personalized (auth + userId-required)
// /api/recommendations pipeline. Reused here as a lightweight, no-auth-
// required "Vibe" filter on top of the plain name search this page
// already does. "all" is the no-filter sentinel, same pattern as
// GenderFilter's own "all" value above.
type VibeFilter = "all" | UserMood;
const VIBE_OPTIONS: FilterPillOption[] = [
  { value: "all", label: "Any Vibe" },
  ...USER_MOODS.map((m) => ({
    value: m,
    label: MOOD_LABELS[m].label,
  })),
];
...
export function CharactersBrowse({
  ...
  // GRID-ENTRANCE: gates the staggered fade+rise below, mirroring how
  // MotionWrapper elsewhere in the app falls back to a plain unanimated
  // render for reduced-motion users rather than just a faster version.
  ...
  // a non-female initialGender above. `vibe === "all"` added alongside it:
  // `initial` was never fetched with a vibe filter either, so it's just as
  // invalid a fallback the moment a vibe pill is active.
  const isDefaultView = debouncedQ === "" && gender === "female" && vibe === "all" && limit === PAGE_SIZE;
  const { data, isLoading, error } = useCharacterSearch({
    q: debouncedQ,
    gender,
    tags: vibe === "all" ? undefined : MOOD_TAGS[vibe],
    limit,
  });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 md:px-8 py-6">
      ...
      <FilterPillGroup
        ...
        className={mode === "search" ? "mb-3" : "mb-6"}
      />
      ...
      {mode === "search" && (
        <FilterPillGroup
          ...
          value={vibe}
          onChange={(v) => setVibe(v as VibeFilter)}
          className="mb-6"
        />
      )}
      ...
      {mode === "describe" && !hasDescribeQuery ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {results.map((c, i) => (
            // GRID-ENTRANCE (2026-09-07): this grid re-populates on every
            // filter/search change (mood pill, gender pill, typed query)
            // but previously just snapped straight to the new result set —
            // the one part of this page that visibly changes content had
            // zero motion. Staggered fade+rise per card, capped at 12
            // slots of stagger so a 60-card grid doesn't queue up seconds
            // of delay on the later rows.
            <motion.div
              key={c.id}
            ...
            >
              <CompanionCard character={c} className="w-full" />
              {/* Sits outside MediaCard's own wrapping <Link> (see that
                  component's header) rather than inside CompanionCard's
                  children — nesting a second link/button-as-link in
                  there would be invalid HTML. Hidden until hover/focus
                  so the browse grid doesn't look cluttered at rest;
                  scoped to this grid only, not every CompanionCard
                  caller (anon hero, dating suggestions, etc.) — the
                  library-browsing surface is where "build on this one"
                  actually makes sense as a card-level affordance. */}
              <div className="absolute bottom-2 right-2 opacity-0 group-hover/remix:opacity-100 focus-within:opacity-100 transition-opacity">
                <GridRemixButton characterId={c.id} />
              </div>
            </motion.div>
          ))}
        </div>
