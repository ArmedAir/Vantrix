...
interface LocalMessage {
  ...
  // re-narrowing it here. `secret_moment` (see SECRET-MOMENT-BUBBLE FIX in
  // message-bubble.tsx) rides the same passthrough for the same reason.
  role: string;
  content: string;
  imageUrl?: string | null;
  videoUrl?: string | null;
  status?: "sending" | "sent" | "failed";
  // TIMESTAMP FIX: previously dropped entirely — toLocal() below read
  // every other ChatMessage field except created_at, and every local
  // send/stream/media path constructed a LocalMessage with no time at
  // all. A transcript with zero timestamps anywhere is the one thing
  // every standard chat surface (iMessage, WhatsApp, Telegram) always
  // has, so its absence read as "not a real chat app" independent of
  // anything else. DB-backed messages get the real created_at; messages
  // that originate client-side this session (a fresh send, a completed
  // stream, a generated photo/video) get Date.now() at the moment they're
  // added to state, which is the closest available approximation since
  // none of those paths get a server timestamp back.
  createdAt?: string;
}
...
function toLocal(m: ChatMessage): LocalMessage {
  return {
    ...
    role: m.role === "user" || m.role === "gift" || m.role === "secret_moment" ? m.role : "assistant",
    content: m.content,
...
export function ChatWindow({
  ...
  // DELETE/REGENERATE (this revision): ids currently mid-delete, so their
  // bubble can dim + disable its own actions instead of looking identical
  // to every other message while a network round trip is in flight — see
  // message-bubble.tsx's isDeleting prop.
            ...
            setMessages((prev) =>
              prev.length > 0
                ...
                : transcript.map((m) => ({
                    ...
                    // No per-message time survives the guest-transcript
                    // handoff (GuestTranscriptMessage never captured one —
                    // see guest-transcript.ts), so this stamps claim time
                    // for all of them, same "closest available
                    // approximation" rule as every other client-only path
                    // above rather than leaving these bubbles blank.
  ...
  async function handleGenerateImage() {
    ...
    if (result) {
      setMessages((prev) => [
        ...prev,
        {
          id: result.messageId ?? newId("image"),
          role: "assistant",
          content: "",
          imageUrl: result.url,
          status: "sent",
  ...
  async function handleGenerateVideo() {
    ...
    if (result) {
      setMessages((prev) => [
        ...prev,
        {
          id: result.messageId ?? newId("video"),
          role: "assistant",
          content: "",
          videoUrl: result.url,
          status: "sent",
  ...
  function handleSend() {
    ...
    setMessages((prev) => [
      ...prev,
      { id, role: "user", content: text, status: "sending", createdAt: new Date().toISOString() },
    ]);
    setDraft("");
  ...
  // Shared by handleDelete and handleRegenerate — both need to remove a
  // persisted row from the server, not just local state, or it reappears
  // on the next page load / infinite-scroll fetch. Returns whether the
  // delete actually succeeded so callers (regenerate especially) can bail
  // out instead of proceeding as if it had.
    ...
    async (messageId: string): Promise<boolean> => {
      try {
        ...
        if (!res.ok && res.status !== 404) {
          // A 404 means it's already gone server-side (e.g. a double
          // click) — treat that as success rather than surfacing an error
          // for a message that's effectively deleted either way.
          return false;
        }
        return true;
      } catch {
  ...
  const handleDelete = useCallback(
    async (messageId: string) => {
      ...
      const ok = await deleteOnServer(messageId);
      if (ok) {
        setMessages((prev) => prev.filter((m) => m.id !== messageId));
      } else {
        setActionError("Couldn't delete that message — please try again.");
      }
      setDeletingIds((prev) => {
  ...
  // Regenerate = delete the assistant reply plus the user turn that
  // produced it, then resend that same user text through the normal
  // send path — see the DELETE route's own doc comment for why this
  // reuses `deliver`/POST /api/chat/stream wholesale rather than adding a
  // parallel "regenerate" mode to that (already very large) route.
  // Deliberately NOT restricted to the last message in the transcript:
  // regenerating an earlier reply still works, it just lands the fresh
  // exchange at the end of the conversation rather than back in its
  // original spot, same as most chat products.
  const handleRegenerate = useCallback(
    async (messageId: string) => {
      ...
      // Nearest preceding user turn — what actually prompted this reply.
      let precedingUser: LocalMessage | null = null;
      ...
      setActionError(null);
      const idsToRemove = precedingUser ? [target.id, precedingUser.id] : [target.id];
      setDeletingIds((prev) => {
        const next = new Set(prev);
      ...
      const results = await Promise.all(idsToRemove.map((id) => deleteOnServer(id)));
      ...
      if (!allOk) {
        setActionError("Couldn't regenerate that reply — please try again.");
        return;
      }

      setMessages((prev) => prev.filter((m) => !idsToRemove.includes(m.id)));

      // Nothing to regenerate *from* — the reply had no preceding user
      // message (e.g. an opener) — deleting it alone is the most sensible
      ...
      void deliver(newUserId, precedingUser.content);
    },
    [isStreaming, messages, deleteOnServer, deliver]
  );
  ...
  return (
    ...
    // exact route (see its own isImmersiveChatRoute comment). Previously
    // this used a hardcoded h-[calc(var(--vvh)-4rem)] that assumed
    // ChatHeader (h-16) was the *only* chrome above this column — that
    // broke the instant a second sibling (AiDisclaimerBanner, variable
    // height depending on collapsed/expanded state) was added to the page
    // alongside it, since 4rem no longer matched the real remaining space.
    // flex-1 + min-h-0 on this column, inside the parent's flex-col +
    // h-full (see (app)/chat/[id]/page.tsx), lets flexbox size this
    // correctly against however much chrome actually sits above it,
    // however many siblings that chrome has or however their heights
    // change — no magic number to keep in sync.
