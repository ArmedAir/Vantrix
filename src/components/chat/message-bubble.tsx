...
import { Loader2, Volume2, Square, Play, Maximize2, RotateCcw, Sparkles, Gift, Trash2, Mail } from "lucide-react";
...
/**
 * TIMESTAMP FIX: small muted clock time rendered per-bubble — see
 * chat-window.tsx's own TIMESTAMP FIX comment for why `createdAt` exists
 * on LocalMessage at all now. Every mainstream chat surface (iMessage,
 * WhatsApp, Telegram) shows a time on every message, not just a subset,
 * so this renders unconditionally rather than only on the last message
 * in a group — the simplest rule that can't misrepresent when something
 * was actually sent. Falls back to rendering nothing (not "just now" or
 * similar) when a bubble has no timestamp at all (the in-flight streaming
 * preview bubble in chat-window.tsx, which passes no createdAt because
 * the reply isn't done yet).
 */
 ...
 */
/**
 * ACTIONS ROW (this revision): every sent message (either role) can now be
 * deleted, and every assistant message can be regenerated — previously
 * `onRetry` only ever fired for a *failed user send* (see SEND-STATE
 * above), with no way to remove a message or ask for a fresh reply.
 * `onDelete`/`onRegenerate` are both optional so callers that don't wire
 * them (the brief streaming-preview bubble in chat-window.tsx, which has
 * no stable messageId yet) simply render without the action row rather
 * than needing a no-op passed in. Kept as two small icon buttons — a
 * dedicated "regenerate" affordance instead of overloading `onRetry`
 * (which chat-window.tsx's handleRetry resends a *failed* message
 * verbatim; regenerating an already-sent assistant reply is a different
 * operation — see its handleRegenerate) so the two can't be confused by
 * a future caller.
 */
const bubbleVariants = {
  hidden: { opacity: 0, y: 8 },
  visible: { opacity: 1, y: 0, transition: { duration: DURATION.micro, ease: EASE_VANTRIX } },
  exit: { opacity: 0, scale: 0.97, transition: { duration: DURATION.micro, ease: EASE_VANTRIX } },
};
...
function MessageBubbleImpl({
  ...
  if (role === "gift") {
    return (
      ...
      >
        <div className="flex max-w-[85%] items-center gap-1.5 rounded-full border border-gold-500/25 bg-gold-500/5 px-3 py-1 text-xs text-gold-400 shadow-card">
  ...
  // SECRET-MOMENT-BUBBLE FIX: a `secret_moment`-role row (see chat/
  // stream/route.ts's Secret Moments System block) is a generated poem/
  // letter/memory-recap/playlist/appreciation note — a first-class
  // "discovery" artifact, not an ordinary reply. Content is persisted as
  // `${title}\n\n${body}` (see that insert site); split it back apart
  // here for a real header instead of showing the title as a plain first
  // line of body text. Rendered as its own envelope card — distinct from
  // both the ordinary bubble (this isn't conversational back-and-forth)
  // and the gift pill above (this is considered, substantial content,
  // not a compact receipt). No action row: retry/regenerate/delete don't
  // apply to a milestone artifact the character generated once,
  // unprompted — nothing here to retry or ask her to redo.
  if (role === "secret_moment") {
    const separatorIndex = content.indexOf("\n\n");
    const momentTitle = separatorIndex === -1 ? "A secret moment" : content.slice(0, separatorIndex);
    const momentBody = separatorIndex === -1 ? content : content.slice(separatorIndex + 2);
    const momentTimeLabel = formatMessageTime(createdAt);
    return (
      <motion.div
        variants={shouldReduceMotion ? undefined : bubbleVariants}
        initial="hidden"
      ...
      >
        <div className="max-w-[85%] rounded-lg border border-gold-500/30 bg-gold-500/5 px-4 py-3 shadow-card transition-colors duration-150 ease-premium hover:border-gold-500/50">
          <div className="mb-1.5 flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-gold-400">
          ...
          {momentTimeLabel && (
            <p className="mt-1.5 text-left text-[11px] leading-none text-text-tertiary">
              {momentTimeLabel}
            </p>
          )}
        </div>
  ...
  // Actions only make sense once a message is durably persisted and
  // stable — never mid-send, never on a message already being removed,
  // and never without a real id to target (the streaming-preview bubble
  // passes no messageId at all, see chat-window.tsx).
  const canShowActions = Boolean(messageId) && !isSending && !isDeleting;
  const canDelete = canShowActions && Boolean(onDelete);
  const canRegenerate = canShowActions && isAssistant && !isFailed && Boolean(onRegenerate);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {timeLabel && (
            <p
              className={cn(
                "mt-1 text-[11px] leading-none text-text-tertiary",
      ...
      {isFailed && (
        <div className="mt-1 flex items-center gap-3 pr-1">
          <button
            ...
            className="flex items-center gap-1 text-xs text-danger transition-opacity ease-premium hover:opacity-80"
          >
          ...
          {canDelete && (
            <button
              ...
              className="text-text-tertiary transition-colors ease-premium hover:text-danger"
            >
      ...
      {/* ACTIONS ROW: low-opacity by default (readable on both hover-capable
          and touch devices, unlike a hover-only reveal) and full-opacity on
          hover/focus — see the doc comment above bubbleVariants for why
          delete and regenerate are separate affordances from onRetry. */}
      {!isFailed && (canDelete || canRegenerate) && (
        <div
          className={cn(
            "mt-1 flex items-center gap-2.5 px-1 opacity-40 transition-opacity ease-premium",
        ...
        >
          {canRegenerate && (
            <button
              ...
              className="text-text-tertiary transition-colors ease-premium hover:text-gold-400"
            >
              <RotateCcw className="h-3 w-3" />
            </button>
          )}
