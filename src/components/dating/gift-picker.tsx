...
const ERROR_MESSAGES: Record<string, string> = {
  RATE_LIMIT_EXCEEDED: "Too many gifts right now — try again in a minute.",
};
...
export function GiftPicker({
  ...
  return (
    <div>
      {sentGift && (
        <p className="mb-3 flex items-center gap-2 rounded-sm border border-gold-500/30 bg-gradient-to-r from-gold-500/10 to-transparent px-3 py-2 text-sm text-gold-400">
                ...
                {gifts.map((gift) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-2xl drop-shadow-[0_1px_2px_rgba(0,0,0,0.4)]">{gift.emoji}</span>
      ...
      {selected && (
        <div className="mt-5 border-t border-border-hairline pt-4">
          ...
          <div className="mt-3 flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={() => setSelected(null)} disabled={isSending}>
              Cancel
            </Button>
          </div>
        </div>
