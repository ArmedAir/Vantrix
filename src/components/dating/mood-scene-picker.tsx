    ...
    <div>
        ...
        {MOOD_ROOMS.map((room) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex flex-col items-center gap-1 overflow-hidden rounded-md border p-3 text-center transition-all ease-premium",
                locked
                  ? "border-border-hairline opacity-40 hover:opacity-60 cursor-pointer"
                  : isSelected
                  ? "border-gold-500 bg-gradient-to-br from-gold-500/20 via-gold-500/5 to-transparent shadow-[0_0_0_1px_rgba(212,175,55,0.15)]"
                  : "border-border-hairline bg-gradient-to-br from-gold-500/[0.06] via-transparent to-transparent hover:border-gold-500/40 hover:from-gold-500/[0.12]",
                isGenerating && !locked && "opacity-60 cursor-not-allowed"
              )}
            >
              {locked && (
                <Lock className="absolute right-2 top-2 h-3 w-3 text-text-tertiary" />
              )}
              <span className="text-2xl drop-shadow-[0_1px_2px_rgba(0,0,0,0.4)]">{room.emoji}</span>
      ...
      <div className="mt-4 border-t border-border-hairline pt-4">
        <button
          ...
          disabled={isGenerating}
          className="mb-2 text-xs font-medium text-gold-400 hover:text-gold-300 disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        {useCustom && (
          <textarea
            ...
            rows={2}
            disabled={isGenerating}
            className="w-full resize-none rounded-sm border border-interactive bg-base px-3 py-2 text-sm text-text-primary placeholder:text-text-tertiary focus:border-gold-500/60 focus:outline-none disabled:opacity-50"
          />
        )}
      </div>
