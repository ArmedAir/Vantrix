 ...
 *
 * HIDE-OVERLAY-PARITY-FIX: this card always rendered its own darkening
 * gradient + title bar on top of the creative, unlike HeroAdsCarousel
 * which already skips that for `hide_overlay` rows (fully-designed
 * creatives with headline/CTA baked into the image itself — see that
 * component's own comment and 20261220_seed_baked_hero_ad_creatives.sql).
 * `hide_overlay` has been a column on every `ads` row and part of the
 * `HeroAd` type this whole time; the inline card was just never updated
 * to read it, so a baked creative placed in the 'inline' position would
 * get a second, redundant title stamped over its own design. Brought
 * into parity with the hero carousel's handling.
 */
export function FeedInlineAd({ ad }: { ad: HeroAd }) {
 ...
 <ExternalLink className="h-4 w-4 text-text-secondary" aria-hidden />
 </div>
 )
 ) : (
 <>
