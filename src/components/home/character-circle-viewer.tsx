...
export function CharacterCircleViewer({
  ...
  /** RSC-BOUNDARY FIX (2026-09-09): this component is "use client", so any
   * prop crossing into it from a Server Component parent (LandingPage,
   * CharacterFeatures) must be serializable — a `(id: string) => string`
   * function is not, and passing one here crashed the entire "/" RSC
   * render in production ("Functions cannot be passed directly to Client
   * Components..."). Swapped for this boolean + a same-shape href() helper
   * built locally below, so a signed-out visitor still gets routed through
   * /login?mode=sign-up&redirect=... without ever sending a function over
