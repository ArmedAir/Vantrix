...
export async function Footer() {
  ...
  return (
    <footer className="border-t border-border-hairline mt-4">
      {/* MOBILE-FOOTER-FIX: the full 5-column link grid stays desktop-only
          (every link already lives one tap away in the sidebar drawer —
          see the component's own §3.6 note above), but hiding the whole
          <footer> on mobile left nothing at all after the last content
          section, which read as a broken dead-space gap above the bottom
          tab bar rather than an intentional end-of-page. A condensed
          one-line credit closes that gap without duplicating nav. */}
      <div className="md:hidden px-4 py-4 text-center text-text-tertiary text-xs">
        © {new Date().getFullYear()} Vantrix Ai
      </div>
      <div className="hidden md:grid max-w-7xl mx-auto px-8 py-10 grid-cols-5 gap-8">
        <div>
      ...
      <div className="hidden md:block max-w-7xl mx-auto px-8 pb-8 text-text-tertiary text-xs">
        © {new Date().getFullYear()} Vantrix Ai. All rights reserved. Made in New York, USA.
      </div>
    </footer>
  );
}
