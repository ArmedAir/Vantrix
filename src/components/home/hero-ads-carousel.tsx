...
export function HeroAdsCarousel({ ads }: { ads: HeroAd[] }) {
 ...
 return (
 // FULL-BLEED FIX (2026-09-14): this used to sit in a max-w-7xl/px-8/py-6
 // box on desktop, which is what produced the framed-with-margins look —
 // separate from the object-contain letterboxing above, this was the
 // page itself never letting the slide reach the viewport edges. Dropped
 // to full width at every breakpoint so the hero now behaves like mobile
 // already did (edge-to-edge), just wider.
 ...
 className="flex overflow-x-auto no-scrollbar snap-x snap-mandatory scroll-smooth rounded-none"
 >
 {ads.map((ad, i) => {
 ...
 // DESKTOP-FILL FIX (2026-09-14): object-contain (see IMAGE-FIT FIX
 // above) trades a full-bleed fill for guaranteed-uncropped baked
 // creatives — correct call on a narrow phone screen where the letterbox
 // bars are thin, but on a wide desktop viewport the same choice reads
 // as "the image doesn't fill the screen" rather than "the image is
 // intentionally uncropped". object-cover on md+ (kept object-contain
 // below md) accepts some cropping of the baked CTA/logo in exchange for
 // a true full-bleed desktop hero — the tradeoff that IMAGE-FIT FIX
 // explicitly chose against, now made in the other direction on request.
 className="object-contain md:object-cover"
 />
 );
 const slideClassName = cn(
 "relative shrink-0 w-full aspect-[8/5] sm:aspect-[21/10] md:aspect-[28/9] snap-center overflow-hidden rounded-none",
