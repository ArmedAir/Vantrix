...
// KNOWN-FOR FIX (2026-09-09): the "Known for" line on the Tonight's Match
// card used to render `character.tags[0]` unconditionally. `tags` mixes
// real descriptive traits ("cartographer", "slow-burn") with rarity/tier
// words ("legendary") and, for some rows, truncated sentence fragments
// left over from a seeding bug. Whichever happened to land first
// produced nonsense copy like "Known for: legendary". This skips both
// categories and picks the first tag that reads like an actual trait.
// Hoisted to module scope (rather than an inline closure/Set literal in
// the render body) so the Set is built once per module load, not once
// per render of every Home page visit.
const KNOWN_FOR_TIER_WORDS = new Set([
 "legendary", "rare", "epic", "common", "uncommon",
 "premium", "exclusive", "featured",
]);

function getKnownForTag(tags: string[]): string | undefined {
 return tags.find(
 (t) => !KNOWN_FOR_TIER_WORDS.has(t.toLowerCase()) && t.trim().split(/\s+/).length <= 3
 );
}
...
export function HeroSplit({
 ...
 <Image
 src="/images/characters/ivan-korrath-gallery-1.jpg"
 alt=""
 fill
 sizes="100vw"
 quality={70}
 ...
 {topMatch && (
 // MATCH-CARD FIX (doc feedback, priority #3 -- "the match card is
 // functional but feels underpowered relative to its importance"):
 // was a bare 44px avatar + name + percentage with no button label at
 // all (the whole card was an unlabeled Link). Bigger portrait, a real
 // "known for" line pulled from the character's own tags column (added
 // to the query in home-hero.ts -- not a fabricated "why this match"
 // reason, since this lightweight widget deliberately doesn't run the
 // recommendation engine that would produce one, see that file's own
 // NOT-REUSING-getDatingWorldHome note), and an explicit personalized
 // CTA instead of an implicit whole-card tap target.
 <Link
 href={`/dating/match/${topMatch.matchId}`}
 className="group flex-1 rounded-lg border border-gold-500/25 shadow-card p-5 md:p-6 transition-colors ease-premium hover:border-gold-500/45"
 >
 <p className="flex items-center gap-1.5 text-[11px] font-extrabold tracking-[0.09em] uppercase text-text-tertiary mb-4">
 <Heart className="h-3.5 w-3.5 text-gold-500" strokeWidth={2} />
 Tonight&rsquo;s match
 </p>
 <div className="flex items-center gap-4">
 <div className="relative h-16 w-16 rounded-full overflow-hidden shrink-0 border-2 border-gold-500/30">
 <Image
 ...
 {(() => {
 const knownFor = getKnownForTag(topMatch.character.tags);
 return knownFor && (
 ...
 <div className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-gold-400 transition-colors ease-premium group-hover:text-gold-300">
 Meet {topMatch.character.name}
