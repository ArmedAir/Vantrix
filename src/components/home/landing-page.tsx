...
import type { DailyWorldChoice } from "@/lib/universe/daily-choice";
 ...
 *
 * WORDMARK THEME FIX: was hand-writing its own `<span>Vantrix</span>` in
 * plain `text-text-primary` alongside <BrandMark> instead of passing
 * `withWordmark` — so this page's wordmark stayed static (neutral color,
 * no shimmer) while every other header's wordmark already re-skins with
 * `data-theme` via the gold-400/500/600 gradient (see logo.tsx). Same
 * "one prop, one definition" reasoning logo.tsx's own docstring calls
 * out for exactly this drift.
 */
function Logo() {
 return (
 <Link href="/" className="flex items-center gap-2.5" aria-label="Vantrix home">
 <BrandMark size={32} withWordmark wordmarkClassName="text-xl" />
 </Link>
 );
}
...
{/* SWITCH-CASE SECTION (2026-09-14): the explicit "why leave a single
    companion app" pitch. Written to stand on its own without naming
    any competitor a visitor arriving from a one-avatar companion app
    will recognize the shape of what they have now in the first pillar
    without Vantrix needing to describe it (or risk describing it
    inaccurately). All three claims here are load-bearing, not
    marketing color: "many characters" links straight to /discover,
    "a world that runs without you" surfaces dailyWorldChoice a real,
    live, world-scoped row (see lib/universe/daily-choice.ts), not
    sample copy, so a visitor can click through and verify it's
    actually happening today and "memory built to hold up" is the one
    competitive claim every companion app makes, so it's phrased as
    what Vantrix does (specifics, not vibes) rather than a disparaging
    comparison this page can't verify about anyone else. */}
<MotionWrapper>
 <section className="border-t border-border-hairline px-5 py-24 md:px-8 md:py-32">
 <div className="mx-auto max-w-[1320px]">
 <div className="max-w-2xl">
 <p className="text-xs font-bold uppercase tracking-[0.2em] text-gold-500">Why switch</p>
 <h2 className="mt-4 font-display text-4xl leading-tight tracking-[-0.03em] md:text-6xl">One companion is a relationship. A world is a reason to come back.</h2>
 <p className="mt-5 text-base leading-7 text-text-secondary md:text-lg">Most companion apps give you one avatar, customized to taste, living only inside your conversation. Vantrix is built around something bigger.</p>
 </div>

 <div className="mt-14 grid gap-px overflow-hidden rounded-lg border border-border-hairline bg-white/[0.08] lg:grid-cols-3">
 <article className="bg-base p-7 md:p-9">
 <div className="grid h-11 w-11 place-items-center rounded-md border border-gold-500/20 bg-gold-500/[0.05] text-gold-400"><Users className="h-5 w-5" /></div>
 <h3 className="mt-6 font-display text-2xl">Not one avatar. A cast.</h3>
 <p className="mt-3 text-sm leading-6 text-text-secondary">Dozens of characters with their own histories, personalities, and relationships to each other — or build your own in the Creation Studio. Outgrow one dynamic without starting over on a new app.</p>
 <Link href="/discover" className="mt-5 inline-flex items-center gap-1.5 text-xs font-semibold text-gold-400 hover:text-gold-300">Meet the cast <ChevronRight className="h-3.5 w-3.5" /></Link>
 </article>

 <article className="bg-base p-7 md:p-9">
 <div className="grid h-11 w-11 place-items-center rounded-md border border-gold-500/20 bg-gold-500/[0.05] text-gold-400"><Globe className="h-5 w-5" /></div>
 <h3 className="mt-6 font-display text-2xl">A world that runs without you.</h3>
 {dailyWorldChoice ? (
 <>
 <p className="mt-3 text-sm leading-6 text-text-secondary">It&apos;s not idle between your messages — right now, the world itself is deciding something:</p>
 <p className="mt-3 rounded-md border border-gold-500/20 bg-gold-500/[0.05] px-3.5 py-2.5 text-xs leading-relaxed text-text-primary">
 {dailyWorldChoice.locationName && <span className="font-semibold text-gold-400">{dailyWorldChoice.locationName}: </span>}
 {dailyWorldChoice.prompt}
 </p>
 </>
 ) : (
 <p className="mt-3 text-sm leading-6 text-text-secondary">Characters have goals, jobs, and daily lives that keep moving between your conversations — you come back to what changed, not a blank slate.</p>
 )}
 <Link href="/discover" className="mt-5 inline-flex items-center gap-1.5 text-xs font-semibold text-gold-400 hover:text-gold-300">See what&apos;s happening <ChevronRight className="h-3.5 w-3.5" /></Link>
 </article>

 <article className="bg-base p-7 md:p-9">
 <div className="grid h-11 w-11 place-items-center rounded-md border border-gold-500/20 bg-gold-500/[0.05] text-gold-400"><BrainCircuit className="h-5 w-5" /></div>
 <h3 className="mt-6 font-display text-2xl">Memory built to hold up.</h3>
 <p className="mt-3 text-sm leading-6 text-text-secondary">Specific things you actually said — not a vague sense of familiarity — carried forward into month three, month six, and beyond. Recall is infrastructure here, not a feature checkbox.</p>
 </article>
 </div>
 </div>
 </section>
