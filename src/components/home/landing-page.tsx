...
/**
 * For routes that live under the (app) group (e.g. the "beyond chat"
 * feature tiles below), a signed-out visitor gets redirected straight to
 * /login?redirect=<path> on its own (see (app)/layout.tsx). We still build
 * the /login link explicitly, with mode=sign-up, so an anonymous visitor
 * lands in account creation rather than the sign-in tab.
 * sanitizeRedirect() in login-form.tsx only accepts a same-origin path
 * starting with a single "/", which every entry here is.
 *
 * NOT used for the character/experience cards below — those now link to
 * /companions/<id> (the public, unauthenticated preview + guest-chat page
 * at src/app/(seo)/companions/[id]/page.tsx) so a first click lands a
 * visitor inside a real conversation instead of the signup wall. See that
 * page's own comment for why the guest-chat funnel existed but had no UI
 * consumer until now.
 */
