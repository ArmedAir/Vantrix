...
import { ALWAYS_FREE_SCENARIO_SLUG } from "@/lib/roleplay/scenarios";
...
function iconForScenario(scenario: RoleplayScenario): LucideIcon {
  ...
  if (genre.includes("tension") || genre.includes("jealous"))
    return AlertCircle;
...
/**
 * "Popular Scenarios" — rendered as a horizontal slide carousel
 * (HorizontalScrollRow + MediaCard), matching the same slide pattern
 * already used by "Legendary Scenes" (featured-scenes.tsx) further down
 * this page, rather than a static grid.
 *
        ...
        <HorizontalScrollRow>
          {scenarios.map((scenario) => {
            ...
            const scoped = Boolean(
              scenario.faction_slug || scenario.location_slug,
            );
            const locked = scenario.slug !== ALWAYS_FREE_SCENARIO_SLUG;

            return (
              <MediaCard
                key={scenario.slug}
                href={`/roleplay/new?scenario=${encodeURIComponent(scenario.slug)}`}
                image={resolveImageSrc(imageSrc, SCENARIO_IMAGE_FALLBACK)}
                alt={scenario.title}
                fallback={SCENARIO_IMAGE_FALLBACK}
                imageClassName="aspect-[4/3] sm:aspect-[16/9]"
                className="shrink-0 w-[220px] sm:w-[280px]"
                badge={
                  <span className="h-8 w-8 rounded-full bg-black/50 backdrop-blur-sm border border-gold-500/40 flex items-center justify-center">
