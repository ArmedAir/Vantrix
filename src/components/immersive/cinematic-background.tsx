...
export function CinematicBackground({
 ...
 >
 {/* COLOR-LEAK FIX: the two glow blobs previously painted here in
     rgb(var(--gold-500))/rgb(var(--gold-700)) — i.e. the live theme
     accent hue — directly onto the background at low opacity, so the
     "black" background actually carried a visible violet/rust/pink
     wash depending on data-theme. Backgrounds must be flat black with
     zero accent color leak, so both blobs and the particle color now
     resolve to --color-base (itself neutral, see globals.css) rather
     than an accent step. Left as no-op divs (not deleted outright) so
     `intensity`/className/particle timing keep working unchanged if a
     neutral vignette effect is wanted again later. */}
 <div
 className="absolute -top-1/4 left-1/2 h-[70%] w-[70%] -translate-x-1/2 rounded-full blur-3xl"
 style={{ background: `rgb(var(--color-base) / ${cfg.glow})` }}
 />
 <div
 ...
 <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_35%,rgb(var(--color-base))_100%)]" />
 </div>
 );
}
