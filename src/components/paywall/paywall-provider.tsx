...
export function PaywallProvider({
  ...
  currentTier: string;
  /** See (app)/layout.tsx's PAYWALL-TRIAL-FIX comment for how this is
   *  computed — defaults false so the guest/signed-out mount point
   *  ((app)/layout.tsx's no-session branch) never has to compute it for
   *  a visitor who can't start a trial without an account anyway. */
