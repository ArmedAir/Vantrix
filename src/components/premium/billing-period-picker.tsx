        ...
        <AnimatePresence mode="wait" custom={direction} initial={false}>
          ...
          >
            <div>
              ...
              {/* NAIRA-VISIBILITY-FIX (2026-09-05): the actual amount
                  Paystack charges is tiers.price_ngn for this exact
                  billing-length row (fetched in getPremiumBillingOptions),
                  not a live-converted guess off the USD figure above — so
                  this can be stated as a fact, not an "approx.". Previously
                  nowhere on this card before the Paystack button; a
                  Nigerian user only learned the Naira total after landing
                  on Paystack's own page. */}
              {selected.totalPriceNgn > 0 && (
                <div className="text-[11px] text-text-tertiary mt-0.5">
                  ₦{selected.totalPriceNgn.toLocaleString("en-NG")} via Paystack
                </div>
              )}
            </div>
