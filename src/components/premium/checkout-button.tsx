...
export function CheckoutButton({
  ...
  async function start() {
    ...
    try {
      ...
      // PRICING-PUBLIC FIX: /premium is now reachable signed-out (see
      // (app)/layout.tsx), so a click here can legitimately come from a
      // visitor with no session at all — every checkout route 401s with
      // code: 'UNAUTHORIZED' in that case (see e.g.
      // api/payments/paystack/initialize/route.ts). Previously this was
      // unreachable dead branch (the page itself required auth), so it
      // fell through to the generic error copy below, which would have
      // shown a signed-out visitor a bare "Unauthorized" instead of
      // routing them to actually sign up.
      if (res.status === 401) {
        window.location.href = `/login?mode=sign-up&redirect=${encodeURIComponent("/premium")}`;
        return;
      }
