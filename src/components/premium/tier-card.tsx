...
export function TierCard({
  ...
  return (
    <Card
      interactive={false}
      className={`relative flex flex-col ${large ? "p-6 sm:p-10" : "p-6"} ${highlighted ? "border-gold-500/50 shadow-gold-glow" : ""}`}
    >
      {highlighted && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full border border-gold-500/40 bg-base px-3 py-1 font-display text-[11px] italic tracking-wide text-gold-400">
          Most chosen
        </span>
      )}
      <h3
        className={`font-display text-text-primary tracking-tight ${large ? "text-xl sm:text-3xl" : "text-lg"}`}
      >
        {tier.name}
      </h3>
      ...
      {/* NAIRA-VISIBILITY-FIX (2026-09-05): flat single-price fallback path
          (no billing-length choice seeded in this environment) — tier.price_ngn
          was already selected by getPremiumTiers() but never rendered, so this
          card showed a $ figure with no Naira equivalent at all before the
          Paystack button below. The multi-option picker (billing-period-picker.tsx)
          gets the equivalent line for its own path. */}
      {!hasBillingChoice && tier.price_ngn > 0 && (
        <p className={`text-text-tertiary mt-0.5 ${large ? "text-xs sm:text-sm" : "text-xs"}`}>
          ₦{tier.price_ngn.toLocaleString("en-NG")}/mo via Paystack
        </p>
      )}

      <div className={`border-t border-border-hairline ${large ? "mt-5 sm:mt-7" : "mt-5"}`} />
