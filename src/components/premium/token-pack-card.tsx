...
export function TokenPackCard({ pack }: { pack: TokenPack }) {
  ...
  return (
    <Card glass interactive={false} className="p-5 flex flex-col items-center text-center relative border-gold-500/20">
      {pack.badge && (
      ...
      <div className="relative h-11 w-11">
        ...
        <div className="relative h-11 w-11 rounded-full border border-gold-500/40 flex items-center justify-center">
          <Coins className="h-[18px] w-[18px] text-gold-500" strokeWidth={1.75} />
        </div>
      </div>
      <div className="font-display text-2xl text-text-primary mt-3 tabular-nums">
        {pack.tokens.toLocaleString()}
      </div>
      <div className="text-[11px] uppercase tracking-[0.15em] text-text-tertiary -mt-0.5">Vantrix Coin</div>
      {pack.bonusTokens > 0 && (
      ...
      <div className="mt-4 h-px w-8 bg-gold-500/25" />
