 ...
 *
 * Lives on /profile/edit next to the rest of ProfileEditForm's fields
 * (identity, not app behavior — see PROFILE / SETTINGS SPLIT note on
 * that component), but is its own component rather than folded into
 * that form: upload is a distinct two-step async flow (POST file, then
 * PATCH the URL) with its own loading/error states, and
 * account-menu.tsx / the top bar read avatarUrl from the server-rendered
 * shell session, so a successful upload has to router.refresh() the way
 * ProfileEditForm's save already does — same pattern, separate concern.
 */
...
export function AvatarUpload({
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const saveRes = await fetch("/api/profile/edit", {
        method: "PATCH",
