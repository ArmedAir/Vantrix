 ...
 * enough.
 *
 * ONBOARDING-SEED: `seededFirstMessage`, when provided (from the pre-chat
 * "Choose Your Opening" onboarding step — see components/onboarding/
 * choose-your-opening.tsx), is sent automatically on mount as this
 * widget's first user turn, rather than requiring the visitor to type
 * it themselves. This is not a separate code path from a normal typed
 * message — it reuses handleSend verbatim, just triggered by an effect
 * instead of a click, so it gets identical error handling, rate-limit
 * behavior, and transcript-saving. Guarded by a ref so React StrictMode's
 * double-invoke (or a re-render) can't send it twice.
 */
export function GuestChatWidget({
  ...
  signUpHref: string;
  seededFirstMessage?: string | null;
}) {
  ...
  useEffect(() => {
    if (!seededFirstMessage || seedFiredRef.current) return;
    seedFiredRef.current = true;
    void sendMessage(seededFirstMessage);
    // Intentionally fires once on mount only — messages/sending/limitReached
    // are read fresh inside sendMessage via closure over the render that's
    // current when this effect runs, matching handleSend's existing
    // guard behavior (sending/limitReached checked at call time).
