...
import { SidebarAdRail } from "@/components/shell/sidebar-ad-rail";
import { ThirdPartyAdSlot } from "@/components/shell/third-party-ad-slot";
...
export function AppChrome({
  ...
  // AD-RAIL-GATE: never on an immersive /chat or /roleplay screen
  // (regardless of tier — those routes don't render Sidebar/TopBar at
  // all, so there's nowhere for a rail to go) and only for free-tier
  // sessions — "No ads" is premium's promised perk (tiers/config.ts).
  // layout.tsx already only fetches sidebarAds for free tier, so
  // `profile.tier` is the sole gate needed here for both slots.
  const showAdRail = !immersive && profile.tier === "free";
    ...
    >
      ...
      {immersive ? (
        ...
        <ScrollChromeProvider>
          ...
          {showAdRail && (
            // AD-RAIL: xl-only — the same breakpoint Sidebar itself
            // switches from icon-only to labeled rows at (see that
            // component), so this only ever appears once there's
            // genuinely spare horizontal space next to real content,
            // never squeezed in against a narrow/collapsed left rail.
            <aside className="hidden xl:flex w-72 shrink-0 flex-col gap-4 border-l border-border-hairline px-4 py-6 pt-16">
              <SidebarAdRail ads={sidebarAds} />
              <ThirdPartyAdSlot />
            </aside>
          )}
          <BottomNav />
