 ...
 * panel above.
 *
 * MOBILE-DRAWER-COMPACT-FIX (2026-09-07): with 8 primary nav items plus
 * Premium, the account disclosure's 5 rows, the category row, and the
 * footer's two rows, this drawer's *default* rendered height (account
 * disclosure previously defaulted open) exceeded the visible viewport on
 * common phone sizes — it only "fit" via the internal overflow-y-auto
 * scroll below, contrary to the product goal of a mobile sidebar that
 * always fits without scrolling. Three changes, no destinations removed:
 *   - accountOpen now defaults to `false` (was `true`) — five fewer rows
 *     rendered until the person actually wants them.
 *   - Primary nav switched from NavLink's "compact" size to a new
 *     "tight" size (see ui/nav-link.tsx) — smaller icon, tighter
 *     padding, smaller type — for this drawer's own list only; the
 *     desktop rail (sidebar.tsx) and admin-sidebar.tsx keep "compact"/
 *     "comfortable" untouched.
 *   - Every other zone (header, account row, disclosure rows, category
 *     pills, footer tiles/legal row, Upgrade CTA) had its own padding
 *     and type size trimmed by roughly the same proportion, and the
 *     panel itself narrowed slightly (`w-[72%] max-w-[280px]` ->
 *     `w-[68%] max-w-[260px]`) per the same "smaller for mobile" request.
 *   - overflow-y-auto on the scroll container stays as a safety net for
 *     unusually short viewports (e.g. a landscape phone), not as the
 *     load-bearing mechanism it was before this pass.
 *
...
export function MobileDrawer({
  ...
  // selectors instead of destructuring the whole store, so this drawer
  // only re-renders on drawerOpen changes, not on every railCollapsed
  // toggle from the desktop rail too.
  const drawerOpen = useShellStore((s) => s.drawerOpen);
  const setDrawerOpen = useShellStore((s) => s.setDrawerOpen);
  const unreadCount = useNotificationStore((s) => s.unreadCount);
  // MOBILE-DRAWER-COMPACT-FIX (2026-09-07): was `true` — with 8 primary
  // nav items + Premium, the account disclosure's 5 extra rows pushed
  // total content past what shorter phone viewports can show without an
  // internal scroll (the explicit ask was "always fit ... without
  ...
  return (
    <div className="md:hidden fixed inset-0 z-50">
      <div
        ...
        aria-hidden
      />
      {/* NARROWER-RAIL FIX: 68%/260px -> 62%/240px per direct request --
      ...
      <div className="absolute left-0 top-0 bottom-0 w-[62%] max-w-[240px] bg-base border-r border-border-hairline animate-slide-in-left flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          {/* ACCOUNT ZONE */}
          <div className="p-2 pb-0.5 space-y-0.5">
            <button
              type="button"
              onClick={() => setAccountOpen((v) => !v)}
              aria-expanded={accountOpen}
              className="w-full flex items-center gap-2 px-1.5 py-1 rounded-xs hover:bg-white/[0.04] transition-colors ease-premium"
            >
              {avatarUrl ? (
                <Image
                  ...
                  width={28}
                  height={28}
                  className="h-7 w-7 rounded-full object-cover border border-border-hairline shrink-0"
                />
              ) : (
                <span className="h-7 w-7 shrink-0 rounded-full bg-white/5 border border-border-hairline flex items-center justify-center text-xs font-semibold text-gold-400">
                  {initials}
                </span>
              )}
            ...
            {accountOpen && (
              <div className="pl-1.5 space-y-0 animate-fade-in">
                <Link
                  href="/profile"
                  onClick={() => setDrawerOpen(false)}
                  className={cn(
