 ...
 * selection page while calling itself "Subscription." Split into two real
 * entries: Premium keeps its original href/treatment (just the accurate
 * label now), and Analytics is a separate entry.
 *
 * ANALYTICS-REPOINTED (2026-09-11): Analytics briefly pointed at the
 * billing-management surface (cancel, billing portal) in the pass above.
 * Per direct request, that surface moved to Profile > Settings >
 * Subscription (see profile/settings/page.tsx's nav row and
 * profile/settings/subscription/page.tsx) where account-management
 * settings actually live, and Analytics was freed up for its own real
 * purpose: the Creators Dashboard (marketplace sales + Creator Fund share
 * — see (app)/analytics/page.tsx and creator-earnings-dashboard.tsx),
 * pulled out of the Premium page it used to render inside of. Analytics
 * intentionally has no `premium: true` gold treatment — that treatment
 * exists to make the upsell entry pop, and this isn't the upsell.
 */
