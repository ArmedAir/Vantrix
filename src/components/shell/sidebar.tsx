 ...
 *   - CATEGORY-DROPDOWN ADDITION (reference-screenshot pass): the prior
 *     "no category row on desktop" call is superseded per direct request
 *     — a Category dropdown (Girls/Guys/Anime, mirroring /characters'
 *     own gender param) now sits between the logo header and the nav
 *     list, expanded-rail only. Reads the current gender from the URL
 *     when already on /characters so the dropdown reflects the active
 *     filter instead of always defaulting to Girls.
 *
 ...
 * "Edit Profile" ( /profile/edit) and "Upgrade" ( /premium) from
...
export function Sidebar({
  ...
  // SELECTOR-RERENDER FIX (see top-bar.tsx's own comment): scoped
  // selectors instead of one whole-store destructure, so the desktop
  // rail doesn't re-render every time the unrelated mobile drawerOpen
  // flag flips.
  ...
  return (
    ...
    >
      {/* LOGO-OUT-OF-SIDEBAR FIX: the wordmark/mark header that used to
          open this rail moved into TopBar (see top-bar.tsx) so the
          "Vantrix" brand lives in one place — the top navigation, visible
          on every breakpoint — instead of duplicated here too. No spacer
          left behind either: the rail is `fixed top-0` and TopBar only
          reserves space to the right of it (`md:left-[76px|240px]`), so
          this corner was never actually covered by TopBar — a spacer
          here just left dead space above the nav. Content now starts
          flush at the top of the rail instead. */}
      {tier === "free" && (
        <Link
          href="/premium"
          className={cn(
            "mx-2 mt-3 shrink-0 flex items-center rounded-xs border border-gold-500/30 bg-gold-500/[0.07] transition-colors ease-premium hover:bg-gold-500/[0.12]",
            railCollapsed ? "justify-center px-0 py-2" : "justify-between gap-2 px-3 py-2"
          )}
        >
      ...
      {!railCollapsed && (
        <div className="relative mx-2 mt-3 shrink-0">
          <button
            type="button"
            onClick={() => setCategoryOpen((v) => !v)}
            aria-expanded={categoryOpen}
            className="flex w-full items-center justify-between gap-2 rounded-xs border border-border-hairline px-3 py-2 text-sm font-medium text-text-primary hover:bg-white/[0.04] transition-colors ease-premium"
          >
            <span className="flex items-center gap-2 truncate">
              <Users className="h-4 w-4 shrink-0 text-text-tertiary" strokeWidth={1.75} />
              Category:{" "}
            ...
            <ChevronDown
              className={cn("h-4 w-4 shrink-0 text-text-tertiary transition-transform", categoryOpen && "rotate-180")}
              strokeWidth={1.75}
            />
          </button>
          {categoryOpen && (
            <div className="absolute left-0 right-0 top-full z-40 mt-1 overflow-hidden rounded-xs border border-border-hairline bg-base shadow-lg animate-fade-in">
              {CATEGORY_OPTIONS.map((option) => (
                <Link
                  ...
                  className={cn(
                    ...
                    option.value === activeCategory ? "text-gold-400" : "text-text-secondary"
                  )}
                >
                  {option.label}
                  {option.value === activeCategory && <Check className="h-3.5 w-3.5" strokeWidth={2} />}
                </Link>
              ))}
            </div>
          )}
      ...
      <nav aria-label="Primary" className="flex-1 overflow-y-auto overscroll-contain py-4 px-2 space-y-1">
        {items.map((item, i) => (
          <div
            key={item.href}
      ...
      <div className="p-2 border-t border-border-hairline shrink-0 space-y-1">
        ...
        {!railCollapsed && (
          <div className="pt-2 mt-1 border-t border-border-hairline space-y-1.5 animate-fade-in">
            {/* DISCORD-RESTORE (reference-screenshot pass): the prior
                DISCORD-FOLD merged Discord into /support's own copy since
                a standalone icon felt redundant with only three grid
                slots. The reference layout gives Discord its own labeled
                button alongside Contact Us / Affiliate / Support in a 2x2
                grid, so it's back as a first-class row here — /support
                still separately lists it in its own copy too, which is
                harmless duplication now that this grid is showing labels,
                not just icons. discordUrl re-added to this component's
                props (passed down from (app)/layout.tsx, same as before
                DISCORD-FOLD removed it). */}
            <div className="grid grid-cols-2 gap-1.5">
              <a
                href={discordUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="flex h-9 items-center justify-center gap-1.5 rounded-xs border border-border-hairline text-xs font-medium text-text-secondary hover:text-text-primary hover:bg-white/[0.04] transition-colors ease-premium"
              >
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-1.5 gap-y-0.5 pb-0.5 text-[10px] text-text-tertiary">
              <Link href="/terms" className="hover:text-text-secondary">Legal Terms</Link>
              <span aria-hidden>•</span>
              <Link href="/privacy" className="hover:text-text-secondary">Privacy</Link>
              <span aria-hidden>•</span>
              <Link href="/support" className="hover:text-text-secondary">Help Center</Link>
            </div>
          </div>
        )}
