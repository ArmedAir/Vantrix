 ...
 * previously claimed a bare-icon theme toggle was restored in this header
 * for mobile — it never actually was (nothing theme-related ever rendered
 ...
 * WORDMARK-REMOVED FIX: the "Vantrix" text span that used to sit next to
 * the mark is gone — logo image only now, everywhere this brand mark
 * renders (see PublicHeader's own identical fix, and /enter's own header).
 * Not hidden via CSS on any breakpoint; the JSX node itself no longer
 * exists, so there's nothing to keep in sync if the mark's size/spacing
 * changes later.
 * LOGO-ANIMATION FIX: wordmark is back, this time via Logo's own
 * `withWordmark` prop (see logo.tsx) instead of a hand-written `<span>`
 * — animated (shimmer + glow halo) and theme-reactive through the same
 * CSS-variable mechanism every other themed surface uses, with zero
 * theme-specific code in this file.
 */
