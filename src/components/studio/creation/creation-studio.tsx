...
export function CreationStudio({ userId = null, remixOfCharacterId = null }: { userId?: string | null; remixOfCharacterId?: string | null }) {
  ...
  const [remixSource, setRemixSource] = useState<{ name: string } | null>(null);
  ...
  useEffect(() => {
    // REMIX: an explicit ?remixOf=<id> deep link is a stronger, more
    // deliberate signal than "there happens to be a leftover autosaved
    // draft" — skip the resume-vs-discard prompt entirely and hydrate
    // straight from the remix seed instead. Any old autosaved draft for
    // this userId is left untouched in storage (not cleared) in case the
    // remix click was a mistake and the creator navigates back to
    // /studio/create with no query param.
    if (remixOfCharacterId) {
      fetch(`/api/characters/${remixOfCharacterId}/remix-seed`)
        .then(async (res) => {
          const body = await res.json().catch(() => ({}));
          if (!res.ok) {
            setRemixError(body.error ?? "Couldn't load that character to remix.");
            setHydrated(true);
            return;
          }
          setDraft(body.seed as CharacterDraft);
          setRemixSource({ name: body.sourceCharacterName as string });
          setHydrated(true);
        })
        .catch(() => {
          setRemixError("Couldn't load that character to remix.");
          setHydrated(true);
        });
      return;
    }
    ...
    <div className="min-h-screen bg-base flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-[200px_1fr] lg:grid-cols-[220px_320px_1fr] gap-6 lg:gap-8 px-4 md:px-8 py-6 max-w-7xl mx-auto w-full">
        ...
        <main className="md:order-3 lg:order-3 min-w-0">
          ...
          {remixSource && (
            <div className="mb-4 rounded-md border border-gold-500/30 bg-gold-500/[0.06] px-4 py-3 text-sm text-text-secondary">
              Remixing <span className="text-text-primary font-medium">{remixSource.name}</span> — the writing carried over, but you&apos;ll pick her own look in the Appearance stage and everything else is yours to change.
            </div>
          )}
          {remixError && (
            <div className="mb-4 rounded-md border border-red-500/30 bg-red-500/[0.06] px-4 py-3 text-sm text-text-secondary">
              {remixError} Starting from a blank character instead.
            </div>
          )}
