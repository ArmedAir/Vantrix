 ...
 * desktop pointer's smaller target). "comfortable" = larger touch
 * targets (py-3, text-[15px]). "tight" = MOBILE-DRAWER-COMPACT-FIX
 * (2026-09-07): an even denser row (py-1.5, text-[13px], h-4 icon) so
 * the drawer's full nav list plus its account/category/footer zones can
 * fit a real mobile viewport with no internal scroll — added without
 * touching "compact"/"comfortable", so the rail and admin sidebar (both
 * still "compact") are unaffected. */
 size?: "compact" | "comfortable" | "tight";
 onClick?: () => void;
}
...
export function NavLink({
 ...
 size === "compact" && "gap-3 px-3 py-2.5 text-sm",
 size === "comfortable" && "gap-3 px-3 py-3 text-[15px]",
 size === "tight" && "gap-2.5 px-2.5 py-1.5 text-[13px]",
 ...
 <Icon
 className={cn(
 "shrink-0 transition-transform duration-200 ease-premium group-hover:scale-110",
 size === "tight" ? "h-4 w-4" : "h-5 w-5",
 active && "scale-105"
 )}
