...
/**
 * DOUBLE-TAP-CLOSE FIX: opened from a single-tap Card thumbnail above,
 * same as chat/media-lightbox.tsx's MediaLightbox (this component's own
 * "matches media-lightbox.tsx's overlay pattern" comment already models
 * the rest of this overlay on that one). A fast double-tap on the
 * thumbnail — the same gesture the feed and character-posts grids train
 * users into elsewhere — opens this overlay on the first tap, then the
 * second tap's click lands on the backdrop (same screen position) and
 * fires onClose before the scene is ever seen. CLOSE_GUARD_MS mirrors
 * media-lightbox.tsx's fix exactly: since this component only ever
 * mounts while a scene is open (see SceneGallery's `{openScene && ...}`
 * above), a plain mount-time ref stamp is enough — no need for
 * media-lightbox.tsx's extra "did the prop identity change" check, since
 * a fresh mount already guarantees a fresh stamp.
 */
