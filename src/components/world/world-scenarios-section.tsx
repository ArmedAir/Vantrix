    ...
    <section className="mt-8">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {scenarios.map((scenario) => {
          // LOCK-BADGE FIX: same mismatch as popular-scenarios.tsx — the
          // real gate (POST /api/roleplay/start) exempts only
          // ALWAYS_FREE_SCENARIO_SLUG, not every scenario with
          // min_tier==='free'. Match that instead of the column so a tile
          // shown as unlocked here is actually startable.
          ...
          return (
            <Link key={scenario.id} href={`/roleplay/new?scenario=${encodeURIComponent(scenario.slug)}`}>
              <Card glass className="relative flex h-full flex-col overflow-hidden">
