...
const envSchema = z.object({
  ...
  // Google Search Console domain-verification token (the "content" value
  // from the HTML-tag verification method, not the full <meta> tag).
  // Optional — layout.tsx only renders the verification meta tag when
  // this is set, so leaving it unset is a no-op rather than a broken tag.
  ...
  // (services/brain — e.g. http://brain:8000 in Docker Compose, or a
  // deployed URL on Fly.io/Railway/your own host) or the Vercel-native
  // equivalent under this same app (src/app/api/brain, see local-brain.ts)
  // once that's wired up to a route.
  ...
  // Local-dev-only escape hatch for the brain service's auth requirement —
  // see brain-service-auth.ts's verifyBrainRouteAuth() and
  // services/brain/main.py's identical DEV_MODE flag. Never set this in
  // any deployed environment.
  BRAIN_SERVICE_DEV_MODE: z.enum(['true', 'false']).default('false'),
  ...
  // ── Advertising (third-party network) ────────────────────────────────────
  // Mainstream networks (AdSense, etc.) reject adult/companion-app content
  // outright, so this deliberately isn't wired to a named provider — it's a
  // generic container-div + async-script loader, the integration shape
  // every adult-permitted network (ExoClick, JuicyAds, TrafficJunky, ...)
  // actually uses. Unset by default (AD_NETWORK_ENABLED defaults to
  // 'false'), so the slot renders nothing until a provider is actually
  // signed up and these are filled in. See components/shell/third-party-ad-slot.tsx.
  // All three are read directly via process.env in that Client Component
  // (not through this `env` module) for the same client-bundle reason as
  // NEXT_PUBLIC_R2_PUBLIC_URL just above — listed here for
  // validation/typing/docs only.
  NEXT_PUBLIC_AD_NETWORK_ENABLED:    z.string().optional(),
  NEXT_PUBLIC_AD_NETWORK_SCRIPT_URL: z.string().url().optional(),
  NEXT_PUBLIC_AD_NETWORK_ZONE_ID:    z.string().optional(),
  ...
  // VoiceStudio (self-hosted, free): optional second-tier TTS fallback that
  // runs before the client-only Web Speech fallback below. VoiceStudio
  // (github.com/debpalash/VoiceStudio, formerly OmniVoice-Studio) exposes an
  // OpenAI-compatible /v1/audio/speech endpoint on a self-hosted instance
  // (Docker or desktop app) — see src/lib/voice/voicestudio-tts.ts. Entirely
  // optional: unset means the ElevenLabs → Web Speech path behaves exactly
  // as before this var existed. Point it at a private/loopback instance,
  // not a public one — the endpoint takes no auth of its own beyond
  // VOICESTUDIO_API_KEY (only needed if that instance was started with a
  // share PIN / API key; loopback-only instances need neither).
  VOICESTUDIO_BASE_URL: z.string().url().optional(),
  VOICESTUDIO_API_KEY:  z.string().optional(),
  // Voice profile id known to that VoiceStudio instance (see its Voice
  // Cloning / Model Catalogue). Falls back to 'default' if unset — every
  // VoiceStudio instance ships that profile out of the box.
  VOICESTUDIO_DEFAULT_VOICE: z.string().optional(),
  // Optional pool of the operator's own cloned/designed VoiceStudio voice
  // profiles, so each character can get a distinct VoiceStudio voice
  // instead of every character sharing VOICESTUDIO_DEFAULT_VOICE. Format:
  // comma-separated `id` or `id:gender` entries — see
  // src/lib/voice/voicestudio-voice-library.ts for the full format and
  // assignment strategy. Unset/empty means every character falls back to
  // VOICESTUDIO_DEFAULT_VOICE, unchanged from before this var existed.
  VOICESTUDIO_VOICE_POOL: z.string().optional(),
  ...
  // ── X / Twitter (character cross-posting) ───────────────────────────────────
  // All optional: unset credentials make the feature a clean no-op (see
  // social_posts / crossposted_to_x in the 20261226 migration and the
  // publisher pipeline that reads these) rather than a startup failure.
  // OAuth 1.0a user-context creds — required to actually post/upload media.
  X_API_KEY:             z.string().optional(),
  X_API_SECRET:          z.string().optional(),
  X_ACCESS_TOKEN:        z.string().optional(),
  X_ACCESS_TOKEN_SECRET: z.string().optional(),
  // App-only bearer token — used for read-only calls (e.g. getMe() connection
  // test) where OAuth 1.0a user context isn't required.
  X_BEARER_TOKEN:        z.string().optional(),
  // Handle (no leading @) the platform posts as / links back to in composed
  // tweet copy. Cosmetic only until real credentials are set.
  X_ACCOUNT_USERNAME:    z.string().optional(),
  // Overridable for testing against a mock/proxy; defaults to the real v2/v1.1
  // endpoints used by x-client.ts.
  ...
  // COST-GUARD: PLATFORM_DAILY_VIDEO_BUDGET above is a request COUNT, not a
  // dollar figure — unlike PLATFORM_DAILY_3D_MODEL_BUDGET below, whose own
  // comment already reasons in dollars ($0.48/generation × 100 = $48/day).
  // Video's count-only guard has no equivalent backstop: at the ~$0.75/clip
  // estimate used platform-wide in lib/media-budget.ts (Kling, ~5s clip,
  // ~$0.15/sec), 500 requests/day is a **$375/day** (~$11,250/month) worst
  // case with nothing to catch it if the real per-clip cost turns out
  // higher than assumed, or if a bug/abuse pattern floods legitimate-looking
  // requests. Second independent guard, same reasoning as peak-budget.ts's
  // two-layer (request count + dollar ceiling) approach for PEAK text.
  // Starting number, not final — tune once a real fal.ai/Kling invoice
  // confirms actual per-clip cost.
  PLATFORM_DAILY_VIDEO_DOLLAR_BUDGET: z.coerce.number().positive().default(75),
  ...
  // Bridges emitNotification()/sendPushToUser() to the Capacitor native
  // shells (mobile-capacitor/). One Firebase project covers both platforms:
  // Android registers a native FCM token directly; iOS registers with FCM
  // too, provided GoogleService-Info.plist + an uploaded APNs auth key are
  // configured in the Firebase console (see NATIVE_APP.md). See
  // src/lib/push/send-native-push.ts.
  //
  // FIREBASE_SERVICE_ACCOUNT_JSON is the full JSON key for a service
  // account with the "Firebase Cloud Messaging API Admin" role, either as
  // raw JSON or base64-encoded (send-native-push.ts detects which). Get it
  // from Firebase Console → Project Settings → Service Accounts → Generate
  // new private key. Left unset, native push just no-ops — same pattern as
  // VAPID above and the Resend/PostHog optional integrations elsewhere.
...
// default it to point at THIS SAME deployment's own /api/brain/* routes
// (src/app/api/brain/{health,embed,rerank}/route.ts, backed by
// local-brain.ts's transformers.js port of services/brain/main.py).
// Zero-config beyond also setting BRAIN_SERVICE_API_KEY: without it,
// verifyBrainRouteAuth() fails closed (see brain-service-auth.ts) and the
// self-call 401s, which every caller (semantic-memory.ts,
// memory-embeddings.ts, character-embeddings.ts, character-recommender.ts)
// already treats as "unavailable  fail open" — identical safety net to
// leaving BRAIN_SERVICE_URL unset entirely, just without the reranking
// upside until the key is set.
//
// An explicitly-set BRAIN_SERVICE_URL always wins over this default, so
// anyone still running the original Python sidecar (services/brain — e.g.
// on Fly.io/Railway/a VPS) on a Vercel-hosted frontend keeps working
// completely unchanged.
if (!cleanedEnv.BRAIN_SERVICE_URL && process.env.VERCEL_URL) {
  cleanedEnv.BRAIN_SERVICE_URL = `https://${process.env.VERCEL_URL}/api/brain`;
}
