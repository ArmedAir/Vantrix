...
/** Server-authored `error` strings are written for logs/API consumers, not
 *  always for a chat bubble — RATE_LIMIT_EXCEEDED in particular comes back
 *  as the literal string "Rate limit exceeded", which reads like raw
 *  infrastructure language rather than something Vantrix would say to a
 *  member. Overridden here so every surface using this hook gets the same
 *  friendly copy instead of each one re-mapping it. */
...
function readErrorBody(body: Record<string, unknown> | null, fallback: string): ChatStreamError {
  ...
  const message =
    (code && FRIENDLY_MESSAGE_BY_CODE[code]) ||
