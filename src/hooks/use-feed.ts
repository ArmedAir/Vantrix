...
export function useFeed() {
  ...
  /** Post as your own character — see /api/characters/:id/posts. Throws with
   *  the server's error message (e.g. plan-gate, moderation, rate-limit) so
   *  the caller can surface it directly rather than a generic failure. */
  const createCharacterPost = useCallback(
    async (
      characterId: string,
      input: { caption?: string; image_url?: string; post_type: "photo" | "text" | "teaser" },
    ) => {
  ...
  return { fetchPosts, toggleLike, fetchComments, submitComment, createCharacterPost };
}
