...
export async function resolveNsfwDiscoveryAccess(userId: string | null): Promise<boolean> {
  return (await resolveMatureAccessStatus(userId)).allowed;
}

export interface MatureAccessStatus {
  allowed:      boolean;
  ageVerified:  boolean;
  nsfwEnabled:  boolean;
}
...
/**
 * Granular counterpart to resolveNsfwDiscoveryAccess() above — same two
 * checks, same queries, but tells the caller *which* of the two steps
 * (DB-backed age verification vs. the nsfw_enabled preference) is still
 * missing, instead of collapsing both into one boolean.
 *
 * Added for the mature-content gate screen on /characters/[id], which —
 * pre-dating this function — could only say "you don't have access" with
 * no way to tell a signed-in, partially-qualified visitor which single
 * step stands between them and the character. resolveNsfwDiscoveryAccess()
 * is kept as a thin wrapper around this rather than duplicated, so the
 * two can never drift out of sync; it has other call sites (discovery/
 * recommendations/dating) that only ever needed the boolean and shouldn't
 * have to change their return-type handling for this.
 *
 * Guests (userId === null) short-circuit to "neither step done" without a
 * DB round-trip, same as resolveNsfwDiscoveryAccess().
 */
export async function resolveMatureAccessStatus(userId: string | null): Promise<MatureAccessStatus> {
  if (!userId) return { allowed: false, ageVerified: false, nsfwEnabled: false };

  const { data: isAgeVerified, error: ageVerifiedError } = await supabaseAdmin
    .rpc('is_user_age_verified', { p_user_id: userId });

  const ageVerified = !ageVerifiedError && isAgeVerified === true;
  ...
  return { allowed: ageVerified && nsfwEnabled, ageVerified, nsfwEnabled };
}
