...
/**
 * How long a self-attested age verification stays valid before the
 * account is routed back to 'pending' for re-check. Not a legal
 * requirement, just a periodic-recheck policy — see
 * lib/age-verification/reverification.ts, which runs on this cadence via
 * /api/cron/age-reverification-tick.
 */
export const AGE_VERIFICATION_VALIDITY_MONTHS = 12;
...
export async function submitSelfAttestation(
  ...
  const { error } = await supabaseAdmin
    ...
    .upsert({
      ...
      expires_at:            addMonths(verifiedAt, AGE_VERIFICATION_VALIDITY_MONTHS).toISOString(),
...
/**
 * Flips lapsed 'verified' rows to 'pending' so the status column reflects
 * reality once AGE_VERIFICATION_VALIDITY_MONTHS has passed since
 * verified_at. Called by /api/cron/age-reverification-tick.
 *
 * Not the actual access gate — is_user_age_verified() (SQL) already checks
 * `expires_at IS NULL OR expires_at > NOW()` live on every call, so a user
 * past their expiry is already denied mature-content access the instant it
 * lapses, with or without this tick ever running. This exists purely so
 * the status column stops saying 'verified' for an account that in
 * practice no longer is, and so the user is routed back into the
 * resubmission flow via the same 'pending' path a manual-review
 * resubmission already uses (see the 'rejected' branch above) rather than
 * silently sitting expired with nothing prompting them to redo it.
 *
 * Batched (default 500/run) rather than unbounded, same reasoning as
 * getEligibleNudges()'s `limit` — keeps a single cron invocation bounded
 * regardless of how many verifications lapse on a given day.
 */
export async function reverifyExpiredAgeVerifications(
  limit = 500
): Promise<{ checked: number; expired: number }> {
  const { data: lapsed, error } = await supabaseAdmin
    .from('age_verifications')
    .select('user_id, date_of_birth, verified_at, expires_at')
    .eq('status', 'verified')
  ...
  if (error) {
    logger.error('age_gate.reverifyExpired.select_failed', { error: error.message });
    return { checked: 0, expired: 0 };
  }

  if (!lapsed?.length) return { checked: 0, expired: 0 };
  ...
  for (const row of lapsed) {
    const { error: updateError } = await supabaseAdmin
      ...
      .update({
        status:            'pending',
        rejection_reason:  'Verification expired — resubmission required',
        expires_at:        null,
        updated_at:        new Date().toISOString(),
      })
      .eq('user_id', row.user_id)
      .eq('status', 'verified'); // guard against a race with a fresh resubmission

    if (updateError) {
      logger.error('age_gate.reverifyExpired.update_failed', { userId: row.user_id, error: updateError.message });
      continue;
    }
    ...
    await logAudit(
      row.user_id,
      'expired',
      { status: 'verified', expires_at: row.expires_at },
      { status: 'pending' },
      {}
    );
  }

  return { checked: lapsed.length, expired: expiredCount };
}
