  ...
  try {
    ...
    for (const char of toPost) {
      try {
        ...
        // FOLLOW-LOOP FIX: same gap post-fanout.ts's header describes —
        // until now an autonomous character post never reached anyone who
        // follows that character. A single character's fan-out failing
        // must never fail this tick's post or the characters after it, so
        // this stays inside the per-character try/catch and swallows its
        // own errors besides.
        if (insertedPost?.id) {
          await notifyFollowersOfNewPost({
            ...
            logger.warn('character-feed:post-fanout-failed', { characterId: char.id, error: String(err) });
          });
        }
      } catch (charErr) {
