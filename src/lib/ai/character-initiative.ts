...
import { maybeGenerateSpontaneousPhoto } from '@/lib/ai/spontaneous-photo';
...
export async function runInitiativeCron(): Promise<{
  ...
  try {
    ...
    const { data: characters } = await supabaseAdmin
      .from('characters')
      .select('id,name,current_goal,goal_progress,personality,backstory,gender,age,description,occupation,visual_seed,hair_color,eye_color,body_type,skin_tone,generation_style')
      .in('id', characterIds)
    ...
    // SPONTANEOUS-PHOTO: bulk-fetched once per cron run (not per row) —
    // relationship stage and profile tier are only needed for the small
    // subset of rows whose initiativeType turns out to be photo-eligible,
    // but a per-row query inside the concurrent worker would mean up to
    // CRON_CONCURRENCY simultaneous single-row lookups firing at once for
    // no benefit over one batched query up front.
    const pairKeys = rows.map(r => `${r.user_id}:${r.character_id}`);
    const { data: relRows } = await supabaseAdmin
      .from('character_relationships')
      .select('user_id,character_id,stage')
      .in('character_id', characterIds);
    const stageMap = new Map(
      (relRows ?? [])
        .filter(r => pairKeys.includes(`${r.user_id}:${r.character_id}`))
        .map(r => [`${r.user_id}:${r.character_id}`, r.stage as RelationshipStage]),
    );

    const userIds = Array.from(new Set(rows.map(r => r.user_id)));
    const { data: profiles } = await supabaseAdmin
      .from('profiles')
      .select('id,tier,role,is_admin')
      .in('id', userIds);
    const tierMap = new Map(
      (profiles ?? []).map(p => [p.id, resolveEffectiveTier(p)]),
    );
    ...
    const { deadlineHit } = await runWithConcurrency(
      ...
      try {
        ...
        const stage = stageMap.get(`${row.user_id}:${row.character_id}`) ?? 'stranger';
        const tier  = tierMap.get(row.user_id) ?? 'free';

        // Run the text opener and the (usually-null) spontaneous photo in
        // parallel — the photo path already fails closed to null on
        // anything short of full success, so it never delays or blocks
        // the guaranteed text message.
        const [{ message, llm }, imageUrl] = await Promise.all([
        ...
        await writeInitiativeToConversation(row.user_id, row.character_id, message, imageUrl);
...
export async function getPendingInitiatives(userId: string): Promise<InitiativePayload[]> {
  const { data, error } = await supabaseAdmin
    .from('character_initiatives')
    .select('user_id,character_id,type,message,urgency,expires_at,image_url')
    .eq('user_id', userId)
    .eq('delivered', false)
    .gt('expires_at', new Date().toISOString())
