...
export async function assembleCompanionContext(
  ...
  const [
    ...
    // meaningful". Raised 30 -> 50 (2026-09-14, recall-reliability pass):
    // for a long-running relationship with hundreds of memory_graph rows,
    // this is also the pool retrieveRelevantMemories()'s pgvector path
    // ultimately searches within (see semantic-memory.ts) when the brain
    // service is unavailable and it falls back to reranking *these*
    // candidates rather than a fresh unbounded query — so this pool being
    // too small is a real recall ceiling, not just a display one. Still a
    // single cheap indexed (user_id, character_id) query either way;
    // formatMemoryGraphForPrompt still caps what's shown to the model at 8.
    safe('getMemoryGraph', getMemoryGraph(userId, characterId, 50), []),
