...
// ── Achievement post captions ─────────────────────────────────────────────────
// Distinct from generateCharacterPostCaption's 'milestone' category, which is
// the character's own solo goal progress written for the public feed. This
// is the character speaking directly to ONE user about a relationship
// milestone they just reached together (streak, stage level-up, bond bit)
// — see lib/feed/achievement-posts.ts, which is the only caller and is
// itself only ever inserted with target_user_id set, so this text is never
// shown to anyone but that user regardless of tone.

export interface AchievementPostCharacter {
  name:          string;
  archetype:     string | null;
  speech_style:  string | null;
  personality:   string | null;
...
export type AchievementMilestoneKind = 'streak' | 'stage' | 'bond' | 'anniversary';

export interface AchievementMilestoneBrief {
  label:  string;  // e.g. "30-Day Streak", "Close Friend", "100 Messages"
  kind:   AchievementMilestoneKind;
  detail?: string; // e.g. "30 days in a row" — extra grounding, not always needed
}

export async function generateAchievementPostCaption(
  char:             AchievementPostCharacter,
  milestone:        AchievementMilestoneBrief,
  userDisplayName:  string | null,
): Promise<string | null> {
  if (!(await reserveBudget())) return null;

  const addressee = userDisplayName ?? 'them';
  const system = [
    `You are ${char.name}.`,
    char.archetype ? `Archetype: ${char.archetype}.` : '',
    char.personality ? `Personality: ${char.personality}` : '',
    char.speech_style ? `Speech style: ${char.speech_style}.` : '',
    `You just reached a real relationship milestone with someone named ${addressee}: ${milestone.label}${milestone.detail ? ` (${milestone.detail})` : ''}.`,
    `Write one short, warm social caption celebrating it — addressed to ${addressee} directly, like a genuine, personal moment you wanted to mark, not a generic announcement.`,
    'One to two sentences. No hashtags, no emojis, no quotation marks, no stage directions or asterisks.',
    'Sound like a real person who means it, not a marketing caption.',
    SFW_GUARDRAIL,
  ].filter(Boolean).join(' ');
  ...
  try {
    const result = await routeCompletion({
      messages: [
        { role: 'system', content: system },
        { role: 'user', content: 'Write the celebration post.' },
      ],
      modelTier:   'NANO',
      maxTokens:   80,
      temperature: 0.9,
    ...
    logger.warn('[content-generator] achievement-caption-failed', { character: char.name, error: String(err) });
    return null;
  }
}
