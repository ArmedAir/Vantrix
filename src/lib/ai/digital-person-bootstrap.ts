...
import {
  ...
  VOICE_PRESETS,
  DEFAULT_WRITING_STYLE_PROFILE as DEFAULT_STYLE,
  DEFAULT_VOICE_PROFILE as DEFAULT_VOICE,
  type WritingStyleProfile,
  type VoiceProfile,
} from './writing-style';
import { resolveCharacterRole } from './character-role';
...
async function selectPreset(input: DigitalPersonInput): Promise<{ style: WritingStyleProfile; voice: VoiceProfile; elevenlabsVoiceId: string; voicestudioVoiceId: string }> {
  // ROLE-CONSCIOUS VOICE FIX (2026-09-14): role is now resolved ONCE, up
  // front, from character-role.ts (moved there verbatim from this
  // function's old private `rules` array), and threaded into BOTH the
  // style/voice-profile preset lookup below AND the literal voice-id
  // resolvers. Previously these ran on separate, disconnected inputs —
  // the archetype match happened in a for-loop AFTER both resolveVoiceId
  // calls had already returned — so a "companion" character's actual
  // ElevenLabs/VoiceStudio voice had no way to reflect that role, only
  // their gender. Gender bucketing and least-used-first uniqueness are
  // both still enforced inside resolveVoiceId/resolveVoiceStudioVoiceId
  // — role is layered in as a preference within that, not a replacement
  // for either (see those functions' own doc comments for the cascade).
  const role = resolveCharacterRole({
  ...
  // no shared mutable state — so there's no reason to pay their DB
  // round-trip latency twice, back-to-back. Previously awaited
  // sequentially, this serialized two network calls on the character
  // creation hot path for no benefit. Running them concurrently roughly
  // halves the time selectPreset() (and therefore bootstrap) spends
  // waiting on voice assignment.
  const [elevenlabsVoiceId, voicestudioVoiceId] = await Promise.all([
    resolveVoiceId(supabaseAdmin, input.characterId, input.gender, role),
    // Same idea for the self-hosted VoiceStudio fallback tier — see
    // voicestudio-voice-library.ts. Assigned in the same pass so a
    // character's ElevenLabs voice and VoiceStudio voice are both settled
    // at creation time, not just whichever one happens to be reachable
    // when a voice message is first requested.
    resolveVoiceStudioVoiceId(supabaseAdmin, input.characterId, input.gender, role),
  ]);

  if (role) {
    return { style: WRITING_STYLE_PRESETS[role], voice: VOICE_PRESETS[role], elevenlabsVoiceId, voicestudioVoiceId };
  }
  return { style: DEFAULT_STYLE, voice: DEFAULT_VOICE, elevenlabsVoiceId, voicestudioVoiceId };
}
...
export async function initializeDigitalPerson(
  ...
  const { style, voice, elevenlabsVoiceId, voicestudioVoiceId } = await selectPreset(input);
  ...
  const { error: styleErr } = await supabaseAdmin
    ...
    .update({
      ...
      voicestudio_voice_id: voicestudioVoiceId,
    })
