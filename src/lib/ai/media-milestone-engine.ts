...
// ── Spontaneous (unprompted) photos ─────────────────────────────────────
//
// Same floor as voice notes, reused deliberately rather than introducing a
// third independent threshold — an unprompted selfie is the same "she's
// warmed up enough to share something without being asked" step as an
// unprompted voice note, just a different medium. See
// spontaneous-photo.ts, which is the only caller of this.
export function canSendSpontaneousPhoto(stage: RelationshipStage): boolean {
  return rankOf(stage) >= rankOf(VOICE_UNLOCK_FLOOR);
}
