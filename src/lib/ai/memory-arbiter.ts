 ...
 *   2. knowledge-graph.ts (KnowledgeEdge) — explicit (subject)-[predicate]->
 *      (object) triplets, deterministically versioned (a new edge for the
 *      same predicate supersedes the old one at write time — see
 *      knowledge-graph.ts's upsertEdge). Ranked above user-fact-graph.ts
 *      because there is no ambiguity left to arbitrate by the time a fact
 *      reaches this layer: the graph itself already resolved "which value
 *      is current" at write time. This module still lets it participate in
 *      topic-collision detection against the fuzzier sources below (e.g. a
 *      stale memory.ts "User location: Chicago" losing to a live
 *      LIVES_IN->Austin edge) rather than merely injecting it separately.
 *   3. user-fact-graph.ts (UserFact) — structured, categorized, has its own
 ...
 *   4. memory.ts (MemoryFact) — legacy flat fact store. Still authoritative
 *      for anything user-fact-graph.ts's fixed category set doesn't cover
 *      (e.g. a bare name extraction), but yields to (2)/(3) on overlap.
 *
...
import { getLiveGraph, type KnowledgeEdge } from '@/lib/ai/knowledge-graph';
...
interface NormalizedFact {
  ...
  timestampMs: number;
  precedence: 0 | 1 | 2; // 0 = knowledge-graph, 1 = user-fact-graph, 2 = memory.ts — lower wins ties
  origin: 'knowledge-graph' | 'user-fact-graph' | 'legacy-memory';
}
...
// Maps a knowledge-graph predicate onto the same coarse topic keys the
// fuzzy sources use, so e.g. a live LIVES_IN edge actually collides with
// (and, per precedence, beats) a stale "User location: Chicago" memory.ts
// fact instead of both being injected side by side. Predicates with no
// obvious fuzzy-source counterpart get their own dedicated topic key,
// which just means they never collide with anything — still injected,
// never arbitrated away.
const PREDICATE_TOPIC_KEY: Partial<Record<string, string>> = {
  LIVES_IN: 'location',
  BORN_IN: 'location',
  WORKS_AS: 'occupation',
  WORKS_AT: 'occupation',
...
function topicKeyForEdge(edge: KnowledgeEdge): string {
  return PREDICATE_TOPIC_KEY[edge.predicate] ?? `kg:${edge.predicate}`;
}

function normalizeKnowledgeEdges(edges: KnowledgeEdge[]): NormalizedFact[] {
  // Only 'user' subject edges compete in this arbitration — 'character'
  // subject edges (self-facts) aren't in scope for the fuzzy sources this
  // module reconciles against.
  return edges
    .filter((e) => e.subject === 'user')
    .map((e) => ({
      topicKey: topicKeyForEdge(e),
      text: `${e.predicate.replace(/_/g, ' ').toLowerCase()}: ${e.object}`,
      confidence: e.confidence,
      timestampMs: Date.parse(e.ingestion_time) || 0,
      precedence: 0,
...
export function arbitrateMemoryContext(
  ...
  // DEFENSIVE (bug found 2026-09-02, RESILIENCE-1 follow-up): this function's
  // own docstring above promises "pure arbitration ... no I/O" and
  // companion-context.ts's call site relies on that to claim it "never
  // throws" — but normalizeUserFacts()/normalizeLegacyFacts()/
  // formatSeedBlock() below called .map()/.length unconditionally, trusting
  // each param actually WAS an array at runtime just because its TS type
  // says so. All three params are pass-throughs of getFactGraph()/getMemory()/
  // getCharacterSeedMemories() results, and nothing validates their actual
  // runtime shape before it reaches here — e.g. getFactGraph()'s Redis cache
  // path does `redis.get<UserFact[]>(...)` and returns whatever comes back
  // unchecked, so one corrupted/mis-shaped cache entry becomes an object,
  // not an array, and this threw (confirmed: `facts.map is not a function`,
  // taking down the whole companion turn via companion-context.ts's
  // Promise.all, which had no reason to suspect a *synchronous* call could
  // fail). Coercing to [] up front restores the "never throws" guarantee
  // this module has always claimed rather than merely assumed.
  const safeLegacy     = Array.isArray(legacyFacts)     ? legacyFacts     : [];
  const safeStructured = Array.isArray(structuredFacts) ? structuredFacts : [];
  const safeSeed       = Array.isArray(seedMemories)    ? seedMemories    : [];
  const safeEdges      = Array.isArray(knowledgeEdges)  ? knowledgeEdges  : [];

  if (safeLegacy !== legacyFacts || safeStructured !== structuredFacts || safeSeed !== seedMemories) {
    logger.warn('memory-arbiter: received non-array fact source, coercing to empty', {
      ...(logCtx ?? {}),
      legacyWasArray:     Array.isArray(legacyFacts),
      structuredWasArray: Array.isArray(structuredFacts),
      seedWasArray:       Array.isArray(seedMemories),
    });
  }

  const normalized = [...normalizeKnowledgeEdges(safeEdges), ...normalizeUserFacts(safeStructured), ...normalizeLegacyFacts(safeLegacy)];
  ...
  const seedBlock = formatSeedBlock(safeSeed);
...
export async function getCanonicalMemoryContext(
  ...
  const [legacyFacts, structuredFacts, seedMemories, knowledgeEdges] = await Promise.all([
    ...
    getLiveGraph(userId, characterId).catch((err) => {
      logger.warn('memory-arbiter: getLiveGraph failed', { userId, characterId, error: String(err) });
      return [] as KnowledgeEdge[];
    }),
  ]);

  return arbitrateMemoryContext(legacyFacts, structuredFacts, seedMemories, { userId, characterId }, knowledgeEdges);
}
