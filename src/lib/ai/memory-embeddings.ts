...
export interface SimilarMemory {
  ...
  // Added by 20270120_match_memory_graph_bitemporal_columns.sql — needed to
  // reconstruct a full MemoryNode (see similarMemoryToNode() below) for a
  // similarity hit that falls outside a caller's own candidate list.
  event_time: string;
  ingestion_time: string;
}

/**
 * Adapts a similarity-search hit into the canonical MemoryNode shape.
 * Exists for callers (see semantic-memory.ts's retrieveRelevantMemories)
 * that want to splice a pgvector-only match — one outside whatever
 * candidate pool they already fetched by other means — into a list of full
 * MemoryNode objects, rather than only using match_memory_graph's rows to
 * reorder a pre-existing list. Safe now that the RPC selects every field
 * MemoryNode needs, not just enough to score and rank a match.
 */
export function similarMemoryToNode(s: SimilarMemory): MemoryNode {
  return {
    id:               s.id,
    event_type:       s.event_type as MemoryEventType,
    title:            s.title,
