...
export interface MemoryNode {
  ...
  tags:             string[];
  created_at:       string;
  // Bitemporal (20260909_bitemporal_knowledge_graph.sql): event_time is when
  // this happened "in the world"; ingestion_time is when the AI learned it.
  // They diverge whenever a caller passes an explicit eventTime to
  // addMemory() (e.g. a late disclosure — "I forgot to tell you last week
  // I got the job"). Display/recency logic should use event_time; audit /
  // "when did the AI find out" logic should use ingestion_time.
  event_time:       string;
...
export async function addMemory(
  ...
  event: {
    ...
    tags?:            string[];
    // Bitemporal override: when this actually happened, if different from
    // now (e.g. a late disclosure). Defaults to NOW() in the DB when
    // omitted, same as ingestion_time — the common case where the two
    // coincide.
    eventTime?:       string;
  },
  ...
  try {
    ...
    const { data } = await supabaseAdmin
      ...
      .insert({
        ...
        tags:             event.tags ?? [],
        // Bitemporal: event_time defaults to the DB's NOW() when omitted;
        // ingestion_time always defaults to NOW() regardless (see migration).
        ...(event.eventTime ? { event_time: event.eventTime } : {}),
      })
      .select('*')
      .single();
...
// Default raised 15 -> 30 (2026-09-14, recall-reliability pass) to match
// the pool size companion-context.ts's FEATURE-7 call already proved
// necessary in production (see its own call site comment) — the 15
// default was a footgun for any *other* caller (dating/date/start.ts aside,
// which deliberately wants a tiny pool) that didn't know to override it.
export async function getMemoryGraph(
  ...
  const { data } = await supabaseAdmin
    ...
    // Sort by event_time (when it happened), not ingestion_time/created_at
    // (when the AI found out) — a late-disclosed memory should still slot
    // into the timeline where it actually belongs.
...
/**
 * Bitemporal timeline read: every memory as it stood at a given point in
 * ingestion time — i.e. "what did the AI know as of date X". Distinct from
 * getMemoryGraph(), which always returns the latest full picture ordered by
 * when things happened. Useful for reconstructing what context the AI
 * actually had available during a past conversation, e.g. debugging why an
 * old reply didn't reference a fact the user had in fact already shared
 * (still en route to being learned as of that ingestion_time).
 */
export async function getMemoryGraphAsOf(
  userId:         string,
  characterId:    string,
  asOfIngestion:  string,
  limit = 15,
): Promise<MemoryNode[]> {
  const { data } = await supabaseAdmin
    .from('memory_graph')
    ...
    .eq('user_id', userId)
    .eq('character_id', characterId)
    .lte('ingestion_time', asOfIngestion)
    .order('emotional_weight', { ascending: false })
    .order('event_time', { ascending: false })
    .limit(limit);
...
export function formatMemoryGraphForPrompt(memories: MemoryNode[]): string {
  ...
  // BUG FIX: this used to unconditionally re-sort by emotional_weight then
  // recency, discarding whatever order the caller arrived with. That
  // silently defeated two separate upgrades that both exist specifically
  // to reorder this list before it gets here: companion-context.ts's
  // FEATURE-7 (Invisible Memory) widened the fetched pool to 30 precisely
  // "so semantic relevance could win a spot in the top slice
  // formatMemoryGraphForPrompt() actually shows" — and semantic-memory.ts's
  // retrieveRelevantMemories() does real pgvector-similarity reordering
  // against the current message for exactly that purpose. Neither ever
  // mattered here: this function put weight back in sole control of the
  // cut regardless. By the time a list reaches this function every caller
  ...
  const lines = top.map(m => {
    ...
    // (when the AI learned it) — matters whenever the two diverge.
    const daysAgo = Math.floor(
      (Date.now() - new Date(m.event_time).getTime()) / (1000 * 60 * 60 * 24)
    );
