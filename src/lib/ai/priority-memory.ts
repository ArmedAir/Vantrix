...
/**
 * Guards promote*() against overwriting a row the user has since corrected
 * via PATCH /api/memories/priority/[id]. Without this, a user fixing a
 * wrong memory would only see it revert the next time the same
 * memory_graph node / user_facts row gets re-promoted — the exact
 * "hallucination drift" failure mode the user-facing memories UI exists to
 * fix. One extra read per promotion call; these are already fire-and-forget
 * per this module's header comment, so it doesn't add request-path latency.
 */
async function isFrozenByUser(
  userId: string,
  characterId: string,
  source: 'memory_graph' | 'user_facts',
  sourceId: string,
): Promise<boolean> {
...
export async function promoteMemoryNode(
  ...
  try {
    ...
    if (await isFrozenByUser(userId, characterId, 'memory_graph', node.id)) return;
...
export async function promoteFact(
  ...
  try {
    ...
    if (await isFrozenByUser(userId, characterId, 'user_facts', fact.id)) return;
...
export interface PriorityMemory {
  ...
  importance:   number;
  created_at:   string;
  // USER-CONTROL: is_pinned/user_edited — see 20270109_priority_memories_
  // user_control.sql and PATCH /api/memories/priority/[id].
  is_pinned:    boolean;
  user_edited:  boolean;
}
...
export async function getPriorityMemories(
  ...
  let query = supabaseAdmin
    ...
    .eq('user_id', userId)
    .eq('character_id', characterId)
    // Pinned first (matches community_posts' pinned-first ordering), then
    // the existing importance/recency sort within each group.
