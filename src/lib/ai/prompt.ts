 ...
 *   5.6. Relationship-Tier Self-Awareness (RaaS) — how much accumulated
 *        history/memory this relationship carries, independent of the
 *        narrative stage above (see raas.ts's formatRelationshipTierForPrompt)
...
import { formatSeedMemoriesForPrompt, filterAccessibleSeedMemories, type CharacterSeedMemory } from '@/lib/ai/character-seed-memory';
...
export interface AssembleOptions {
  ...
  /**
   * memory-tiers/memory-tier-engine.ts — the new independent short/medium/
   * long-term memory subsystem's getTieredMemoryContext().promptBlock.
   * Injected alongside, not instead of, canonicalMemory/memoryFacts above:
   * this subsystem deliberately shares no keys, tables, or read/write path
   * with memory.ts/user-fact-graph.ts/memory-graph.ts (see that module's
   * README). Empty string until the consolidation cron has promoted
   * anything, so omitting/leaving this undefined is a true no-op.
   */
  tieredMemoryPrompt?:    string | null;
  /**
   * lib/commerce/raas.ts's formatRelationshipTierForPrompt() output — the
   * character's own private awareness of how much accumulated history it
   * currently carries with this specific person (Spark/Bond/Soulbound,
   * gated by the RaaS relationship tier — see companion-context.ts's
   * `commerce` field). Deliberately never names the tier/plan/price; see
   * that formatter's own doc comment. Injected right after the narrative
   * Relationship Context section (5) since it's the same axis — "how well
   * do I know this person" — just the monetized-depth half of it rather
   * than the earned-stage half.
   */
  relationshipTierPrompt?: string | null;
  /**
...
export function assembleFullPrompt(opts: AssembleOptions): string {
  ...
  // LEAK FIX: filtered by availableSecretTiers before formatting — this used
  // to inject every seed memory unconditionally, including locked-tier
  // secrets (dark secrets carry the highest importance weight in the table,
  // so they were reliably visible from message one). The Secret-Tier Gate
  // section below only ever added advisory text asking the model not to
  // reveal them; it never removed the content. This is the actual gate.
  ...
  if (seedMemories && seedMemories.length > 0) {
    const accessible = filterAccessibleSeedMemories(seedMemories, availableSecretTiers);
    const sm = formatSeedMemoriesForPrompt(accessible);
  ...
  // 5.6. Relationship-Tier Self-Awareness (RaaS) — see raas.ts's
  // formatRelationshipTierForPrompt() for why this is a distinct axis from
  // the narrative stage above: that's the earned, story-driven relationship
  // progression; this is the monetized-depth axis (how much accumulated
  // history/memory this relationship actually carries) that every
  // relationship has from its very first message (defaults to 'spark'),
  // independent of whether a `relationship` row exists yet.
  if (relationshipTierPrompt) {
  ...
  if (tieredMemoryPrompt && tieredMemoryPrompt.trim()) {
    sections.push(tieredMemoryPrompt.trim());
  }
