...
//
// HARD CAP FIX (2026-09-07): OpenRouter rejects the whole request with a
// 400 ("'models' array must have 3 items or fewer") once this list grows
// past 3 — which OPENROUTER_MODEL_PRIORITY's 6 entries always did (5 left
// after filtering out the primary). Every 'openrouter' call — chat-lines
// generation, moderation's primary AI check, ordinary chat — was silently
// failing on this alone. Moderation's fallback attempt then failed too
// (same provider, same cap), so both attempts came back null and
// moderation fail-closed on every comment/post/reply platform-wide; see
// aiModerationCheck's "Content review is temporarily unavailable" path in
// @/lib/moderation. Sliced to the top 3 by priority — still real
// redundancy, just within what the API actually accepts.
const OPENROUTER_MODELS_ARRAY_MAX = 3;

function buildOpenRouterFallbackChain(primary: string): string[] {
  return OPENROUTER_MODEL_PRIORITY.filter(m => m !== primary).slice(0, OPENROUTER_MODELS_ARRAY_MAX);
}
...
export async function routeCompletion(req: ProviderRequest): Promise<ProviderResponse> {
  ...
  for (const providerName of order) {
    ...
    try {
      ...
      // (freeSMART/POWER). A PEAK escalation (premiumPEAK)
