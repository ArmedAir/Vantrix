...
// Exported so any code path that creates a relationship row outside the
// normal ensureRelationship() flow (e.g. the relationship route's PATCH
// insert-fallback, for a user who customizes a nickname before their first
// chat message) uses the real per-stage cap instead of a second hardcoded
// number that can silently drift out of sync with this one.
export const STAGE_XP_CAPS: Record<RelationshipStage, number> = {
  stranger:     50,
  acquaintance: 150,
  friend:       300,
  close_friend: 500,
