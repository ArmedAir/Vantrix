...
export async function getFactGraph(
  ...
  try {
    ...
    // DEFENSIVE (see memory-arbiter.ts's arbitrateMemoryContext, 2026-09-02):
    // this return type promises UserFact[], but a raw redis.get<T>() cast
    // doesn't validate the cached value's actual shape — a corrupted or
    // mis-written cache entry under this key would otherwise be returned
    // as-is and crash the first .map() a caller runs over it. Falling
    // through to the Supabase read (which also refreshes the cache below)
    // is strictly safer than trusting an unverified cache hit.
