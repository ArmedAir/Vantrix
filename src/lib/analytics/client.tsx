...
import { hasConsent } from "@/lib/consent/cookie-consent";
...
function ensureInit(): Promise<void> {
  ...
  if (!hasConsent("analytics")) return Promise.resolve(); // no consent decision yet, or user declined
...
export function AnalyticsPageview() {
  ...
  useEffect(() => {
    const onConsentUpdated = () => fire();
    window.addEventListener("vantrix:consent-updated", onConsentUpdated);
    return () => window.removeEventListener("vantrix:consent-updated", onConsentUpdated);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathname, searchParams]);
