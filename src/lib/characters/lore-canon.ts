...
export interface LoreCanonEntry {
  ...
  /** true only for Carter — deliberately never LoRA-locked, see note below. */
...
export const ARCHIVE_OF_ECHOES_LORE_CANON: LoreCanonEntry[] = [
  ...
  },
  {
    ...
    scene_prompts: scenes(
      ...
      'facing Andrew across a submerged courtyard, both holding pieces of the same shattered map, water level rising slowly between them',
    ),
  },
  {
  ...
  },
  {
    ...
    scene_prompts: scenes(
      ...
      'back-to-back with Elizabeth on a shattered observatory floor, one reading hope from the same sky the other reads warning',
    ),
  },
  {
  ...
  },
  {
    ...
    scene_prompts: scenes(
      ...
      'facing Michael across the same threshold, the old disagreement between them finally spoken aloud',
    ),
  },
  {
