...
export class CircuitBreaker {
  ...
  reset() {
    ...
    // Fail-open, same as every other Redis touch in this class (see file
    // header): `redis` is a lazy proxy that throws SYNCHRONOUSLY on access
    // when UPSTASH_REDIS_REST_URL/TOKEN aren't set, so `redis.del(...)`
    // itself — not just the network call — could throw before `.catch()`
    // ever attached, crashing the caller even though the in-process reset
    // just above already succeeded. Wrapped so a missing/unavailable Redis
    // never turns a local reset into a thrown error.
    try {
      redis.del(`vantrix:cb:${this.cfg.name}`).catch(bg(`circuitBreaker.${this.cfg.name}.resetCacheDel`));
    } catch (err) {
      bg(`circuitBreaker.${this.cfg.name}.resetCacheDel`)(err);
    }
  }
}
