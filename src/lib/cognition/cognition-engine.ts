 ...
 * refinement of it: habit-engine.ts tracks individual cueresponse
 ...
 * same weekly cadence as wisdom's cron (already live —
 * api/cron/wisdom-habit-maintenance/route.ts). GAP-FIX: as of this pass,
 * runCognitionCycle() (= consciousness-loop.ts's runConsciousnessCycle())
 * now calls considerAutomaticResponse() when a caller supplies `cue`
 * (chat/stream/route.ts derives it via deriveHabitCue(), also re-exported
 * below) — the storage gap and the call-site wiring are both closed now.
 */
...
// Note: HabitCue itself is already re-exported below, alongside the rest
// of habit-engine.ts's surface.
