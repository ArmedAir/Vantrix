...
export async function getCreatorCharacters(
  ...
  let query = supabase
    ...
    .eq("active", true)
    .eq("is_public", true)
    // LIST-DETAIL-PARITY FIX: matches getCharacterDetail()'s gate — see the
    // identical fix on /api/characters. These cards link to
    // /characters/[id], which 404s on a non-live character.
    .eq("is_live", true)
