...
// COLUMN-DRIFT FIX (this session): dating_milestones has two nullable
// milestone-identifier columns, `milestone_type` and `milestone` (see
// 20240101_production.sql). Every real INSERT into this table — date/[id]/
// complete/route.ts, mood/route.ts, gifts/route.ts — writes `milestone`,
// none ever populate `milestone_type`, so `milestone_type` is NULL on
// every row that exists in production. getMatchesForUser() used to select
// only `milestone_type`, so match/[id]/page.tsx's Milestones list always
// got `milestone_type: null` for its 3 most recent entries: the label
// rendered blank, and worse, that null flowed straight into
// ShareCardButton's request body as milestoneKey/milestoneLabel — both
// required strings server-side (zod) — so every milestone share attempt
// failed with a 400 "Invalid request". all-milestones-drawer.tsx already
// coalesces `m.milestone_type ?? m.milestone ?? ""` correctly; this type
// and the query/mapping below now do the same at the source, so every
// downstream reader of `milestone_type` keeps working unchanged.
...
export async function getMatchesForUser(
  ...
  const { data: milestonesData } = matchIds.length
    ? await supabaseAdmin
        ...
        .select('match_id, milestone_type, milestone, created_at')
        .in('match_id', matchIds)
        .order('created_at', { ascending: false })
        .returns<RawMilestoneRow[]>()
    : { data: [] as RawMilestoneRow[] };
  ...
  for (const ms of milestonesData ?? []) {
    ...
    milestoneByMatch[ms.match_id]!.push({
      match_id: ms.match_id,
      milestone_type: ms.milestone_type ?? ms.milestone ?? '',
