...
export async function getDatingWorldHome(userId: string): Promise<DatingWorldHome> {
  try {
    ...
    if (candidates.length > 0) {
      ...
      // Wrapped in try/catch (not just `.catch()`) for the same reason as
      // CircuitBreaker.reset() (circuit-breaker.ts): `redis` throws
      // SYNCHRONOUSLY on access when Redis env vars aren't set. Uncaught,
      // that would bubble to this function's outer catch and blank the
      // entire dating home page (EMPTY_WORLD) just because the "tonight's
      // pinned match" cache — which is allowed to be missing — errored.
      let pinnedId: string | null = null;
      try {
        pinnedId = await redis.get<string>(key).catch(() => null);
      } catch { /* non-critical — pin is best-effort */ }
