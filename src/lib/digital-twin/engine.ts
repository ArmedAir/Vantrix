...
// ── Scoped deletion ─────────────────────────────────────────────────────
//
// data-privacy-panel.tsx's DELETE /api/user/delete wipes the whole account.
// Until now there was no way to wipe just the twin — the trained persona
// (digital_twin_profiles) plus everything it's ever said
// (digital_twin_messages) — without going through full account deletion.
// Deliberately does NOT touch the underlying source data (the user's own
// chat messages / community posts): that history belongs to the user's
// normal chat/community activity independent of the twin feature, and
// buildStyleProfile() can re-derive a fresh twin from it later if the user
// opts back in. Idempotent: deleting an already-empty/non-existent twin
