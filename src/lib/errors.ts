 ...
 *
 * LOGGING-FIX (2026-09-09): the plain-object fallback below used to be
 * `{ error: String(err) }`. For a non-Error thrown value that's still a
 * real object without a custom toString — e.g. a PostgREST/Supabase
 * error shape that reached here as `instanceof Error` false (observed in
 * production on /api/characters: "Characters GET error" logged with
 * `"error":"[object Object]"`, the default Object.prototype.toString
 * result, on every occurrence) — String() silently threw away every
 * field that would have explained the failure. JSON.stringify at least
 * preserves the object's own enumerable properties (message/code/details/
 * hint for a PostgrestError-shaped value); primitives (string thrown,
 * number, etc.) still come through readably since JSON.stringify of a
 * primitive is just its literal.
 */
