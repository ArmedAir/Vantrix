...
import { PER_VIDEO_COST_USD_ESTIMATE } from '@/lib/media-budget';
...
function videoDollarBudgetKey(): string {
  const day = new Date().toISOString().slice(0, 10); // YYYY-MM-DD UTC
  return `vantrix:daily:vid:platform-cents:${day}`;
}
...
async function checkPlatformVideoBudget(): Promise<{ allowed: boolean; used: number; limit: number }> {
  ...
  const dollarLimitCents = Math.round(env.PLATFORM_DAILY_VIDEO_DOLLAR_BUDGET * 100);
  const dollarKey = videoDollarBudgetKey();
  const estimatedCostCents = Math.round(PER_VIDEO_COST_USD_ESTIMATE * 100);

  try {
    const pipe = redis.pipeline();
    pipe.incr(key);
    pipe.incrby(dollarKey, estimatedCostCents);
    ...
    pipe.expire(dollarKey, ttlSeconds);

    const [count, spendCents] = await pipe.exec() as [number, number, unknown, unknown];

    // COST-GUARD: two independent checks, same as peak-budget.ts's
    // request-count + dollar-ceiling pair for PEAK text. Either tripping
    // denies the request — count alone was the only guard before this.
    if (count > limit) return { allowed: false, used: count, limit };
    if (spendCents > dollarLimitCents) {
      logger.warn('animate-portrait: platform video DOLLAR budget exceeded (request count still under its own limit)', {
        spendCents, dollarLimitCents, requestCount: count,
      });
...
export interface AnimatePortraitInput {
  ...
  /**
   * Only set by the manual /api/characters/[id]/animate route, which is the
   * sole call site that reserves against the per-user media-budget ceiling
   * (see checkMediaBudget('video', ...) there) before triggering this. The
   * other auto-trigger call sites (creation, admin backfill, cron backfill)
   * never reserve a per-user budget, so they must leave this undefined.
   * Threaded through to the fal webhook (as a query param) purely so the
   * webhook — which otherwise only ever sees characterId — knows whose
   * reservation to settle/release once the job resolves. Never used for
   * anything else (no gating/moderation logic keys off this).
   */
  billedUserId?: string;
}
...
export async function animateCharacterPortrait(
  ...
  const { characterId, imageUrl, billedUserId } = input;

  try {
    const webhookUrl = new URL(`${env.NEXT_PUBLIC_APP_URL}/api/webhooks/fal-animate`);
    webhookUrl.searchParams.set('characterId', characterId);
    if (billedUserId) webhookUrl.searchParams.set('billedUserId', billedUserId);
...
async function runPrimaryAnimation(input: AnimatePortraitInput): Promise<void> {
  const { characterId, billedUserId } = input;
  ...
  if (!budget.allowed) {
    ...
    // BUDGET-LEAK-FIX: the job never reached fal — if a per-user reservation
    // was made (manual /animate route), it must be released here or it sits
    // "reserved" against the user's monthly video ceiling for 35 days
    // (media-budget.ts's TTL) despite nothing ever being billed. Previously
    // nothing in this file or the fal-animate webhook ever settled or
    // released this reservation at all, on any path.
    if (billedUserId) {
      const { releaseMediaReservation } = await import('@/lib/media-budget');
      await releaseMediaReservation('video', billedUserId);
    }
    await supabaseAdmin
      .from('characters')
      .update({
  ...
  if (!falResult.success || !falResult.falRequestId) {
    ...
    // BUDGET-LEAK-FIX: same reasoning as above — submit itself failed, so
    // no provider compute was ever billed; release rather than leave it
    // stuck reserved.
    if (billedUserId) {
      const { releaseMediaReservation } = await import('@/lib/media-budget');
      await releaseMediaReservation('video', billedUserId);
