...
const NEGATIVE_PROMPT_DEFAULT = [
  ...
  'low quality, low detail, noise, grain, artifacts, compression artifacts, jpeg artifacts,',
  'overexposed, underexposed, dim, dark, murky, muddy lighting, poorly lit, low contrast',
].join(' ');

/**
 * Appended to every character prompt (scenes + base images) to push Flux
 * toward sharper, higher-fidelity renders with brighter, more even lighting.
 * Kept separate from the caller-supplied prompt so it always applies
 * regardless of scene content.
 */
const QUALITY_LIGHTING_BOOST =
  'highly detailed, sharp focus, crisp fine detail, high resolution, professional photography, ' +
  'bright even lighting, well-lit, luminous, vivid colors, high dynamic range';
...
export async function generateScene(
  ...
  // Full prompt: locked character identity + scene context + quality/lighting boost
  const fullPrompt = `${facePrompt} ${scenePrompt}, ${QUALITY_LIGHTING_BOOST}`;
...
export async function generateBaseImage(
  ...
  const boostedPrompt = `${prompt}, ${QUALITY_LIGHTING_BOOST}`;
  ...
  for (let i = 0; i < canonScenes.length; i += batchSize) {
    ...
    const results = await Promise.allSettled(
      batch.map(scene => generateScene({
        ...
        facePrompt,
        scenePrompt: scene,
        steps: 36, // bumped from 28 for sharper canon sets; still lighter than the 50-step default since this generates ~40 images per character
      }))
    );
