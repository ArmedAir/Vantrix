...
export interface ModerationHold {
  ...
  reviewer_notes: string | null;
  /** Set only for post-publish holds (async feed-comment review) — see
   *  20260909_async_comment_moderation.sql. Null for pre-publish
   *  keyword-match holds. */
  comment_id: string | null;
  /** Same scope as comment_id — lets the panel link straight to the post
   *  instead of unpacking submitted_payload JSON. See
   *  20260909b_comment_moderation_hardening.sql. */
  post_id: string | null;
}

export interface ModerationPromptConfig {
  id?: string;
...
export async function setKeywordAction(
  ...
  if (!res.ok) throw new Error("Failed to update keyword action");
}

// ── Moderation holds (hold_for_review keyword matches) ─────────────────────
export const fetchModerationHolds = (status: string) =>
  fetchQueue<ModerationHold>("moderation-holds", status);
export const reviewModerationHold = (id: string, status: string, notes?: string) =>
  patchQueueItem("moderation-holds", { id, status, notes });

// ── Moderation prompt tuning (admin-only, additive, hard-block-guarded) ────
export async function fetchModerationPromptConfig(): Promise<ModerationPromptConfig> {
  const res = await fetch("/api/admin/moderation-settings");
  if (!res.ok) throw new Error("Failed to load moderation settings");
  const data = await res.json();
  return data.config ?? { extra_allow_notes: "", extra_block_notes: "" };
}

export async function updateModerationPromptConfig(input: {
  extraAllowNotes?: string;
  extraBlockNotes?: string;
}): Promise<void> {
  const res = await fetch("/api/admin/moderation-settings", {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.error ?? "Failed to save moderation settings");
  }
}
...
// ── Creator Fund self-dealing / farming queue ───────────────────────────────
// See 20270115_creator_fund_character_value_score.sql's creator_fund_flags
// table and /api/admin/creator-fund-flags — same fetchQueue/patchQueueItem
// shape as every other queue above.

export interface CreatorFundFlag {
  id: string;
  created_at: string;
  character_id: string;
  creator_id: string;
...
export const fetchCreatorFundFlags = (status: string) =>
  fetchQueue<CreatorFundFlag>("creator-fund-flags", status);
export const reviewCreatorFundFlag = (id: string, status: string, notes?: string) =>
  patchQueueItem("creator-fund-flags", { id, status, notes });
