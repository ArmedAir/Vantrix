...
/**
 * Lightweight name-only lookup for the "Remixed from X" credit line on
 * the detail page — only called when remixed_from_character_id is set,
 * same "don't pay for a lookup most rows don't need" posture as
 * character-initiative.ts's nearWeeklyBoundary-gated memory_graph query.
 */
export async function getRemixSourceName(id: string): Promise<string | null> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("characters")
    .select("name")
