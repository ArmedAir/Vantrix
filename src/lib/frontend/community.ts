...
import { getPostById, getPostsPage, getRepliesForPost } from "@/lib/community/get-posts";
...
/**
 * ROOT-CAUSE FIX (2026-09-12): these three used to go through
 * fetchInternal (HTTP self-fetch) — see get-posts.ts's header comment for
 * the full writeup of why that was 404ing intermittently on post open and
 * on like/reply-triggered refreshes. Now calls the in-process logic
 * directly, same fix already applied to getCommunityBySlug above.
 */
export async function getCommunityPosts(
  ...
  try {
    ...
    return await getPostsPage(slug, user?.id ?? null, params);
  } catch (err) {
...
export async function getCommunityPost(id: string): Promise<CommunityPost | null> {
  try {
    const { user } = await getAuthedUser();
    return await getPostById(id, user?.id ?? null);
  } catch (err) {
...
export async function getCommunityReplies(id: string): Promise<CommunityReply[]> {
  try {
    const { user } = await getAuthedUser();
    return await getRepliesForPost(id, user?.id ?? null);
  } catch (err) {
