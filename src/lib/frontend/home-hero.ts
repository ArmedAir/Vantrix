...
export interface HomeHeroTopMatch {
  ...
  isNew: boolean;
  character: { id: string; name: string; image_url: string | null; tags: string[] };
}
...
interface TopMatchRow {
  ...
  match_tier: string;
  milestones: number;
  character: { id: string; name: string; image_url: string | null; tags: string[] | null } | null;
}
  ...
  try {
    const [profileRes, streakRes, matchRes] = await Promise.all([
      ...
      supabaseAdmin
        .from("dating_matches")
        .select(
          "id, compatibility_pct, match_tier, milestones, character:characters!dating_matches_character_id_fkey ( id, name, image_url, tags )"
        )
        .eq("user_id", userId)
        .gte("bond_score", 20)
    ...
    const topMatch: HomeHeroTopMatch | null =
      matchRow && matchRow.character
        ? {
            ...
            character: { ...matchRow.character, tags: matchRow.character.tags ?? [] },
          }
        : null;
