...
export interface PremiumBillingOption {
  ...
  // NAIRA-VISIBILITY-FIX (2026-09-05): tiers.price_ngn is a real column on
  // every billing-interval row (see 20261025_billing_discount_...sql —
  // annual/quarterly each have their own NGN figure, not a computed
  // multiple of the monthly one), but it was never selected here, so a
  // Nigerian visitor picking Yearly/Quarterly saw a $ price on this card
  // and only found out the actual Naira amount after landing on
  // Paystack's own checkout page. Selected below and surfaced so the
  // card can show it up front, same as Paystack will actually charge.
...
export async function getPremiumBillingOptions(baseTierSlug: string): Promise<PremiumBillingOption[]> {
  ...
  const { data } = await supabase
    .from("tiers")
    .select("id,billing_interval,price_ngn")
    .eq("base_tier_slug", baseTierSlug);

  const rows = (data as { id: string; billing_interval: string; price_ngn: number }[] | null) ?? [];
  const idByInterval = new Map(rows.map(r => [r.billing_interval as BillingPeriod, r.id]));
  const ngnByInterval = new Map(rows.map(r => [r.billing_interval as BillingPeriod, r.price_ngn]));
  ...
  return getBillingPlans()
    ...
    .map(plan => ({
      ...
      totalPriceNgn: ngnByInterval.get(plan.id) ?? 0,
    }));
}
