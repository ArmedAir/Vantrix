...
export type KeywordWatchAction = 'log' | 'hold_for_review';

export interface WatchedKeyword {
  id:       string;
  keyword:  string;
  isRegex:  boolean;
  action:   KeywordWatchAction;
}
...
async function loadActiveKeywords(): Promise<WatchedKeyword[]> {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select('id, keyword, is_regex, action')
    .eq('active', true);
  ...
  if (error) {
    ...
    // NOTE: this "fail open" posture is fine for 'log' entries (worst case,
    // a hit goes unrecorded) but also means a DB hiccup could momentarily
    // fail open on a 'hold_for_review' entry too — that's an accepted
    // tradeoff consistent with this module's log-only-by-default design
    // (see file header); hold_for_review still sits behind the two hard
    // layers in moderateCharacter(), which are unaffected by this cache.
  ...
  cachedKeywords = (data ?? []).map(row => ({
    ...
    action:  ((row as { action?: string }).action === 'hold_for_review' ? 'hold_for_review' : 'log') as KeywordWatchAction,
  }));
  cachedAt = now;
  return cachedKeywords;
}
...
/**
 * Same shape as checkKeywords(), but filtered to only keywords an admin
 * explicitly marked action='hold_for_review'. Used exclusively by
 * moderateCharacter() (content-CREATION surfaces) to decide whether a
 * submission should be held rather than published — never called from the
 * live chat reply path. See keyword_watchlist.action's migration comment
 * for the full scope decision.
 */
export async function checkKeywordsForHold(text: string): Promise<KeywordMatch[]> {
  if (!text) return [];
  const keywords = (await loadActiveKeywords()).filter(kw => kw.action === 'hold_for_review');
  if (keywords.length === 0) return [];
