...
import { sendNativePushToUser } from "@/lib/push/send-native-push";
...
export async function emitNotification(input: EmitNotificationInput): Promise<string | null> {
  ...
  if (typePrefs.push && !skipPush) {
    // Web Push (browser/PWA subscriptions) and native push (Capacitor
    // iOS/Android app via FCM) are two independent delivery channels to
    // two independent device tables — fire both, same payload shape.
    // Each one individually no-ops if unconfigured or the user has no
    // registered devices on that channel, so this is always safe to call.
    Promise.allSettled([
      ...
      .then(async ([webResult, nativeResult]) => {
        const sent =
          (webResult.status === "fulfilled" && webResult.value.sent > 0) ||
          (nativeResult.status === "fulfilled" && nativeResult.value.sent > 0);
