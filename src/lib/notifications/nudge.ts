...
export async function getEligibleNudges(limit = 500): Promise<NudgePayload[]> {
  ...
  for (const match of matches) {
    ...
    // SYNCHRONOUSLY on access when Redis env vars aren't set. Uncaught, that
    // would abort this whole loop instead of just failing open on this one
    // match's cooldown check, same fail-open intent as isUserAtDailyCap()
    // above ("Redis unavailable — fail open (send the nudge rather than
    // suppress it)").
    const matchKey = nudgeKey(match.user_id, match.id);
    let fired: unknown = null;
    try {
      fired = await redis.get(matchKey).catch(() => null);
