 ...
 *
 * Single source of truth for the 15 inbox categories from the product
 * spec (14 original + streak_risk, added for the daily-streak win-back
 * push — see src/lib/notifications/streak-risk.ts). `NOTIFICATION_DEFAULTS`
 * is what notification_preferences.prefs
...
export const NOTIFICATION_LABELS: Record<NotificationType, { label: string; description: string }> = {
  ...
  character_new_post:     { label: "New post from a character you follow", description: "A character you follow shares a new post" },
  ...
  streak_risk:            { label: "Streak reminder",              description: "Your daily streak is still open and hasn't been checked in today" },
};
...
export const CATEGORY_TYPES: Record<NotificationCategory, readonly NotificationType[]> = {
  ...
  social: ["dating_match", "character_liked", "character_followed", "character_new_post", "community_reply", "gift_received"],
  world: ["milestone_unlocked", "world_event", "character_birthday", "streak_risk"],
