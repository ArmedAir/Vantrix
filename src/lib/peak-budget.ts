...
//
// Pricing basis (2026-09-05, updated for the current two-tier model — see
// @/lib/tiers/config's BASE_MONTHLY_PRICE): premium sells at $9.99/mo
// monthly, $6.99/mo quarterly (30% off), or $3.99/mo annual (60% off) — same
// tier, same PEAK budget pool, all three billing cycles draw from the same
// per-user monthly budget below. Sized against the WORST case (annual,
// $3.99/mo effective revenue) so the budget stays safe no matter which
// cycle a given premium user is on, not just the sticker monthly price.
// Target: PEAK AI cost ≤ 30% of worst-case revenue = ~$1.20/user/month.
// At an estimated ~$0.0285/request (2000 in / 1500 out tokens, PEAK's
// $3/$15 per-M pricing), that's ~42 requests/month — rounded down to 40.
...
export const PEAK_MONTHLY_REQUEST_BUDGET: Record<Tier, number> = {
  ...
  spark:      0,
  basic:      0,
  // TIER-COLLAPSE FIX (2026-09-05): this was left at 0 when the old 6-tier
  // model (spark/basic/premium/elite/enterprise) collapsed to two tiers
  // (free/premium). 'elite' — the old top paid tier, and the one premium
  // now represents — had a real budget (650 req / $35, sized for its old
  // $49.99/mo price). Premium never got ported over, so PEAK_ELIGIBLE_PLANS
  // in model-router.ts marks premium eligible for PEAK, classifyComplexity
  // can flag a message as PEAK-worthy, but checkPeakBudget() always
  // returned `tier_ineligible` before even checking Redis — every deep/
  // emotional-moment premium message was silently downgraded to POWER.
  // Re-sized from scratch for premium's real (lower) price — see pricing
  // comment on PEAK_MONTHLY_DOLLAR_CEILING_USD below.
  premium:    40,
...
export const PEAK_MONTHLY_DOLLAR_CEILING_USD: Record<Tier, number> = {
  ...
  spark:      0,
  basic:      0,
  premium:    1.19,  // ≤ 30% of premium's worst-case ($3.99/mo annual) revenue —
                     // see pricing comment above. Independent second guard
                     // alongside the 40-request ceiling; catches long-output
                     // abuse even if request count alone looks fine.
