...
// TWO-TIER MODEL: 'premium' is the only paid tier that exists (see
// tiers/config.ts) — it gets the top 'high' lane outright, so a Premium
// member's job is always dequeued ahead of every pending free-tier job.
// Previously this checked for 'elite'/'enterprise'/'basic', tier ids that
// were fully retired by the Aug 2026 single-plan refactor — 'high' was
// unreachable dead code and premium sat in 'normal', one rung below where
// the current pricing model says it should be. Any unrecognized tier
// string falls back to 'low', same fallback used by normaliseTierForGate()
// in lib/auth/plan.ts.
export function tierToPriority(tier: string): QueuePriority {
  return tier === 'premium' ? 'high' : 'low';
}
