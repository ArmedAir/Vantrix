 ...
 *
 * SCALE-FIX (2026-09-04): checkAndSignalScaleOut() previously only wrote a
 * Redis key — the only consumer anywhere in the codebase was the
 * admin-facing GET /api/queue/workers route, so the signal meant nothing
 * unless a human happened to be looking at that endpoint. Now fires an
 * actual alert (Sentry, unsampled — this event is rare and high-signal,
 * unlike the 10%-sampled logger.error() path — plus ANOMALY_WEBHOOK_URL if
 * configured, reusing the existing generic ops-alert channel rather than
 * adding a new one) on the rising edge only, so a persistent overload
 * doesn't spam the channel every time this is checked.
 *
...
export async function checkAndSignalScaleOut(queueDepth: number): Promise<boolean> {
  ...
  try {
    // Rising-edge check happens BEFORE the setex below refreshes/creates
    // the key, so this is only true the first time the threshold is
    // crossed in a given overload episode, not on every subsequent poll
    // while it persists (worker-runner checks every 30s; the serverless
    // /api/queue/worker route checks on every invocation, ~every minute).
    const alreadySignalled = (await redis.exists(SCALE_SIGNAL_KEY)) === 1;
    ...
    if (!alreadySignalled) {
      notifyScaleOut(queueDepth).catch(err =>
        logger.error('scaler: notifyScaleOut failed', { error: err instanceof Error ? err.message : String(err) })
      );
    }
    return true;
  } catch { return false; }
...
/**
 * Fire-and-forget alert on the rising edge of a scale-out signal. Never
 * throws into the caller — a failing alert must not be what breaks queue
 * processing. Two independent channels so one being unconfigured/down
 * doesn't mean silence:
 *   - Sentry, unsampled (unlike logger.error()'s 10% default sample rate —
 *     this event is rare enough that every occurrence is worth seeing).
 *   - ANOMALY_WEBHOOK_URL, if set — the same Slack/PagerDuty-compatible
 *     webhook src/lib/ai/anomaly-detector.ts already posts to, reused here
 *     rather than introducing a second ops-alert channel to configure.
 */
async function notifyScaleOut(queueDepth: number): Promise<void> {
  const message = `Vantrix queue depth (${queueDepth}) crossed the scale-out threshold (${SCALE_OUT_THRESHOLD}). No autoscaler is wired to this signal — capacity must be added manually: run more worker-runner instances, or confirm /api/queue/worker's drain loop is keeping up. See GET /api/queue/workers for live worker/DLQ state.`;

  const sentry = import('@sentry/nextjs')
    .then(Sentry => Sentry.captureMessage(message, 'warning'))
    .catch(err => logger.error('scaler: Sentry alert failed', { error: err instanceof Error ? err.message : String(err) }));

  const webhookUrl = env.ANOMALY_WEBHOOK_URL;
  const webhook = webhookUrl
    ? fetch(webhookUrl, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ text: ` *Queue scale-out threshold crossed*\n${message}` }),
      }).catch(err => logger.error('scaler: webhook alert failed', { error: err instanceof Error ? err.message : String(err) }))
    : Promise.resolve();

  await Promise.allSettled([sentry, webhook]);
}
