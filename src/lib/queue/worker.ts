...
async function executeJob(job: ChatJob): Promise<{ reply: string; tokensUsed: number }> {
  ...
  const {
    ...
    commerce: { relationshipTier },
  } = companionContext;

  // ENGINE-PARITY (see file header): chat/stream/route.ts's own
  // SELF-AWARENESS FIX computes this from the same companionContext.commerce
  // field — kept identical here rather than left out, which is exactly the
  // drift this file's header describes.
  const relationshipTierPrompt = formatRelationshipTierForPrompt(relationshipTier);
