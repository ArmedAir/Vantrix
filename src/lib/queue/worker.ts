 ...
 *     candidates, theory-of-mind, belief-pipeline, reputation prompts) is
 *     deeply interleaved, turn-decision-making machinery — porting it
 *     safely is a dedicated task of its own, not something to fold into
 *     this pass. A queued (503-fallback) reply still plans against a
 *     simpler, more direct response-planner call instead of that full
 *     intent system. Flagged here so the next person (or the next me)
 *     finds this note instead of rediscovering the gap from a screenshot.
 *
