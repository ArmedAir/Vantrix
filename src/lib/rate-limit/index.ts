...
export async function checkActionLimit(
  userId: string,
  action: 'community_post' | 'community_reply' | 'community_room_chat' | 'character_create' | 'ai_recommend' | 'character_concept_generate' | 'character_post_create',
): Promise<RateLimitResult> {
  const CEILINGS: Record<typeof action, { max: number; windowMs: number }> = {
    community_post:    { max: 10, windowMs: 60 * 60_000 }, // 10/hour
    // A user posting as their own character — same spam-prevention shape
    // as community_post (1 write per UI action, not a hot loop), scoped
    // per-character-owner rather than per-community.
    character_post_create: { max: 12, windowMs: 60 * 60_000 }, // 12/hour
...
/**
 * Read-only companion to checkSwipeLimit — same midnight-UTC-aligned
 * `vantrix:swipe:*` key, but never increments it.
 *
 * SWIPE-DISPLAY-FIX: GET /api/dating/deck was calling checkSwipeLimit()
 * (the atomic increment-and-check used by POST /api/dating/swipe to
 * actually *record* a swipe) just to report the caller's current usage
 * for the "X swipes left today" UI. Since checkMidnightAlignedCap
 * unconditionally INCRs the counter on every call, that meant simply
 * opening or refreshing the deck page consumed one swipe from the daily
 * cap — with no swipe having happened. Reload the page enough times (or
 * let a background remount/refetch do it) and `swipes.remaining` hits 0,
 * SwipeDeck fires the paywall and disables swiping, and no amount of
 * actually swiping fixes it because the cards were never the problem —
 * the counter was already spent by the act of looking at them. This hit
 * every tier, but premium's much higher ceiling (1000/day vs free's 10)
 * meant it surfaced there as "swiping stops working after a bunch of
 * normal browsing" rather than free's near-immediate lockout.
 *
 * Fix: GET /api/dating/deck now calls this instead — a plain redis.get
 * with no side effect — while POST /api/dating/swipe keeps using
 * checkSwipeLimit() to atomically gate + record the one real action that
 * should ever move this counter.
 */
export async function peekSwipeLimit(
  userId: string,
  tier: Tier,
): Promise<{ allowed: boolean; used: number; limit: number }> {
  const limit = getTierLimits(tier).dailySwipes;
  ...
  try {
    ...
    // Mirrors checkMidnightAlignedCap's own allowed condition (count > limit
    // means blocked) one increment early: the *next* real swipe would push
    // the stored count to used + 1, so this is allowed iff used < limit.
    return used < limit
      ? { allowed: true, used, limit }
      : { allowed: false, used, limit };
  } catch {
    // Redis down — same fail-open posture as checkSwipeLimit itself.
    return { allowed: true, used: 0, limit };
  }
}
