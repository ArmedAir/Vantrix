 ...
 *
 * MOOD-TAGS-SHARE FIX: this used to be its own copy of the list now living
 * in ./moods as MOOD_TAGS — kept as a local alias (rather than rewriting
 * every MOOD_TAG_MAP reference below) so the browse page's vibe filter and
 * this scorer can't silently drift onto two different tag sets again.
 */
const MOOD_TAG_MAP = MOOD_TAGS;
...
// SWIPE-EXHAUSTION-FIX: getSwipedIds used to exclude every character a user
// had EVER swiped on, forever — a permanent, ever-growing blocklist with no
// way back in. That's fine at catalog scale, but this app currently only
// has ~30 dating_enabled characters, and premium's swipe cap is 1000/day
// (see tiers/limits.ts). Confirmed live: a real premium account had swiped
// all 30/30 eligible characters (15 like + 6 super_like + 9 pass), so
// GET /api/dating/deck legitimately returned zero candidates forever —
// that's the actual "premium swipes not displaying" report, not a fetch/
// render bug. The deck was working exactly as coded; the code just had no
// way to ever refill itself once a small catalog was fully swiped.
//
// Fix: only two things should ever be excluded —
//   1. Characters with a real dating_matches row (existingMatches below) —
//      a match is a standing relationship the user already has, surfaced
//      in Your World/Matches; it should never come back into the swipe
//      deck as if unmet.
//   2. Swipes (any direction, matched or not) within a short recycle
//      window — stops a card reappearing the instant after it's swiped,
//      same as before, just not forever.
// Everything else (a pass, or a like that the reciprocation-roll in
// POST /api/dating/swipe didn't turn into a match) recycles back into the
// deck once the window passes, so a small/slow-growing catalog never goes
// permanently dry.
const SWIPE_RECYCLE_WINDOW_MS = 24 * 60 * 60 * 1000; // 24h

async function getSwipedIds(userId: string): Promise<Set<string>> {
  const recycleCutoff = new Date(Date.now() - SWIPE_RECYCLE_WINDOW_MS).toISOString();

  const [{ data: matches }, { data: recentSwipes }] = await Promise.all([
    supabaseAdmin.from('dating_matches').select('character_id').eq('user_id', userId),
    supabaseAdmin
      .from('dating_swipes')
      .select('character_id')
      .eq('user_id', userId)
      .gte('created_at', recycleCutoff),
  ]);

  const ids = new Set<string>();
  for (const m of matches ?? []) ids.add(m.character_id as string);
  for (const s of recentSwipes ?? []) ids.add(s.character_id as string);
  return ids;
}
...
export async function getRecommendations(
  ...
  try {
    ...
    let query = supabaseAdmin
      ...
      // LIST-DETAIL-PARITY FIX: matches getCharacterDetail()'s gate — see
      // the identical fix on /api/characters. Every candidate this engine
      // returns ends up as a card linking to /characters/[id].
      .eq('is_live', true);
