...
/**
 * ALREADY-DONE FIX: World's "Scenarios Here" section (Location/Faction
 * detail pages) listed every scoped scenario unconditionally — a user who
 * already started (or finished) one kept seeing it offered again forever,
 * with no way to tell it apart from something new. `roleplay_sessions` has
 * one row per (user, scenario) start, regardless of completion status, so
 * "already participated" just means "has any row here" — a scenario a user
 * abandoned mid-way still counts as done for this list; re-entering an
 * in-progress one already happens through the character's own chat/
 * conversation, not by re-picking it from this list.
 */
async function getParticipatedScenarioIds(userId: string, scenarioIds: string[]): Promise<Set<string>> {
  ...
  if (error) {
    logger.warn('roleplay:scenarios:participated-lookup-failed', { error, userId });
    return new Set(); // fail soft — better to (rarely) re-show a done scenario than hide everything
  }

  return new Set((data ?? []).map(r => r.scenario_id as string));
}
 ...
 *
 * ALREADY-DONE FIX: `userId` (optional — every (app) route already
 * requires auth except bare "/", which never reaches this function, but
 * kept optional so a future unauthenticated caller degrades to the full
 * list rather than throwing) excludes scenarios the user has already
 * participated in — see getParticipatedScenarioIds() above.
 */
export async function listScenariosForLocation(locationSlug: string, userId?: string | null): Promise<RoleplayScenario[]> {
  const { data, error } = await supabaseAdmin
    .from('roleplay_scenarios')
    .select(SCENARIO_COLUMNS)
    .eq('is_active', true)
  ...
  const participated = await getParticipatedScenarioIds(userId, rows.map(r => r.id));
  return rows.filter(r => !participated.has(r.id));
}

/** Faction counterpart to listScenariosForLocation() — see that function's doc comment (including the ALREADY-DONE FIX). */
export async function listScenariosForFaction(factionSlug: string, userId?: string | null): Promise<RoleplayScenario[]> {
  const { data, error } = await supabaseAdmin
    .from('roleplay_scenarios')
    .select(SCENARIO_COLUMNS)
    .eq('is_active', true)
