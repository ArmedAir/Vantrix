...
export interface PublicCharacter {
  ...
  created_at: string;
  // ONBOARDING-WORLD-PROOF (2026-09-14): characters.current_goal is the
  // same ambition field memory-graph.ts's AMBITION_MILESTONES fires
  // "daily life update" memories off of — it's bio-adjacent copy the
  // creator/seed already wrote (not a live status feed), always global to
  // the character rather than any one user relationship, so it's exactly
  // as public-safe as `description` or `occupation` above. Exposed so the
  // pre-signup character-reveal step (character-reveal.tsx) can show a
  // concrete, honest "she's working on X right now" line — proof the
