...
// @ts-expect-error — deep import of an internal, unexported @supabase/ssr
// module (see the CAUTION comment above). No .d.ts is published for this
// path, so this call site is unchecked; the shapes are pinned by hand to
// match @supabase/ssr@0.12.0's cookies.js exactly. Re-verify against that
// file's source (not just its behavior) on any @supabase/ssr version bump.
