 ...
 *
 * ROOT-CAUSE FIX ("voting keeps repeating after already voted"): this
 * previously used `.maybeSingle()`, which — unlike its name suggests —
 * does not just return null for "no row"; it *errors* if more than one
 * row is ever matched, and that error was silently discarded here (only
 * `data` was destructured, never `error`). `castVote()` below treats any
 * falsy return from this function as "no existing vote" and proceeds to
 * INSERT another row. So the one scenario this function most needs to
 * handle correctly — "a vote already exists for this user" — was exactly
 * the scenario a swallowed multi-row error would silently misreport as
 * "no vote," reopening the vote buttons and letting the same user insert
 * again. The live `UNIQUE (choice_id, user_id)` constraint (see
 * 20260905_daily_world_choice.sql) makes 2+ rows impossible today, but
 * this function had zero defense if that ever changed (a migration
 * rerun, a restored backup predating the constraint, a manual DB edit) —
 * exactly the class of thing that turns into a silent, hard-to-trace
 * "why is this one user stuck re-voting forever" bug report. `.limit(1)`
 * with explicit ordering can never error on extra rows — it just
 * deterministically returns the earliest one — and any real query error
 * is now logged instead of disappearing.
 */
export async function getUserVote(choiceId: string, userId: string): Promise<"a" | "b" | null> {
  ...
  if (error) {
    logger.error("getUserVote failed", { error: error.message, choiceId, userId });
    return null;
  }
