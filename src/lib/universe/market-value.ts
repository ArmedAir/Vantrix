...
export async function getMarketValueLeaderboard(limit = 20): Promise<CharacterMarketValue[]> {
  ...
  const { data, error } = await supabaseAdmin
    .from('character_market_value')
    // LIST-DETAIL-PARITY FIX: this leaderboard had no visibility filter at
    // all — tickMarketValue() below only ever checks `active` when scoring,
    // so an unpublished or since-deactivated character's card could still
    // rank here and link to /characters/[id], which requires active,
    // is_public, AND is_live (see getCharacterDetail() / the identical fix
    // on /api/characters). `!inner` makes the join filterable; a character
    // row failing any of these now excludes its market-value row entirely
    // instead of returning it with a dead-end card.
    .select('*, character:characters!inner( id, name, image_url, active, is_public, is_live )')
