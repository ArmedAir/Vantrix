...
async function fetchResidentsForLocation(locationId: string, limit: number): Promise<LocationResident[]> {
  const { data: occupations, error } = await supabaseAdmin
    ...
    .select('character_id, employer, character:characters( id, name, image_url, occupation, active, is_public, is_live )')
  ...
  return occupations
    .map((row) => {
      const c = row.character as { id: string; name: string; image_url: string | null; occupation: string | null; active: boolean | null; is_public: boolean | null; is_live: boolean | null } | null;
      // LIST-DETAIL-PARITY FIX: was `active === false` only — a resident
      // link goes to /characters/[id], which requires active, is_public,
      // AND is_live (getCharacterDetail()). Matching that gate here so a
      // Residents card is never a dead end.
      if (!c || c.active === false || c.is_public === false || c.is_live === false) return null;
      return {
        id:         c.id,
        name:       c.name,
...
export async function getLocationBySlug(slug: string): Promise<LocationDetail | null> {
  ...
  const [
    ...
    supabaseAdmin.from('scarce_assets').select('*, holder:characters( id, name, image_url, active, is_public, is_live )').eq('location_id', location.id),
  ...
  return {
    ...
    assets:     ((assets ?? []) as (ScarceAsset & { holder?: { id: string; name: string; image_url: string; active?: boolean | null; is_public?: boolean | null; is_live?: boolean | null } | null })[]).map((a) => {
      const h = a.holder;
      // LIST-DETAIL-PARITY FIX: see fetchResidentsForLocation's identical
      // comment above — a holder link goes to /characters/[id].
...
export async function getFactionBySlug(slug: string): Promise<FactionDetail | null> {
  ...
  const { data: memberships } = await supabaseAdmin
    .from('faction_memberships')
    .select('character_id, role, is_public, joined_at, character:characters( id, name, image_url, active, is_public, is_live )')
  ...
  const members: FactionMemberRow[] = (memberships ?? []).map((m) => {
    const c = m.character as { id: string; name: string; image_url: string; active?: boolean | null; is_public?: boolean | null; is_live?: boolean | null } | null;
    // LIST-DETAIL-PARITY FIX: the page's own filter only checked
    // membership-level `is_public` and that the join resolved — not the
    // character's own active/is_public/is_live, which /characters/[id]
    // (getCharacterDetail) requires. Nulling the character here reuses
    // that existing `m.character &&` filter to drop the row, since the
    // link is built from `m.character_id` directly.
    const qualifies = !!c && c.active !== false && c.is_public !== false && c.is_live !== false;
    return {
      character_id: m.character_id,
      role:         m.role,
      is_public:    m.is_public,
      joined_at:    m.joined_at,
      character:    qualifies ? (c as { id: string; name: string; image_url: string }) : null,
