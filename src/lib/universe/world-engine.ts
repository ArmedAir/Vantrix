...
export async function getUniverseState(): Promise<UniverseState> {
  ...
  // Warm cache. Wrapped in try/catch (not just `.catch()`) for the same
  // reason as CircuitBreaker.reset() (circuit-breaker.ts): `redis` throws
  // SYNCHRONOUSLY on access when Redis env vars aren't set, so
  // `redis.set(...)` itself could throw before `.catch()` ever attaches —
  // which, uncaught, would break this function's "never throws" contract
  // (see BOOTSTRAP-FIX comment above) right before the successful `state`
  // below is returned.
  try {
    void redis.set(CACHE_K, state, { ex: CACHE_S }).catch(bg('worldEngine.cacheWarm.read'));
  } catch (err) {
    bg('worldEngine.cacheWarm.read')(err);
  }
...
export async function advanceUniverseTick(): Promise<UniverseState> {
  ...
  // Same fail-open wrapping as getUniverseState() above — a synchronous
  // Redis throw here must not turn a successful tick advance into a thrown
  // error for the caller.
  try {
    void redis.set(CACHE_K, next, { ex: CACHE_S }).catch(bg('worldEngine.cacheWarm.tick'));
  } catch (err) {
    bg('worldEngine.cacheWarm.tick')(err);
  }
  return next;
}
