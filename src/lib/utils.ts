...
/**
 * Compact social-proof formatting ("12.2M", "3M", "330K") — the
 * convention every polished consumer app uses for like/follower counts.
 * CompanionCard previously rendered these via raw `.toLocaleString()`
 * ("12,200,394"), which reads like an unstyled DB dump next to the rest
 * of the card's restrained, premium treatment. One decimal place above
 * 1,000, dropped when it would just be ".0" — never fabricates precision
 * the source count doesn't have.
 */
...
// CONSOLIDATED FALLBACK (2026-09-12): World hub (locations + factions) and
// Story Mode scenario covers previously had their own dedicated fallback
// assets (world-placeholder.png, scenario-placeholder.png) so a missing
// banner/cover never silently rendered a character's portrait as a
// stand-in for the wrong subject. Per direction, those separate placeholder
// images have been removed — CHARACTER_IMAGE_FALLBACK is now the single
// fallback used everywhere, including world/scenario call sites. These two
// names are kept as aliases (rather than touching every call site's
// import) so existing `fallback={WORLD_IMAGE_FALLBACK}` /
// `SCENARIO_IMAGE_FALLBACK` usage keeps working unchanged.
export const WORLD_IMAGE_FALLBACK = CHARACTER_IMAGE_FALLBACK;
export const SCENARIO_IMAGE_FALLBACK = CHARACTER_IMAGE_FALLBACK;
