...
// Search/social crawlers exempted from the first-visit → /enter redirect
// below, so root keeps getting indexed/previewed as the real marketing
// page rather than the interactive onboarding flow. Deliberately a plain
...
export const config = {
  matcher: [
    "/((?!_next/static|_next/image|favicon.ico|\\.well-known|.*\\.(?:svg|png|jpg|jpeg|gif|webp|ico|woff|woff2|ttf|eot)$).*)",
  ],
};
