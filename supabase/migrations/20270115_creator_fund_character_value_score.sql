...
-- Migration: 20270115_creator_fund_character_value_score.sql
-- ============================================================================
--
-- Adds a SECOND, additive creator-earnings stream on top of the existing
-- per-purchase RaaS marketplace (20260910_raas_unification.sql /
-- lib/commerce/raas.ts's raas_creator_earnings ledger). That stream pays a
-- creator when a specific buyer purchases a bond/soulbound tier on their
-- character. This one is a pooled royalty fund — like a Spotify/YouTube
-- creator fund — that pays out of a small, fixed slice of platform
-- subscription revenue, split across ALL monetization-eligible characters
-- by how much *genuine relationship engagement* they drove that period,
-- not by raw message/user counts. The two streams are tracked in separate
-- tables (raas_creator_earnings vs character_value_scores below) and composed
-- together only at the dashboard-read layer — see
-- lib/commerce/character-fund.ts's getCreatorDashboard().
--
-- Five pieces:
--   1. characters.monetization_status — a character is NOT enrolled in the
--      fund just by being public. A creator must explicitly upgrade it
--      (pay character_monetization_upgrade_fee, see app_config below),
--      same "pay to unlock a monetization surface" shape as raas_pricing's
--      bond/soulbound tiers, gated behind the character already being
--      active + is_public + moderation-approved.
--   2. app_config rows — every dial on the program (pool size, split,
--      upgrade fee) is admin-editable via the existing app_config table,
--      not hardcoded, so finance/product can tune this without a
--      deploy. Defaults below are DELIBERATELY conservative — see the
--      comment on creator_fund_eligible_revenue_pct.
--   3. compute_character_engagement_signals() — one set-based SQL
--      aggregation over conversations/messages/character_likes/
--      character_follows/character_relationships/user_reports for every
--      monetization-eligible character in a period, with the character's
--      OWN creator excluded from every signal (self-dealing can't even
--      enter the numbers, not just get penalized after the fact).
--   4. character_value_scores — the resulting ledger. One row per
--      (character, period). Mirrors raas_creator_earnings' payout_status
--      lifecycle (pending -> paid, or void) plus a pending_review state
--      for rows a fraud heuristic flagged.
--   5. creator_fund_flags — a non-blocking review queue, same shape as
--      abuse_signals (2026072800_abuse_signals_review_queue.sql): written
--      by the cron job, read/actioned by admins at /admin/safety. A flag
--      freezes only that character's payout for that period
--      (payout_status = 'pending_review'); it never blocks the character
--      itself or its other periods.
--
-- All the actual scoring math (usage weighting, the bounded quality
-- multiplier, normalization so a period's payouts always sum to exactly
-- that period's pool) lives in TypeScript
-- (lib/commerce/character-fund.ts), not here — this migration only ships
-- the schema, the admin-tunable config, and the SQL aggregation that's
-- genuinely cheaper done set-based in Postgres than row-by-row in JS.
-- ============================================================================
...
-- Only eligible characters are ever aggregated by
-- compute_character_engagement_signals() below — this index makes that
-- filter (and the admin "monetized characters" list) cheap.
...
--
-- One call per period covering every monetization-eligible character.
-- STABLE + SECURITY DEFINER so it can be invoked via supabaseAdmin.rpc()
-- from the weekly cron job without depending on RLS (the cron runs as
-- service_role already, but this keeps the function's own grants tight
-- and explicit rather than relying on caller privilege alone).
--
-- Every non_creator_messages / *_agg CTE below excludes the character's
-- own creator_id up front — see the module header's point (3). A
-- creator's own chats with their own character never enter active_users,
-- returning_users, saves, follows, progression, or session-minutes at
-- all; they only ever show up via creator_own_messages /
-- total_messages_incl_creator, which exist purely to power the
-- creator_self_usage_dominant fraud heuristic in TypeScript.
...
AS $$
  ...
  -- A "meaningful conversation" needs at least 6 messages in the period
  -- (a real back-and-forth, not a one-line drive-by) from a non-creator
  -- user. This is the module's direct answer to "don't simply count
  -- messages" — a conversation counts once here regardless of whether it
  -- has 6 messages or 600.
  meaningful AS (
  ...
  -- Per (character, user, day) session span, capped at 180 minutes/day so
  -- one forgotten open tab can't dominate the sum.
  session_minutes AS (
    SELECT character_id,
  ...
  -- NOTE: "saves" is in the schema and in USAGE_WEIGHTS for when it
  -- becomes available, but there is currently no per-event likes log to
  -- aggregate by period from — character_likes and character_follows
  -- were originally proposed together (see
  -- 20260804_character_likes_and_follows.sql's own header), but likes
  -- shipped as a cumulative `characters.liked_by` JSONB array + `like_count`
  -- column instead of a queryable table with created_at, while follows DID
  -- ship as a real table (follows_agg below). Hardcoded to 0 here rather
  -- than silently joining a table that doesn't exist; swap this out for a
  -- real per-period aggregate the moment a timestamped likes table exists.
  follows_agg AS (
  ...
  -- No per-period stage-change event log exists yet, so
  -- users_with_progression / milestones_reached are a snapshot proxy:
  -- relationships that were actually touched (updated_at) in this period
  -- and have moved past 'stranger'. Documented as a proxy, not exact —
  -- flagged here so it's the first thing to swap out if/when a real
  -- relationship-progression event log ships.
  progression_agg AS (
  ...
  SELECT
    ...
    0::INTEGER, -- saves: no timestamped likes log exists yet, see follows_agg's comment above
...
-- Second-pass, targeted heuristic: counts a character's "returning" users
-- (non-creator, active on >=2 distinct days this period) whose OWN account
-- was created within 3 days before the character itself was created —
-- i.e. a cluster of brand-new accounts that showed up right when the
-- character launched and kept coming back. Classic sockpuppet/farming
-- shape. Called from TypeScript only for characters that already look
-- like edge cases (small returning_users count) rather than for every
-- character every period, to keep this cheap.
...
CREATE TABLE IF NOT EXISTS character_value_scores (
  ...
  -- Raw signals as returned by compute_character_engagement_signals() —
  -- kept on the row (not just the derived numbers below) so the Creator
  -- Studio dashboard can show real counts ("12,483 active users"), not
  -- just an opaque earnings figure.
  ...
  -- Derived — see lib/commerce/character-fund.ts's computeCharacterValueScores()
  -- for the exact formulas. retention_rate is stored purely for display
  -- ("72% 7-day retention"-style cards); it does not independently affect
  -- payout beyond already being baked into quality_multiplier.
  ...
  usage_share          NUMERIC NOT NULL DEFAULT 0,  -- weighted_score / sum(weighted_score) across the period, 0-1; the actual normalized "Character Usage Share"
  retention_rate        NUMERIC NOT NULL DEFAULT 0,  -- returning_users / active_users

  -- Payout — same eligible_pool/gross/split/earned shape as raas_creator_earnings,
  -- extended with a pool snapshot and a review hold.
  eligible_pool_tokens  INTEGER  NOT NULL DEFAULT 0,  -- the WHOLE period's pool this row's share was cut from (same value on every row for a given period_start)
  gross_tokens          INTEGER  NOT NULL DEFAULT 0,  -- eligible_pool_tokens * usage_share, this character's cut before the platform split
  ...
  -- One row per character per period — the weekly cron is safe to retry;
  -- a second run for the same period no-ops via ON CONFLICT DO NOTHING
  -- (see character-fund.ts) rather than double-paying.
...
-- Same non-blocking review-queue shape as abuse_signals: written by the
-- cron job (service-role only, no client insert policy), read/actioned by
-- admins holding the 'creator_fund.review' permission at /admin/safety.
-- A flag here does not block the character or the creator — it only
-- holds that one period's payout row in 'pending_review' until a human
...
CREATE TABLE IF NOT EXISTS creator_fund_flags (
  ...
  -- 'creator_self_usage_dominant' | 'low_diversity_returning_users' |
  -- 'coordinated_new_accounts' | 'moderation_reports_spike'
  flag_type TEXT    NOT NULL,
  score     INTEGER NOT NULL,           -- 0-100, higher = more suspicious
...
-- Mirrors purchase_relationship_tier()'s shape: validate + charge the fee
-- + flip status in one transaction, so a mid-flight failure can never
-- charge the creator without enrolling the character, or enroll it for
-- free. Ownership, public/approval, and already-enrolled checks all live
-- here (not just in the route) so this function is safe to call from
-- anywhere, not just the one route that happens to check first today.
...
BEGIN
  ...
  -- insufficient_tokens on its own if the creator can't afford it, which
  -- aborts this whole transaction (the status flip below never runs).
