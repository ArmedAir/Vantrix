  ...
  theme: {
    extend: {
      ...
      keyframes: {
        ...
        // pulses — reads as "the logo has its own light source" rather
        // than the mark bouncing. Uses shadow-gold-glow (var(--gold-glow-
        // shadow)), which is already re-themed per [data-theme] in
        // globals.css, so this automatically matches whichever of the
        // four themes is active with zero theme-specific code here.
        "glow-pulse": {
          "0%, 100%": { opacity: "0.55" },
          "50%": { opacity: "1" },
        },
